...
/* Probably not able to use C99 semantics in gcc < 4.3.0 */
    ...
    !defined(C99_INLINE_SEMANTICS)
#define C99_INLINE_SEMANTICS 1
#endif

/* Apple's gcc build >5400 (since Xcode 3.0) doesn't support GNU inline in C99
   mode
...
#define CHECK_VECTOR_REAL(x)                                                   \
  do {                                                                         \
    if (TYPEOF(x) != REALSXP)                                                  \
      error("bad REALSXP vector");                                             \
  } while (0)
#define CHECK_VECTOR_CPLX(x)                                                   \
  do {                                                                         \
    if (TYPEOF(x) != CPLXSXP)                                                  \
      error("bad CPLXSXP vector");                                             \
  } while (0)
#define CHECK_VECTOR_RAW(x)                                                    \
  do {                                                                         \
    if (TYPEOF(x) != RAWSXP)                                                   \
      error("bad RAWSXP vector");                                              \
  } while (0)
#else
#define CHECK_VECTOR_LGL(x)                                                    \
  do {                                                                         \
...
#define CHECK_SCALAR_LGL(x)                                                    \
  do {                                                                         \
    CHECK_STDVEC_LGL(x);                                                       \
    if (XLENGTH(x) != 1)                                                       \
      error("bad LGLSXP scalar");                                              \
  } while (0)
#define CHECK_SCALAR_INT(x)                                                    \
  do {                                                                         \
    CHECK_STDVEC_INT(x);                                                       \
    if (XLENGTH(x) != 1)                                                       \
      error("bad INTSXP scalar");                                              \
  } while (0)
#define CHECK_SCALAR_REAL(x)                                                   \
  do {                                                                         \
    CHECK_STDVEC_REAL(x);                                                      \
    if (XLENGTH(x) != 1)                                                       \
      error("bad REALSXP scalar");                                             \
  } while (0)
#define CHECK_SCALAR_CPLX(x)                                                   \
  do {                                                                         \
    CHECK_STDVEC_CPLX(x);                                                      \
    if (XLENGTH(x) != 1)                                                       \
      error("bad CPLXSXP scalar");                                             \
  } while (0)
#define CHECK_SCALAR_RAW(x)                                                    \
  do {                                                                         \
    CHECK_STDVEC_RAW(x);                                                       \
    if (XLENGTH(x) != 1)                                                       \
      error("bad RAWSXP scalar");                                              \
  } while (0)
...
/* regular allocVector() as a special case of allocVector3() with no custom
 * allocator */
...
INLINE_FUN SEXP list1(SEXP s) { return CONS(s, R_NilValue); }
...
INLINE_FUN Rboolean isFunction(SEXP s) {
  return (TYPEOF(s) == CLOSXP || TYPEOF(s) == BUILTINSXP ||
          TYPEOF(s) == SPECIALSXP);
}
...
INLINE_FUN Rboolean
isVector(SEXP s) /* === isVectorList() or isVectorAtomic() */
{
  switch (TYPEOF(s)) {
  case LGLSXP:
  case INTSXP:
...
INLINE_FUN int stringPositionTr(SEXP string, const char *translatedElement) {
  ...
  for (i = 0; i < slen; i++) {
    Rboolean found =
        !strcmp(translateChar(STRING_ELT(string, i)), translatedElement);
    vmaxset(vmax);
    if (found)
      return i;
  }
