 ...
 */
static char *write_source_file(const char *func_name, InstructionInfo *insts, int inst_count, const char * const *opcode_names) {
    char dir_templ[] = "/tmp/rcp_jit_XXXXXX";
    if (!mkdtemp(dir_templ)) return NULL;
    ...
    /* Line 1: Prologue/Shim */
    fprintf(fp, "prologue\n");
    
    /* Lines 2..N+1: Opcodes */
    for (int i = 0; i < inst_count; i++) {
        const char *name = "unknown";
        if (insts[i].opcode >= 0 && opcode_names) {
            name = opcode_names[insts[i].opcode];
        }
        fprintf(fp, "%s\n", name);
    }
...
static void *create_debug_elf(const char *func_name,
    ...
    /* First pass: collect instruction info */
    int inst_capacity = 64;
    int inst_count = 0;
    InstructionInfo *insts = malloc(inst_capacity * sizeof(InstructionInfo));

    for (int i = 0; i < bytecode_count; ) {
        uint64_t addr = (uint64_t)inst_addrs[i];
        if (addr != 0) {
            if (inst_count >= inst_capacity) {
                inst_capacity *= 2;
                insts = realloc(insts, inst_capacity * sizeof(InstructionInfo));
            }

            int opcode = bytecode[i];
            int arg_count = (opcode >= 0 && opcode < num_opcodes && opcode_arg_counts)
                          ? opcode_arg_counts[opcode] : 0;

            /* Calculate instruction size */
            uint64_t next_addr = 0;
            for (int j = i + arg_count + 1; j < bytecode_count; j++) {
                if ((uint64_t)inst_addrs[j] != 0) {
                    next_addr = (uint64_t)inst_addrs[j];
                    break;
                }
            }
            uint64_t inst_size = (next_addr != 0) ? (next_addr - addr) : ((uint64_t)code_addr + code_size - addr);

            insts[inst_count].addr = addr;
            insts[inst_count].size = inst_size;
            insts[inst_count].opcode = opcode;
            inst_count++;
    ...
    char *source_path = write_source_file(func_name, insts, inst_count, opcode_names);
    if (!source_path) {
        free(insts);
        return NULL;
    }
    ...
    /* Header will be filled later, remember start */
    size_t info_start = 0;
    ...
    /* Line Number Program */
    buf_write_u8(&dbg_line, 0); // Extended
    buf_write_uleb128(&dbg_line, 1 + 8); // Length of instruction (1 byte opcode + 8 bytes addr)
    buf_write_u8(&dbg_line, DW_LNE_set_address);
    buf_write_u64(&dbg_line, (uint64_t)code_addr);
    
    /* Calculate shim size */
    uint64_t shim_size = 0;
    if (inst_count > 0 && insts[0].addr > (uint64_t)code_addr) {
        shim_size = insts[0].addr - (uint64_t)code_addr;
    } else if (inst_count == 0) {
        shim_size = code_size;
    }

    /* Start at Line 1 (Prologue) */
    /* If we have a shim, emit a line entry for it (Line 1) */
    if (shim_size > 0) {
        buf_write_u8(&dbg_line, DW_LNS_copy);
        
        /* Advance PC past shim */
        buf_write_u8(&dbg_line, DW_LNS_advance_pc);
        buf_write_uleb128(&dbg_line, shim_size);
    }
    
    /* Advance line from 1 to 2 (First Opcode) */
    buf_write_u8(&dbg_line, DW_LNS_advance_line);
    buf_write_sleb128(&dbg_line, 1);

    for (int i = 0; i < inst_count; i++) {
        /* Emit row for current address */
        buf_write_u8(&dbg_line, DW_LNS_copy);
        
        /* Advance PC */
        buf_write_u8(&dbg_line, DW_LNS_advance_pc);
        buf_write_uleb128(&dbg_line, insts[i].size);
    ...
    int num_opcodes)
{
    ...
    void *elf = create_debug_elf(func_name, code_addr, code_size,
                                  ...
                                  opcode_names, opcode_arg_counts, num_opcodes,
                                  &elf_size);
    if (!elf)
        return NULL;
