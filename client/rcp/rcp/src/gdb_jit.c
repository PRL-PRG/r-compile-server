...
static void build_debug_frame(Buffer *dbg_frame, void *code_addr,
  ...
  // Stencil CFI was generated by the compiler when the stencil was built as a
  // normal standalone function. That compiler-generated CFI encodes the stack
  // layout it "expects" at each instruction (e.g., CFA = RSP + 8 because a CALL
  // would have pushed a return address). However, we do not use the stencils
  // this way. At runtime, stencils are inlined into a JIT function after
  // _RCP_INIT. There is no CALL into each stencil, so there's no 8-byte return
  // address per stencil. Instead, the stack depth is determined by _RCP_INIT's
  // frame (base_cfa_offset = RCP_INIT_CFA_OFFSET computed by the extractor).
  //
  ...
  assert(stencils[0]->debug_frame);
  {
    ...
    // FDE layout:
    //   [0-3]   Length (4 bytes)
    //   [4-7]   CIE Pointer (4 bytes)
    //   [8-15]  PC begin (8 bytes)
  ...
  for (int i = 1; i < instruction_count; i++) {
    ...
    if (frame_data) {
      ...
      // Pop the state saved after the prologue (via remember_state).
      // This resets register rules and CFA offset to the known "clean" state
      // established right after the function prologue (_RCP_INIT).
      // This is crucial because the previous stencil might have left the stack
      // description in a "dirty" state (e.g., inside a push/pop sequence).
      buf_write_u8(dbg_frame, DW_CFA_restore_state);

      // Immediately push this "clean" state back onto the stack.
      // DWARF's restore_state is a pop operation (it removes the state).
      // We need to keep a copy of the clean state on the stack for the *next*
      // stencil iteration to restore from.
      buf_write_u8(dbg_frame, DW_CFA_remember_state);

      // We explicitly reset the CFA (Canonical Frame Address) offset to the
      // function's base stack depth. While restoring state should handle this,
      // explicit re-definition ensures the current stencil starts with the
      // assumption that SP is at the standard depth (base_cfa_offset)
      ...
      // buffer. This function adjusts the internal offset values within that
      // stream to match the JITted function's stack depth instead of the
      // stencil's original standalone stack depth.
      copy_cfi_with_adjusted_cfa(dbg_frame, cfi_start, cfi_end, &fde_last_addr,
                                 base_cfa_offset);
    }
  }
