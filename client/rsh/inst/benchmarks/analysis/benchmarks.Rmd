...
```{r timer-profiles}
timer_profiles <- select(timer_profiles, -TraceDate, -Workdir)
timer_profiles |>
  rename(name = file) |>
  left_join(speedups, by = "name") |>
  relocate(speedup, .after = name) |>
...
df <- df |> left_join(timer_profiles, by = c("name" = "file"))
...
y <- train[["speedup"]]
rf_model <- randomForest(x = x, y = y, data = train, importance = TRUE, ntree = 10000)
rf_model
```
