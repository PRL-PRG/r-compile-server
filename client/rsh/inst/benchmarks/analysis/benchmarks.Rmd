...
## Collating opcodes into categories 

```{r opcode-categories}
opcode_category <- local({
  category_map <- c(
    # Constants
    "LDCONST" = "Constant",
    "LDFALSE" = "Constant",
    ...
    # "Native" code
    "DOTCALL" = "External",
    "CALLBUILTIN" = "BuiltinSpecial",
    "CALLSPECIAL" = "BuiltinSpecial",
    # Type checking
    "ISNULL" = "TypeChecking", "ISOBJECT" = "TypeChecking", "ISCHARACTER" = "TypeChecking",
    "ISLOGICAL" = "TypeChecking", "ISINTEGER" = "TypeChecking", "ISCOMPLEX" = "TypeChecking",
    "ISSYMBOL" = "TypeChecking",
    "CHECKFUN" = "TypeChecking",
    ...
    # Variable lookup
    "GETVAR" = "Lookup",
    "GETVAR_MISSOK" = "Lookup",
    "GETFUN" = "Lookup",
    "GETBUILTIN" = "Lookup",
    "GETINTLBUILTIN" = "Lookup",
    "DOLLAR" = "Lookup",
    ...
    # Complex assigments
    "GETTERCAL" = "ComplexAssignment",
    "SETTERCAL" = "ComplexAssignment",
    "STARTASSIGN" = "ComplexAssignment",
    "ENDASSIGN" = "ComplexAssignment",
    "STARTASSIGN2" = "ComplexAssignment",
    "ENDASSIGN2" = "ComplexAssignment",
    "STARTSUBASSIGN" = "ComplexAssignment",
    "STARTSUBASSIGN_N" = "ComplexAssignment",
    "STARTSUBASSIGN2_N" = "ComplexAssignment",
    "VECSUBASSIGN" = "ComplexAssignment",
    "VECSUBASSIGN2" = "ComplexAssignment",
    "MATSUBASSIGN" = "ComplexAssignment",
    "MATSUBASSIGN2" = "ComplexAssignment",
    "DFLTSUBASSIGN" = "ComplexAssignment",
    "DOLLARGETSUB" = "ComplexAssignment",
    "GETTER_CALL" = "ComplexAssignment",
    "SETTER_CALL" = "ComplexAssignment",
    "DOLLARGETS" = "ComplexAssignment",

    # Subsetting
    "VECSUBSET" = "Subsetting",
    "VECSUBSET2" = "Subsetting",
    "MATSUBSET" = "Subsetting",
    "MATSUBSET2" = "Subsetting",
    "DFLTSUBSET" = "Subsetting",
    "DFLTSUBSET2" = "Subsetting",
    "STARTSUBSET" = "Subsetting",
    "STARTSUBSET2" = "Subsetting",
    "STARTSUBSET_N" = "Subsetting",
    "STARTSUBSET2_N" = "Subsetting",
  ...
  function(opcode) {
    res <- unname(category_map[opcode])
    if (is.na(res)) {
      # print(paste("Unknown opcode:", opcode))
      return(opcode)
    }
    return(res)
  }
...
```{r opcode-categories-apply}
categories_hits <- opcode_hits |>
  mutate(category = map_chr(opcode, opcode_category)) |>
  group_by(category) |>
  summarize(across(-opcode, sum, na.rm = TRUE), .groups = "drop")

categories_hits |>
  knitr::kable(digits = 2, caption = "Opcode categories")
```

Now, we look at the most used categories for all benchmarks:
```{r categories-top}
categories_top <- categories_hits |>
  pivot_longer(-category, names_to = "benchmark", values_to = "hits")
categories_top |>
  group_by(category) |>
  summarize(total_hits = sum(hits, na.rm = TRUE)) |>
  arrange(desc(total_hits)) |>
  mutate(pct = total_hits / sum(total_hits) * 100, cpct = cumsum(pct)) |>
  knitr::kable(digits = 2, caption = "Total hits per category")
```

## Number of opcodes  

```{r opcode-count, fig.width=12, fig.height=16}
# plot a bar plot of the number of static, executed opcodes, and ratio per benchmark
bc_profile_with_speedups <- left_join(bc_profiles, speedups, by = "name") |>
  select(name, nb_static_opcodes, nb_executed_opcodes, speedup) |>
  mutate(ratio = nb_executed_opcodes / nb_static_opcodes)
bc_profile_with_speedups |>
  pivot_longer(
    cols = c(nb_static_opcodes, nb_executed_opcodes),
    names_to = "type", values_to = "value"
  ) |>
  mutate(type = factor(type,
    levels = c("nb_static_opcodes", "nb_executed_opcodes"),
    labels = c("Static Opcodes", "Executed Opcodes")
  )) |>
  # Order by ratio
  ...
  geom_point(
    data = bc_profile_with_speedups,
    aes(x = name, y = speedup, color = "Speedup"),
    size = 3,
    inherit.aes = FALSE
  ) +
...
And now, a linear regression on the number of executed , static, and ratio of opcodes to see how they affect the speedup:

```{r opcode-regression}
lm_opcode <- lm(log1p(speedup) ~ nb_static_opcodes + nb_executed_opcodes + ratio, data = bc_profile_with_speedups)
summary(lm_opcode)
```
...
timer_profiles <- timer_profiles |>
  mutate(
    bultin_special_pct = (BuiltinSum_self + SpecialSum_self) / TotalRuntime * 100,
...
gap_stat <- clusGap(clustering_data_scaled,
...
```{r mclust, fig.width=12, fig.height=12}
# Breaks the notebook!
# library(mclust)
# gmm <- Mclust(clustering_data_scaled)
...
```{r datasets}
# Replace all columns names in df starting with <.Primitive>:something with
# <.Primitive>something, using stringr::str_replace
df <- df |>
  left_join(timer_profiles, by = "name") |>
  select(-TimerUnit)
...
```{r rf-model}
x <- train[, setdiff(names(train), "speedup")]
y <- train[["speedup"]]
rf_model <- randomForest(x = x, y = y, data = train, importance = TRUE, ntree = 10000, nodesize = 100)
rf_model
```
...
```{r importance-plot,  fig.width=12, fig.height=24}
...
library(xgboost)
xgb_train <- xgb.DMatrix(data = as.matrix(scale(x)), label = y)
