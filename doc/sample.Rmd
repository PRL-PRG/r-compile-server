...
- We can constant-fold expressions like `∀x:!obj[]. length(x) >= length(x)` by adding a GVN/CSE/e-graph analysis that looks for functions without "read-outside-state" effects that have equivalent inputs, produces an equivalent output.
...
- We should have explicit optimization for `1L:…` and `1:…` ranges, especially those where the start is a real but we can convert to an integer.
...
patchlabels <- function(cntxt:env) {
  ...
  ^3:
    ...
    goto ^4
  ^4(_5(_2:^1, ci:^3)):
    codeBuf.2:val[]@fresh = `[[`(codeBuf.1, _5)
    goto ^forStep
  ^forExit:
...
- I assume we have polymorphic type annotations on `lapply`. However, we may not even need them (or polymorphic effect annotations), because alternatively, we could specialize `lapply` for `int` and some other functions.
  ...
  - I also assume we can simplify `as.integer(lapply(…))` into `sapply(…)` (they're equivalent if the function returns an integer, at least excluding any edge cases I didn't realize).
...
- `@mutate`, `@borrow`, and `@fresh`: there are definitely some cases where we can elide copying that GNU-R can't (at least in the example I posted on Slack earlier), but there are also cases where GNU-R's runtime reference counting can elide copying but we can't, particularly in loops where the copying would be very frequent and expensive.
...
- There are some optimizations that I assumed, that are trivial in the specific case but I don't really know if we'll implement them in the general case. e.g. converting `as.integer(lapply(…))` into `sapply(…)`.
