...
/// Rewrite that replaces every dispatch and static callee with a better one.
///
...
///
/// e-graph equivalent of [org.prlprg.fir.opt.specialize.OptimizeCallee].
public record OptimizeCallee(int threshold) implements RewriteOptimization {
  @Override
  ...
  private @Nullable Callee run(Abstraction scope, AbstractionFeedback feedback, Call call) {
  ...
  private static Signature bestSignature(Callee callee, ImmutableList<Type> argumentTypes) {
    return org.prlprg.fir.opt.specialize.OptimizeCallee.bestSignature(callee, argumentTypes);
  }
}
