...
/// Rewrite that replaces [Load]s that statically resolve (via [OriginAnalysis]) to registers or
/// constants.
///
/// Currently only resolves loads in the local environment.
///
/// e-graph equivalent of [org.prlprg.fir.opt.specialize.ResolveLoad].
public record ResolveLoad() implements RewriteOptimization {
  @Override
