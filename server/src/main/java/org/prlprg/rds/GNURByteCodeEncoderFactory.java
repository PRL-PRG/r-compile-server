...
public class GNURByteCodeEncoderFactory {
    ...
    public int[] args(@NotNull BcInstr instr, ConstPool.Builder cpb) {
        return switch (instr) {
            ...
            case BcInstr.StartLoopCntxt i -> new int[]{i.isForLoop() ? 1 : 0, labelMapping.extract(i.end())};
            case BcInstr.EndLoopCntxt i -> new int[]{
                    i.isForLoop() ? 1 : 0,
            };
            case BcInstr.StartFor i -> new int[]{i.ast().idx(), i.elemName().idx(), labelMapping.extract(i.step())};
            ...
            case BcInstr.StartSubsetN i -> new int[]{
                    i.ast().idx(), labelMapping.extract(i.after()),
            };
            case BcInstr.StartSubassignN i -> new int[]{
                    i.ast().idx(), labelMapping.extract(i.after()),
            };
