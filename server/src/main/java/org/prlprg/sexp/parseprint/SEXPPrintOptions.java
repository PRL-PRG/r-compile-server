...
/// Pass this as a context to [#print], [#toString], and other functions that print an [SEXP] to
/// customize how it's printed.
///
...
///
/// @param printDelimited Ensure that all [SEXP]s except `NULL` and scalars are printed within
/// `<` and `>`.
///
/// Default: `false`.
///
...
public record SEXPPrintOptions(
  ...
  /// Print every part of the [SEXP], and guarantee it's surrounded by `<` and `>` if not
  /// `NULL` or scalar.
  ///
