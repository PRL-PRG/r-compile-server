    ...
    if (Files.exists(snapshotPath)) {
      try (var is = Files.newInputStream(snapshotPath)) {
        ...
        throw new RuntimeException(
            "Unable to load snapshot for %s (file: %s)".formatted(snapshotName, snapshotPath), e);
      }
    } else {
      if (oracle != null) {
        expected = oracle.get();
      ...
      try {
        ...
        throw new RuntimeException(
            "Unable to create snapshot path: " + snapshotPath.getParent(), e);
      }
      ...
      try (var os = Files.newOutputStream(snapshotPath)) {
        ...
        throw new RuntimeException(
            "Unable to save snapshot for %s (file: %s)".formatted(snapshotName, snapshotPath), e);
      }
    }
