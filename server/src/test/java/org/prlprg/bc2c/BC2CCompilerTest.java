...
//                        .asScalarString()
//                        .orElseThrow(
//                                () -> new IllegalArgumentException("Expected code as string, got:
// " + codeSxp));
//
//        var value = v.get(1);
//
...
//                        .asScalarString()
//                        .orElseThrow(
//                                () -> new IllegalArgumentException("Expected a string output, got:
// " + outputSxp));
//
...
//        return SEXPs.vec(SEXPs.string(result.code()), result.value(),
// SEXPs.string(result.output()));
//    }
//
...
//                ast2bc
//                        .compile()
//                        .orElseThrow(() -> new RuntimeException("Compilation did not produce byte
// code"));
//
//        try {
...
//                    R.capturingEval("source('%s',
// local=F)$value".formatted(rFile.getAbsolutePath()));
//            var pair = splitValueAndPC(nestedWithOutput.first());
//            var res = new TestResult(code, pair.first(), pair.second(),
// nestedWithOutput.second());
//
...
//
//            Files.copyURL(Objects.requireNonNull(getClass().getResource("Makefile")),
// makeFile.toPath());
