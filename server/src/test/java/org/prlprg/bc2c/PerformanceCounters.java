...
public record PerformanceCounters(int slowArith, int slowMath1, int slowUnary, int slowRelop) {
    ...
    public static PerformanceCounters from(SEXP sexp) {
        ...
        var fieldNames = Arrays.stream(fields).map(RecordComponent::getName).map(Strings::camelCaseToSnakeCase).toArray(String[]::new);

        var nums = v.coerceToInts();
        var names = v.names();
        if (names.size() != fields.length || names.size() != v.size()) {
            throw new IllegalArgumentException("Expected named int of size: " + fields.length + ", got: " + sexp);
        }
