#? stdlib
`spineplot.default` <- function (x, y = NULL, breaks = NULL, tol.ylab = 0.05, off = NULL, 
    ylevels = NULL, col = NULL, main = "", xlab = NULL, ylab = NULL, 
    xaxlabels = NULL, yaxlabels = NULL, xlim = NULL, ylim = c(0, 
        1), axes = TRUE, weights = NULL, ...) 
{
    if (missing(y)) {
        if (length(dim(x)) != 2L) 
        ...
        if (x.categorical) {
            if (is.null(weights)) {
                ...
                tab <- as.table(tapply(weights, list(x, y), FUN = sum, 
                  na.rm = TRUE))
                tab[is.na(tab)] <- 0
            }
            xnam <- levels(x)
    ...
    if (x.categorical) {
        xat <- c(0, cumsum(proportions(marginSums(tab, 1)) + 
            off))
        ...
        if (is.null(breaks)) {
            breaks <- if (is.null(weights)) 
                nclass.Sturges(x)
            else ceiling(log2(sum(weights)) + 1)
        }
        ...
        if (length(breaks) == 1L) {
            ...
            rg <- if (is.null(weights)) 
                range(x, na.rm = TRUE)
            else range(x[weights > 0], na.rm = TRUE)
        ...
        if (is.null(weights)) {
            ...
            tab <- as.table(tapply(weights, list(x1, y), FUN = sum, 
                na.rm = TRUE))
            tab[is.na(tab)] <- 0
        }
        xat <- c(0, cumsum(proportions(marginSums(tab, 1))))
    ...
    yat <- rbind(0, apply(proportions(tab[, ncol(tab):1L, drop = FALSE], 
        1), 1L, cumsum))
    yat[is.na(yat)] <- 1
    if (is.null(xlim)) 
