...
`rbind` <- function (..., deparse.level = 1) 
{
    ...
    nmsym <- names(symarg)
    nm <- c(function(i) NULL, function(i) if (is.symbol(s <- symarg[[i]])) deparse(s) else NULL, 
    ...
    for (i in (na - 1L):1L) {
        d2 <- dim(r)
        r <- rbind2(argl[[i]], r)
        ism1 <- !is.null(d1 <- dim(argl[[i]])) && length(d1) == 
            2L
        ism2 <- !is.null(d2) && length(d2) == 2L
        if (ism1 && ism2) 
            next
        nn1 <- !is.null(N1 <- if ((l1 <- Nrow(argl[[i]])) && 
            !ism1) Nms(i))
        nn2 <- !is.null(N2 <- if (i == na - 1L && Nrow(argl[[na]]) && 
            !ism2) Nms(na))
        if (nn1 || nn2) {
