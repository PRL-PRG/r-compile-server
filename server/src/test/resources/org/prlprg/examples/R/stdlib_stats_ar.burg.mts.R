#? stdlib
`ar.burg.mts` <- function (x, aic = TRUE, order.max = NULL, na.action = na.fail, 
    demean = TRUE, series = NULL, var.method = 1L, ...) 
{
    ...
    z <- .C(C_multi_burg, as.integer(n.used), resid = as.double(x), 
    ...
    order <- z$order
    ar <- if (order) 
        -aperm(array(z$coefs, dim = c(nser, nser, order.max + 
