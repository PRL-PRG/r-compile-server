...
`.build_packages` <- function (args = NULL, no.q = interactive()) 
{
    ...
    Ssystem <- function(command, args = character(), ...) system2(command, 
        args, stdout = NULL, stderr = NULL, ...)
    do_exit <- if (no.q) 
        function(status) (if (status) 
            stop
        else message)(".build_packages() exit status ", status)
    else function(status) q("no", status = status, runLast = FALSE)
    parse_description_field <- function(desc, field, default = TRUE, 
        logical = TRUE) str_parse(desc[field], default = default, 
        logical = logical)
    Usage <- function() {
        cat("Usage: R CMD build [options] pkgdirs", "", "Build R packages from package sources in the directories specified by", 
            sQuote("pkgdirs"), "", "Options:", "  -h, --help\t\tprint short help message and exit", 
            ...
            "  --md5                 add MD5 sums", "  --log                 log to file 'pkg-00build.log' when processing ", 
    ...
    add_build_stamp_to_description_file <- function(ldpath, pkgdir, 
        ...
        db["Packaged"] <- sprintf("%s; %s", format(Sys.time(), 
            "%Y-%m-%d %H:%M:%S", tz = "UTC", usetz = TRUE), user)
        fields <- .expand_package_description_db_R_fields(db)
        .write_description(if (length(fields)) 
            c(db, fields)
        else db, ldpath)
    }
    temp_install_pkg <- function(pkgdir, libdir) {
        dir.create(libdir, mode = "0755", showWarnings = FALSE)
        if (nzchar(install_dependencies) && all((repos <- getOption("repos")) != 
            "@CRAN@")) {
            available <- utils::available.packages(repos = repos)
            db <- .read_description(file.path(pkgdir, "DESCRIPTION"))
            package <- db["Package"]
            available <- rbind(available[available[, "Package"] != 
                package, , drop = FALSE], db[colnames(available)])
            depends <- package_dependencies(package, available, 
                which = install_dependencies)
            depends <- setdiff(unlist(depends), rownames(utils::installed.packages()))
            if (length(depends)) {
                message(paste(strwrap(sprintf("installing dependencies %s", 
                  paste(sQuote(sort(depends)), collapse = ", ")), 
                  exdent = 2L), collapse = "\n"), domain = NA)
                utils::install.packages(depends, libdir, available = available[-nrow(available), 
                  , drop = FALSE], dependencies = NA)
            }
        }
        if (WINDOWS) {
            cmd <- file.path(R.home("bin"), "Rcmd.exe")
            args <- c("INSTALL -l", shQuote(libdir), "--no-multiarch", 
                shQuote(pkgdir))
        }
        else {
            cmd <- file.path(R.home("bin"), "R")
            args <- c("CMD", "INSTALL -l", shQuote(libdir), "--no-multiarch", 
                shQuote(pkgdir))
        }
    ...
    prepare_pkg <- function(pkgdir, desc, Log) {
        ...
        ensure_installed <- function() if (!pkgInstalled) {
            messageLog(Log, "installing the package to build vignettes")
            pkgInstalled <<- temp_install_pkg(pkgdir, libdir)
        }
        pkgInstalled <- build_Rd_db(pkgdir, libdir, desc)
        if (file.exists("INDEX")) 
            update_Rd_index("INDEX", "man", Log)
        doc_dir <- file.path("inst", "doc")
        if ("makefile" %in% dir(doc_dir)) {
            messageLog(Log, "renaming 'inst/doc/makefile' to 'inst/doc/Makefile'")
            file.rename(file.path(doc_dir, "makefile"), file.path(doc_dir, 
                "Makefile"))
        }
        if (vignettes && parse_description_field(desc, "BuildVignettes", 
            TRUE)) {
            ...
            if (!is.null(vigns) && length(vigns$docs)) {
                ...
                if (!is.na(R_LIBS)) {
                  ...
                  Sys.setenv(R_LIBS = path_and_libPath(libdir, 
                    R_LIBS))
                }
                else {
                ...
                args <- c("--vanilla", "--default-packages=", 
                ...
                if (res$status) {
                  ...
                  vigns <- pkgVignettes(dir = ".", output = TRUE, 
                    source = TRUE)
                ...
                if (basename(vigns$dir) == "vignettes") {
                  ...
                  if (!all(copied)) {
                    warning(sprintf(ngettext(sum(!copied), "%s file\n", 
                      "%s files\n"), sQuote("inst/doc")), strwrap(paste(sQuote(basename(tocopy[!copied])), 
                      collapse = ", "), indent = 4, exdent = 2), 
                  ...
                  if (file.exists(extras_file)) {
                    ...
                    if (length(extras)) {
                      ...
                      for (e in extras) inst <- inst | grepl(e, 
                        allfiles, perl = TRUE, ignore.case = TRUE)
                ...
                saveRDS(vignetteIndex, file = vignette_index_path, 
                  version = 2L)
            }
        }
        else {
            ...
            if (file.exists(fv)) {
                ...
                db <- readRDS(fv)
                pdfs <- file.path("inst", "doc", db[nzchar(db$PDF), 
                  ]$PDF)
                missing <- !file.exists(pdfs)
                if (any(missing)) {
                  msg <- c("Output(s) listed in 'build/vignette.rds' but not in package:", 
                    strwrap(sQuote(pdfs[missing]), indent = 2L, 
                      exdent = 2L), "Run R CMD build without --no-build-vignettes to re-create")
        ...
        if (compact_vignettes != "no" && length(pdfs <- dir(doc_dir, 
            ...
            qpdf <- if (compact_vignettes %in% c("qpdf", "gs+qpdf", 
                "both")) 
                Sys.which(Sys.getenv("R_QPDF", "qpdf"))
            else ""
            res <- compactPDF(pdfs, qpdf = qpdf, gs_cmd = gs_cmd, 
    ...
    cleanup_pkg <- function(pkgdir, Log) {
        ...
        if (dir.exists("src")) {
            ...
            if (WINDOWS) {
                have_make <- nzchar(Sys.which(Sys.getenv("MAKE", 
                  "make")))
                if (file.exists(fn <- "Makefile.ucrt") || file.exists(fn <- "Makefile.win")) {
                  if (have_make) 
                    Ssystem(Sys.getenv("MAKE", "make"), paste0("-f ", 
                      fn, " clean"))
                  ...
                  if (file.exists(fn <- "Makevars.ucrt") || file.exists(fn <- "Makevars.win")) {
                    if (have_make) {
                      makefiles <- paste("-f", shQuote(file.path(R.home("share"), 
                        "make", "clean.mk")), "-f", fn)
                      Ssystem(Sys.getenv("MAKE", "make"), c(makefiles, 
                        "clean"))
                    }
                    else warning("unable to run 'make clean' in 'src'", 
                      domain = NA)
                  }
                  unlink(c(Sys.glob(c("*.o", "*.so", "*.dylib", 
                    "*.mod")), paste0(pkgname, c(".a", ".dll", 
                    ".def")), "symbols.rds"))
                  if (dir.exists(".libs")) 
                    unlink(".libs", recursive = TRUE)
                  if (dir.exists("_libs")) 
                    unlink("_libs", recursive = TRUE)
                }
            }
            else {
                makefiles <- paste("-f", shQuote(file.path(R.home("etc"), 
                  Sys.getenv("R_ARCH"), "Makeconf")))
                if (file.exists("Makefile")) {
                  ...
                  if (file.exists("Makevars")) {
                    makefiles <- paste(makefiles, "-f", shQuote(file.path(R.home("share"), 
                      "make", "clean.mk")), "-f Makevars")
                    Ssystem(Sys.getenv("MAKE", "make"), c(makefiles, 
                      "clean"))
                  }
                  unlink(c(Sys.glob(c("*.o", "*.so", "*.dylib", 
    ...
    update_Rd_index <- function(oldindex, Rd_files, Log) {
        newindex <- tempfile()
        res <- tryCatch(Rdindex(Rd_files, newindex), error = function(e) {
            errorLog(Log, "computing Rd index failed:", conditionMessage(e))
            do_exit(1L)
        })
    ...
    build_Rd_db <- function(pkgdir, libdir, desc) {
        build_partial_Rd_db_path <- file.path("build", "partial.rdb")
        if (file.exists(build_partial_Rd_db_path)) 
            unlink(build_partial_Rd_db_path)
        build_refman_path <- file.path(pkgdir, "build", paste0(basename(pkgdir), 
            ".pdf"))
        if (file.exists(build_refman_path)) 
            unlink(build_refman_path)
        db <- .build_Rd_db(pkgdir, stages = NULL, os = c("unix", 
            "windows"), step = 1)
        if (!length(db)) 
            return(FALSE)
        names(db) <- substring(names(db), nchar(file.path(pkgdir, 
            "man")) + 2L)
        btinfo <- do.call(rbind, lapply(db, .Rd_get_Sexpr_build_time_info))
        if (!any(btinfo[, "\\Sexpr"])) {
            return(FALSE)
        }
        else {
        ...
        if (length(containsLaterSexprs)) {
            for (i in containsLaterSexprs) {
                db[[i]] <- prepare_Rd(db[[i]], stages = c("install", 
                  "render"), stage2 = FALSE, stage3 = FALSE)
            }
            stage23 <- db[containsLaterSexprs]
            dir.create("build", showWarnings = FALSE)
            build_stage23_Rd_db_path <- file.path("build", "stage23.rdb")
        ...
        needRefman <- manual && parse_description_field(desc, 
            "BuildManual", FALSE) && any(btinfo[, "later"])
        if (needRefman) {
            messageLog(Log, "building the PDF package manual")
            dir.create("build", showWarnings = FALSE)
            ..Rd2pdf(c("--force", "--no-preview", "--quiet", 
                paste0("--output=", build_refman_path), pkgdir), 
                quit = FALSE)
        }
        return(TRUE)
    }
    fix_nonLF_in_files <- function(pkgname, dirPattern, Log) {
        ...
        files <- dir(sDir, pattern = dirPattern, full.names = TRUE, 
            recursive = TRUE)
        for (ff in files) {
    ...
    fix_nonLF_in_source_files <- function(pkgname, Log) {
        fix_nonLF_in_files(pkgname, dirPattern = "\\.([cfh]|cc|cpp|hpp)$", 
            Log)
    }
    fix_nonLF_in_make_files <- function(pkgname, Log) {
        fix_nonLF_in_files(pkgname, paste0("^(", paste(c("Makefile", 
            "Makefile.in", "Makefile.win", "Makefile.ucrt", "Makevars", 
            "Makevars.in", "Makevars.win", "Makevars.ucrt"), 
            collapse = "|"), ")$"), Log)
        makes <- dir(pkgname, pattern = "^Makefile$", full.names = TRUE, 
            recursive = TRUE)
        for (ff in makes) {
            lines <- readLines(ff, warn = FALSE)
            writeLinesNL(lines, ff)
    ...
    find_empty_dirs <- function(d) {
        ...
        if (length(files) <= 2L) {
            if (keep_empty1) {
                printLog(Log, "WARNING: directory ", sQuote(d), 
                  " is empty\n")
            }
            else {
    ...
    fixup_R_dep <- function(pkgname, ver = "2.10") {
        ...
        for (dep in Rdeps) {
            if (dep$op != ">=") 
                next
            if (dep$version >= package_version(ver)) 
                return()
        }
        flatten <- function(x) {
            if (length(x) == 3L) 
        ...
        printLog(Log, "  NB: this package now depends on R (>= ", 
            ver, ")\n")
    }
    resave_data_rda <- function(pkgname, resave_data) {
        ...
        if (resave_data == "best") {
            files <- Sys.glob(c(file.path(ddir, "*.rda"), file.path(ddir, 
                "*.RData"), file.path(pkgname, "R", "sysdata.rda")))
            ...
            rdas <- checkRdaFiles(Sys.glob(c(file.path(ddir, 
                "*.rda"), file.path(ddir, "*.RData"))))
            if (nrow(rdas)) {
    ...
    resave_data_others <- function(pkgname, resave_data) {
        ...
        dataFiles <- filtergrep("\\.(rda|RData)$", list_files_with_type(ddir, 
            "data"))
        if (!length(dataFiles)) 
            return()
        resaved <- character()
        ...
        if (length(Rs)) {
            ...
            lapply(Rs, function(x) {
                ...
                save(list = ls(envir, all.names = TRUE), file = sub("\\.[Rr]$", 
                  ".rda", x), compress = TRUE, compression_level = 9, 
                  envir = envir, version = 2L)
                resaved <<- c(resaved, x)
            })
            printLog(Log, "  NB: *.R converted to .rda: other files may need to be removed\n")
        }
        tabs <- grep("\\.(CSV|csv|TXT|tab|txt)$", dataFiles, 
            value = TRUE)
        if (length(tabs)) {
            ...
            if (resave_data == "gzip") {
                ...
                lapply(tabs, function(nm) {
                  ...
                  con <- gzfile(nm3[1L], "wb", compression = 9L)
                  writeLines(x, con)
                  close(con)
                  con <- bzfile(nm3[2L], "wb", compression = 9L)
                  writeLines(x, con)
                  close(con)
                  con <- xzfile(nm3[3L], "wb", compression = 9L)
                  writeLines(x, con)
                  close(con)
    ...
    if (!is.na(Renv)) {
        ...
        if (nzchar(rarch) && file.exists(Renv <- paste0("~/.R/build.Renviron.", 
            rarch))) 
            readRenviron(Renv)
        else if (file.exists(Renv <- "~/.R/build.Renviron")) 
            readRenviron(Renv)
    }
    compact_vignettes <- Sys.getenv("_R_BUILD_COMPACT_VIGNETTES_", 
        "no")
    resave_data <- Sys.getenv("_R_BUILD_RESAVE_DATA_", "gzip")
    keep_empty <- config_val_to_logical(Sys.getenv("_R_BUILD_KEEP_EMPTY_DIRS_", 
        "FALSE"))
    install_dependencies <- Sys.getenv("_R_BUILD_INSTALL_DEPENDENCIES_")
    if (nzchar(install_dependencies) && (install_dependencies %notin% 
        c("strong", "most", "all"))) 
        install_dependencies <- if (config_val_to_logical(install_dependencies)) 
            "most"
        else ""
    if (is.null(args)) {
    ...
    while (length(args)) {
        ...
        if (a %in% c("-h", "--help")) {
            ...
            cat("R add-on package builder: ", R.version[["major"]], 
            ...
            compression <- match.arg(substr(a, 15, 1000), c("none", 
                "gzip", "bzip2", "xz"))
        }
    ...
    if (compact_vignettes %notin% c("no", "qpdf", "gs", "gs+qpdf", 
        "both")) {
    ...
    for (pkg in pkgs) {
        ...
        if (is.na(intname <- desc["Package"]) || !length(intname) || 
            !nzchar(intname)) {
            errorLog(Log, "invalid 'Package' field")
            do_exit(1L)
        }
        ...
        if (WINDOWS) {
            ...
            ver <- suppressWarnings(system2("cp", "--version", 
                stdout = TRUE, stderr = FALSE))
            GNU_cp <- any(grepl("GNU coreutils", ver))
            cp_sw <- if (GNU_cp) 
                "-LR --preserve=timestamps"
            else "-pLR"
        ...
        exclude <- exclude | (isdir & (bases %in% c("check", 
            "chm", .vc_dir_names)))
        exclude <- exclude | (isdir & grepl("([Oo]ld|\\.Rcheck)$", 
            bases))
        exclude <- exclude | bases %in% c("Read-and-delete-me", 
            "GNUMakefile")
        exclude <- exclude | startsWith(bases, "._")
        exclude <- exclude | (isdir & grepl("^src.*/[.]deps$", 
            allfiles))
        exclude <- exclude | (allfiles == paste0("src/", pkgname, 
            "_res.rc"))
        ...
        exclude <- exclude | grepl(paste0("^", pkgname, "_[0-9.-]+", 
            exts, "$"), allfiles)
        ...
        setwd(Tdir)
        if (!WINDOWS) 
            .Call(C_dirchmod, pkgname, group.writable = FALSE)
        add_build_stamp_to_description_file(file.path(pkgname, 
            "DESCRIPTION"), pkgdir, user)
        messageLog(Log, "checking for LF line-endings in source and make files and shell scripts")
        ...
        for (dir in c("Meta", "R-ex", "chtml", "help", "html", 
            "latex")) {
            d <- file.path(pkgname, dir)
            if (dir.exists(d)) {
                msg <- paste("WARNING: Removing directory", sQuote(d), 
                  "which should only occur", "in an installed package")
                printLog(Log, paste(strwrap(msg, indent = 0L, 
                  exdent = 2L), collapse = "\n"), "\n")
                unlink(d, recursive = TRUE)
            }
        }
        unlink(file.path(pkgname, c("src-i386", "src-x64", "src-x86_64", 
            "src-ppc")), recursive = TRUE)
        if (dir.exists(file.path(pkgname, "data")) || file_test("-f", 
            ...
            if (!str_parse_logic(desc["LazyData"], FALSE)) {
                ...
                tryCatch(add_datalist(pkgname), error = function(e) printLog(Log, 
                  "  unable to create a 'datalist' file: may need the package to be installed\n"))
            }
        ...
        if (!dir.exists(file.path(pkgname, "data"))) {
            ...
            ndb <- names(db)
            omit <- character()
            for (x in c("LazyData", "LazyDataCompression")) if (x %in% 
                ndb) 
                omit <- c(omit, x)
            if (length(omit)) {
                printLog(Log, sprintf("Omitted %s from DESCRIPTION\n", 
        ...
        if (!hasDep350) {
            ...
            if (length(toonew)) {
                ...
                printLog(Log, paste(c(strwrap(msg, indent = 2L, 
                  exdent = 2L), paste0("  ", .pretty_format(sort(toonew)))), 
                  collapse = "\n"), "\n")
            }
        }
        ...
        ext <- switch(compression, none = "", gzip = ".gz", bzip2 = ".bz2", 
            xz = ".xz")
        filename <- paste0(pkgname, "_", desc["Version"], ".tar", 
            ext)
        filepath <- file.path(startdir, filename)
        messageLog(Log, "building ", sQuote(filename))
        res <- utils::tar(filepath, pkgname, compression = compression, 
            compression_level = 9L, tar = Sys.getenv("R_BUILD_TAR"), 
            extra_flags = NULL)
        if (res) {
