...
`.check_packages` <- function (args = NULL, no.q = interactive(), warnOption = 1) 
{
    ...
    R_runR0 <- function(..., timeout = def_tlim) R_runR(..., 
        timeout = timeout)
    R_runR2 <- if (WINDOWS) {
    ...
    else function(cmd, env = "R_DEFAULT_PACKAGES='utils,grDevices,graphics,stats'", 
        timeout = 0) {
        out <- R_runR(cmd, R_opts2, env, timeout = timeout)
    ...
    snapshot <- function() {
        ...
        xtra <- Sys.getenv("_R_CHECK_THINGS_IN_OTHER_DIRS_XTRA_", 
            "")
        xtra <- if (nzchar(xtra)) 
            strsplit(xtra, ";", fixed = TRUE)[[1L]]
        else character()
        dirs <- c(home, dirname(tempdir()), "/dev/shm", if (.Platform$OS.type == 
            ...
            ".cache"), if (.Platform$OS.type == "windows") file.path(Sys.getenv("APPDATA"), 
            "R", "data") else if (Sys.info()["sysname"] == "Darwin") file.path(home, 
            "Library", "Application Support", "org.R-project.R") else file.path(home, 
            ".local", "share"), xtra)
        x <- vector("list", length(dirs))
        names(x) <- dirs
        ...
        for (i in seq_along(xtra)) x[[4 + i]] <- snap1(dirs[4 + 
            i], FALSE, user, udomain)
        x
    }
    ...
    check_pkg <- function(pkg, pkgname, pkgoutdir, startdir, 
        ...
        if (dir.exists(pkg)) {
            ...
            errorLog(Log, "Package directory ", sQuote(pkg), 
                " does not exist.")
            summaryLog(Log)
            do_exit(1L)
        }
        if (config_val_to_logical(Sys.getenv("_R_CHECK_FUTURE_FILE_TIMESTAMPS_", 
            ...
            if (config_val_to_logical(Sys.getenv("_R_CHECK_SYSTEM_CLOCK_", 
                ...
                now <- tryCatch({
                  foo <- suppressWarnings(readLines("https://worldtimeapi.org/api/timezone/etc/UTC", 
                    warn = FALSE))
                  as.POSIXct(gsub(".*\"datetime\":\"([^Z]*).*", 
                    "\\1", foo), "UTC", "%Y-%m-%dT%H:%M:%S")
                }, error = function(e) NA)
                if (identical(NA, now)) {
                  now <- tryCatch({
                ...
                if (FALSE && identical(NA, now)) {
                  now <- tryCatch({
                    foo <- suppressWarnings(readLines("http://worldclockapi.com/api/json/utc/now", 
                      warn = FALSE))
                    as.POSIXct(gsub(".*\"currentDateTime\":\"([^Z]*).*", 
                      "\\1", foo), "UTC", "%Y-%m-%dT%H:%M")
                  }, error = function(e) NA)
                }
                if (identical(NA, now)) {
                  any <- TRUE
                  ...
                  if (abs(unclass(now_local) - unclass(now)[1]) > 
                    300) {
                    any <- TRUE
                    fmt <- "%Y-%m-%d %H:%M"
            ...
            leeway <- Sys.getenv("_R_CHECK_FUTURE_FILE_TIMESTAMPS_LEEWAY_", 
                "5m")
        ...
        if (!extra_arch) {
            ...
            if (!is_base_pkg && haveR && R_check_ascii_code) 
                check_non_ASCII()
        }
        if (do_install && pkgname != "base") {
            if (this_multiarch) {
                ...
                for (arch in inst_archs) {
                  printLog(Log, "* loading checks for arch ", 
                    sQuote(arch), "\n")
                  check_loading(arch)
                }
                Log$stars <<- "*"
        ...
        if (!is_base_pkg && !extra_arch) 
            check_src_dir(desc)
        check_src()
        if (do_install && dir.exists("src") && length(so_symbol_names_table)) 
            check_sos()
        if (dir.exists("src") && length(files <- dir("src", pattern = "[.](f|f90|f95)$"))) {
            checkingLog(Log, "usage of KIND in Fortran files")
            bad <- character()
            details <- character()
            for (f in files) {
                lines <- readLines(file.path("src", f), warn = FALSE)
                lines <- grep("^([cC]| *!)", lines, value = TRUE, 
                  invert = TRUE, useBytes = TRUE)
                if (any(z <- grepl("[(].*(kind|KIND) *= *[1-9].*[)]", 
                  lines, useBytes = TRUE))) {
                  bad <- c(bad, f)
            ...
            if (!length(bad)) 
                ...
                warningLog(Log)
                msg <- ngettext(length(bad), "Found the following file with non-portable usage of KIND:\n", 
                  "Found the following files with non-portable usage of KIND:\n", 
                  domain = NA)
                wrapLog(msg)
                verbose <- config_val_to_logical(Sys.getenv("_R_CHECK_FORTRAN_KIND_DETAILS_", 
                  "FALSE"))
                if (verbose) 
        ...
        if (any(f <- file.exists(miss))) {
            ...
            warningLog(Log)
            printLog(Log, paste("  file", paste(sQuote(miss[f]), 
                collapse = ", "), "will not be installed: please remove it\n"))
        }
        if (dir.exists("inst/doc")) {
            if (R_check_doc_sizes) 
                check_doc_size()
            else if (as_cran) 
                warningLog(Log, "'qpdf' is needed for checks on size reduction of PDFs")
        }
        if (dir.exists("inst/doc") && do_install) 
            check_doc_contents()
        if (dir.exists("vignettes")) 
            check_vign_contents(ignore_vignettes)
        setwd(pkgoutdir)
        ...
        if (dir.exists(tests_dir) && length(dir(tests_dir, pattern = "\\.(R|r|Rin)$"))) 
            run_tests()
        setwd(pkgoutdir)
        if (!ignore_vignettes) 
            run_vignettes(desc)
    ...
    check_file_names <- function() {
        ...
        bad <- vapply(strsplit(basename(allfiles[is_man]), ""), 
            function(x) any(grepl("[^ -~]|%", x)), NA)
        if (length(bad)) 
            bad_files <- c(bad_files, (allfiles[is_man])[bad])
        bad <- tolower(basename(allfiles))
        bad <- sub("[.].*", "", bad)
        bad <- grepl("^(con|prn|aux|clock[$]|nul|lpt[1-9]|com[1-9])$", 
            bad)
        bad_files <- c(bad_files, allfiles[bad])
        if (nb <- length(bad_files)) {
            errorLog(Log)
            msg <- ngettext(nb, "Found the following file with a non-portable file name:\n", 
        ...
        non_ASCII_files <- allfiles[grepl("[^-A-Za-z0-9._!#$%&+,;=@^(){}'[\\]]", 
        ...
        if (any(lens > 100L)) {
            ...
            msg <- ngettext(length(bad_files), "Found the following non-portable file path:\n", 
    ...
    check_permissions <- function(allfiles) {
        ...
        if (length(alldirs <- unique(dirname(allfiles)))) {
            ...
            bad_files <- c(bad_files, alldirs[(mode & "700") < 
                as.octmode("700")])
        }
        if (length(bad_files)) {
            errorLog(Log)
    ...
    check_serialization <- function(allfiles) {
        ...
        if (length(bad)) {
            ...
            wrapLog("Such files are only readable in R >= 3.5.0.\n", 
                "Recreate them with R < 3.5.0 or", "save(version = 2) or saveRDS(version = 2)", 
                "as appropriate")
        }
        else resultLog(Log, "OK")
    }
    check_meta <- function() {
        ...
        dfile <- if (is_base_pkg) 
            "DESCRIPTION.in"
        else "DESCRIPTION"
        any <- FALSE
        Rcmd <- sprintf("tools:::.check_package_description_encoding(\"%s\")", 
            dfile)
        ...
        out <- R_runR0(Rcmd, R_opts2, "R_DEFAULT_PACKAGES=NULL")
        if (length(out)) {
            if (any(!grepl("^Malformed (Title|Description)", 
                out))) {
                errorLog(Log)
                printLog0(Log, paste(out, collapse = "\n"), "\n")
                summaryLog(Log)
        ...
        check_license <- if (!is_base_pkg) {
            Check_license <- Sys.getenv("_R_CHECK_LICENSE_", 
                NA_character_)
        ...
        if (!is.na(aar <- db["Authors@R"])) {
            ...
            if (check_incoming && any(!is.na(yorig))) {
                ...
                y <- c(Author = utils:::.format_authors_at_R_field_for_author(aar), 
                  Maintainer = utils:::.format_authors_at_R_field_for_maintainer(aar))
                clean_up <- function(x) trimws(gsub("[[:space:]]+", 
                  " ", x))
                yorig <- sapply(yorig, clean_up)
                y <- sapply(y, clean_up)
                diff <- y != yorig
                if (diff[1L] && grepl("https://orcid.org/", y[1L], 
                  fixed = TRUE)) {
        ...
        db <- .read_description(dfile)
        fields <- c("LazyData", "KeepSource", "ByteCompile", 
            "UseLTO", "StagedInstall", "Biarch", "BuildVignettes")
        bad <- fields[vapply(fields, function(f) {
            !is.na(x <- db[f]) && suppressWarnings(is.na(utils:::str2logical(x)))
        }, NA)]
        if (length(bad)) {
            if (!any) 
                noteLog(Log)
            any <- TRUE
            printLog(Log, paste(c("Malformed field(s):", bad), 
                collapse = " "), "\n")
        }
        if (!is_base_pkg && is.na(db["Packaged"])) {
            if (!any) 
                (noteLog(Log))
            any <- TRUE
            printLog(Log, "Checking should be performed on sources prepared by 'R CMD build'.", 
                "\n")
        }
            ...
            if (nzchar(BR)) {
                ...
                if (!isURL) {
                  ...
                  msg <- if (is.na(findEmail(BR))) {
                    ...
                    else "BugReports field should be the URL of a single webpage"
                  }
                  else "BugReports field is not a suitable URL but contains an email address:\n   use the Contact field instead"
                }
            }
            else {
        ...
        act <- if (Check_R_deps %in% c("note", "warn")) 
            TRUE
        ...
        if (act) {
            ...
            if (length(Rver) && Rver[[1L]]$op == ">=") {
                ...
                if (notOK && (((ver[1L] == 4L) && (ver[2L] >= 
                  max(0L, thisver[2L] - 2L))) || ((ver[1L] == 
                  3L) && (ver[2L] >= thisver[2L] + 5L)))) {
                  if (Check_R_deps == "warn") 
                    warningLog(Log)
                  else if (!any) 
                    noteLog(Log)
                  any <- TRUE
                  printLog0(Log, sprintf("Dependence on R version %s not with patchlevel 0\n", 
    ...
    check_build <- function() {
        ...
        if (any(!keep)) {
            ...
            msg <- c("Vignette(s) without any output listed in 'build/vignette.rds'", 
                strwrap(sQuote(db$file[!keep]), indent = 2L, 
                  exdent = 2L))
        ...
        elts <- db[keep, ]$File
        miss <- (nzchar(elts) & !file.exists(file.path("inst", 
            "doc", elts)))
        if (any(miss)) {
            if (!any) 
                warningLog(Log)
    ...
    check_top_level <- function() {
        ...
        if (file.exists("configure") && any(grepl("Generated by GNU Autoconf", 
            readLines("configure", warn = FALSE), ignore.case = TRUE))) {
            if (!any(file.exists(c("configure.ac", "configure.in")))) {
                desc <- .read_description("DESCRIPTION")
                is_FOSS <- parse_description_field(desc, "License_is_FOSS", 
                  NA)
                if (is.na(is_FOSS)) {
                  val <- desc["License"]
                  is_FOSS <- !is.na(val) && isTRUE(analyze_license(val)$is_FOSS)
                }
                if (is_FOSS) {
                  if (!any) 
                    warningLog(Log)
                  wrapLog("Found a", sQuote("configure"), "file without source file", 
                    sQuote("configure.ac"), "or", sQuote("configure.in"), 
                ...
                if (file.exists("configure.in")) {
                  ...
                  wrapLog("Found a", sQuote("configure.in"), 
                    "file:", sQuote("configure.ac"), "has long been preferred.\n")
                }
                check_autoconf <- check_incoming || config_val_to_logical(Sys.getenv("_R_CHECK_AUTOCONF_", 
                  "FALSE"))
                if (check_autoconf) {
                  ...
                  if (nzchar(Sys.which(autoreconf))) {
                    ver <- system2(autoreconf, "--version", stdout = TRUE, 
                      stderr = TRUE)[1]
                    if (grepl("2[.]6[89]", ver)) 
                      autoreconf_options <- unique(c(autoreconf_options, 
                        "--warnings=obsolete"))
                    td <- tempfile()
                    dir.create(td)
                    file.copy(".", td, recursive = TRUE)
                    od <- setwd(td)
                    autoreconf_options <- c(autoreconf_options, 
                      "-f", "-i")
                    out <- suppressWarnings(system2(autoreconf, 
                      autoreconf_options, stdout = TRUE, stderr = TRUE, 
                      timeout = 60))
                    setwd(od)
        ...
        if (config_val_to_logical(Sys.getenv("_R_CHECK_BASHISMS_", 
            "FALSE")) && any(file.exists("configure", "cleanup"))) {
            if (!nzchar(Sys.which("checkbashisms"))) {
                ...
                for (f in c("configure", "cleanup")) {
                  if (file.exists(f) && any(grepl("^#! */bin/bash", 
                  ...
                  if (file.exists(f) && any(grepl("^#!.*env bash", 
                    readLines(f, 1L, warn = FALSE)))) {
                    if (!any) {
                      any <- TRUE
                  ...
                  if (file.exists(f) && !any(grepl("Generated by GNU Autoconf", 
                    readLines(f, warn = FALSE)))) {
                    out <- suppressWarnings(system2("checkbashisms", 
                      c("-p -n", f), stdout = TRUE, stderr = TRUE, 
                      timeout = 60))
                    if (length(out) && !any(grepl("could not find any possible bashisms", 
                      out))) {
                      o <- grep("does not appear to have a #! interpreter line;", 
                        out)
        ...
        if (check_incoming) {
            md_files <- c(Filter(file.exists, c(file.path("inst", 
                "README.md"), "README.md"))[1L], Filter(file.exists, 
                c(file.path("inst", "NEWS.md"), "NEWS.md"))[1L])
            ...
            if (length(md_files)) {
                if (nzchar(Sys.which("pandoc"))) {
                  for (ifile in md_files) {
                    ...
                    if (out$status) {
                      if (!any) 
                        warningLog(Log)
                      any <- TRUE
                      printLog(Log, sprintf("Conversion of '%s' failed:\n", 
                  ...
                  if (!any) 
                    noteLog(Log)
                  any <- TRUE
                  printLog(Log, "Files 'README.md' or 'NEWS.md' cannot be checked without 'pandoc' being installed.\n")
                }
            }
            if (file.exists(nfile <- file.path("inst", "NEWS.Rd")) && 
                ...
                if (inherits(bad, "error")) {
                  if (!any) 
                    noteLog(Log)
                  any <- TRUE
                  printLog0(Log, c("Encountered the following errors for NEWS.Rd HTML conversion/validation:\n", 
                  ...
                  if (!any) 
                    noteLog(Log)
                  any <- TRUE
                  printLog0(Log, c("Found the following validation problem for the NEWS.Rd HTML conversion:\n", 
                    sprintf("NEWS.html:%s:%s (%s:%s): %s\n", 
                      bad[, "line"], bad[, "col"], bad[, "srcFile"], 
                      bad[, "srcLine"], bad[, "msg"])))
                }
            }
        }
        ...
        if (length(topfiles)) {
            lic <- desc["License"]
            if (!is.na(lic)) {
                found <- vapply(topfiles, function(x) grepl(x, 
                  lic, fixed = TRUE), NA)
        ...
        if (length(topfiles)) {
            lic <- desc["License"]
            if (!is.na(lic)) {
                found <- vapply(basename(topfiles), function(x) grepl(x, 
                  lic, fixed = TRUE), NA)
                topfiles <- topfiles[!found]
                if (length(topfiles)) {
                  if (!any) 
        ...
        if (!is_base_pkg && R_check_toplevel_files) {
            if (is.null(topfiles0)) {
                topfiles <- dir()
                topfiles <- topfiles[file.info(topfiles, extra_cols = FALSE)$ctime <= 
                  .unpack.time]
            }
            else topfiles <- topfiles0
            known <- c("DESCRIPTION", "INDEX", "LICENCE", "LICENSE", 
                "LICENCE.note", "LICENSE.note", "MD5", "NAMESPACE", 
                "NEWS", "PORTING", "COPYING", "COPYING.LIB", 
                "GPL-2", "GPL-3", "BUGS", "Bugs", "ChangeLog", 
                "Changelog", "CHANGELOG", "CHANGES", "Changes", 
                "INSTALL", "README", "THANKS", "TODO", "ToDo", 
                "INSTALL.windows", "README.md", "NEWS.md", "configure", 
                "configure.win", "cleanup", "cleanup.win", "configure.ucrt", 
                "cleanup.ucrt", "configure.ac", "configure.in", 
                "datafiles", "R", "data", "demo", "exec", "inst", 
                "man", "po", "src", "tests", "vignettes", "build", 
                ".aspell", "java", "tools", "noweb")
            ...
            if (lt <- length(topfiles)) {
                ...
                cp <- grep("^copyright", topfiles, ignore.case = TRUE, 
                  value = TRUE)
                if (length(cp)) 
    ...
    check_detritus <- function() {
        ...
        if (length(bad)) {
            ...
            noteLog(Log)
            printLog0(Log, "The following files look like leftovers:\n", 
        ...
        if (dir.exists("demo")) {
            ...
            if (!file.exists(index) || !length(readLines(index, 
                ...
                if (!any) 
                  warningLog(Log)
                any <- TRUE
                printLog0(Log, sprintf("Empty or missing file %s.\n", 
                  sQuote(index)))
            }
            else {
    ...
    check_subdirectories <- function(haveR, subdirs) {
        ...
        if (any(ind)) {
            if (!any) 
                warningLog(Log)
            any <- TRUE
            msg <- ngettext(sum(ind), "Found the following directory with the name of a check directory:\n", 
                "Found the following directories with names of check directories:\n", 
                domain = NA)
            printLog0(Log, msg, .format_lines_with_indent(all_dirs[ind]), 
        ...
        if (any(ind)) {
            if (!any) 
                warningLog(Log)
            any <- TRUE
            msg <- ngettext(sum(ind), "Found the following directory with the name of a Rd2pdf directory:\n", 
        ...
        if (!is_base_pkg && (is_tar || R_check_vc_dirs)) {
            ...
            if (any(ind)) {
                if (!any) 
                  warningLog(Log)
                any <- TRUE
                msg <- ngettext(sum(ind), "Found the following directory with the name of a version control directory:\n", 
        ...
        if (dir.exists("data") && !length(list_files_with_type("data", 
            "data"))) {
            if (!any) 
                warningLog(Log)
            any <- TRUE
        ...
        if (dir.exists("demo")) {
            ...
            if (!length(demos)) {
                ...
                bad <- character()
                for (d in demos) tryCatch(parse(file = d), error = function(e) bad <<- c(bad, 
                  basename(d)))
                if (length(bad)) {
                  if (!any) 
                    warningLog(Log)
        ...
        if (dir.exists("src")) {
            if (!any(file.exists(file.path("src", c("Makefile", 
                "Makefile.win", "Makefile.ucrt", "install.libs.R"))))) {
        ...
        if (dir.exists("inst")) {
            R_system_subdirs <- c("Meta", "R", "data", "demo", 
                "exec", "libs", "man", "help", "html", "latex", 
                "R-ex", "build")
            ...
            if (any(suspect)) {
                ...
                if (length(suspect)) {
                  if (!any) 
                    warningLog(Log)
                  any <- TRUE
                  wrapLog("Found the following non-empty", "subdirectories of 'inst' also", 
                    "used by R:\n")
        ...
        if (file.exists(file.path("inst", "CITATION"))) {
            Rcmd <- if (do_install) 
                sprintf("tools:::.check_citation(\"inst/CITATION\", \"%s\")\n", 
                  file.path(if (is_base_pkg) 
                    .Library
                  else libdir, pkgname))
            else "tools:::.check_citation(\"inst/CITATION\")\n"
            out <- R_runR0(Rcmd, R_opts2, "R_DEFAULT_PACKAGES=utils")
            if (length(out)) {
                if (!any) 
                  warningLog(Log)
                any <- TRUE
                printLog(Log, "Invalid citation information in 'inst/CITATION':\n")
                printLog0(Log, .format_lines_with_indent(out), 
                  "\n")
            }
        }
        files <- dir(".", pattern = "^CITATION.*", recursive = TRUE)
        files <- files[file_path_sans_ext(basename(files)) == 
            "CITATION" & files != file.path("inst", "CITATION")]
        if (length(files)) {
            if (!any) 
                noteLog(Log)
            any <- TRUE
            msg <- ngettext(length(files), "Found the following CITATION file in a non-standard place:\n", 
        ...
        else if (file.exists(nfile <- "NEWS.md") && requireNamespace("commonmark", 
            quietly = TRUE) && requireNamespace("xml2", quietly = TRUE)) 
            nread <- .build_news_db_from_package_NEWS_md
        else if (file.exists(nfile <- "NEWS") && config_val_to_logical(Sys.getenv("_R_CHECK_NEWS_IN_PLAIN_TEXT_", 
            "FALSE"))) 
        ...
        if (!is.null(nread)) {
            ...
            if (length(.messages)) {
                if (!any) 
                  noteLog(Log)
                any <- TRUE
                printLog(Log, sprintf("Problems with news in '%s':\n", 
                  nfile))
                bad <- TRUE
                printLog0(Log, paste0("  ", unlist(strsplit(.messages, 
                  "\n", fixed = TRUE)), collapse = "\n"), "\n")
            }
            else {
    ...
    check_non_ASCII <- function() {
        ...
        out <- R_runR0("tools:::.check_package_ASCII_code('.')", 
            R_opts2, c("R_DEFAULT_PACKAGES=NULL", "_R_NO_REPORT_MISSING_NAMESPACES_=true"))
        if (length(out)) {
            warningLog(Log)
            msg <- ngettext(length(out), "Found the following file with non-ASCII characters:\n", 
    ...
    check_R_code <- function() {
        ...
        if (do_install) {
            Rcmd <- paste(opW_shE_F_str, sprintf("tools:::.check_packages_used(package = \"%s\")\n", 
                pkgname))
            out <- R_runR2(Rcmd, "R_DEFAULT_PACKAGES=NULL")
            if (length(out)) {
                if (any(grepl("(not declared from|Missing or unexported object|Including base/recommended)", 
                  out))) 
                  warningLog(Log)
                else noteLog(Log)
                printLog0(Log, paste(c(out, ""), collapse = "\n"))
            ...
            Rcmd <- paste(opW_shE_F_str, sprintf("tools:::.check_packages_used(dir = \"%s\")\n", 
                pkgdir))
            out <- R_runR0(Rcmd, R_opts2, "R_DEFAULT_PACKAGES=NULL")
            if (length(out)) {
                if (any(grepl("(not declared from|Missing or unexported object)", 
                  out))) 
                  warningLog(Log)
                else noteLog(Log)
                printLog0(Log, paste(c(out, ""), collapse = "\n"))
        ...
        Rcmd <- paste(opWarn_string, "\n", "options(expressions=1000)\n", 
            if (do_install) 
                sprintf("tools::checkS3methods(package = \"%s\")\n", 
                  pkgname)
            else sprintf("tools::checkS3methods(dir = \"%s\")\n", 
                pkgdir))
        out <- R_runR2(Rcmd)
        if (length(out)) {
            ...
            if (length(out2)) {
                printLog0(Log, paste(c(if (length(out1)) "", 
                  out2, ""), collapse = "\n"))
        ...
        Rcmd <- paste(opWarn_string, "\n", if (do_install) 
            sprintf("tools::checkReplaceFuns(package = \"%s\")\n", 
                pkgname)
        else sprintf("tools::checkReplaceFuns(dir = \"%s\")\n", 
            pkgdir))
        out <- R_runR2(Rcmd)
        if (length(out)) {
            warningLog(Log)
            ...
            wrapLog("The argument of a replacement function", 
                "which corresponds to the right hand side", "must be named 'value'.\n")
        }
        else resultLog(Log, "OK")
        if (nzchar(R_check_FF)) {
            ...
            else sprintf("tools::checkFF(dir = \"%s\", registration = %s, check_DUP = %s)\n", 
                pkgdir, "FALSE", DUP))
            out <- R_runR2(Rcmd)
            Sys.unsetenv("_R_CHECK_FF_AS_CRAN_")
            if (length(out)) {
                if (any(grepl("^Foreign function calls? with(out| empty)", 
                  out)) || (!is_base_pkg && any(grepl("to a base package:", 
                  out))) || any(grepl("^Undeclared packages? in", 
                  out)) || any(grepl("parameter[s]*, expected ", 
                  out))) 
                  warningLog(Log)
                else noteLog(Log)
                printLog0(Log, paste(c(out, ""), collapse = "\n"))
                if (!is_base_pkg && any(grepl("to a base package:", 
                  out))) 
                  wrapLog("Packages should not make", ".C/.Call/.External/.Fortran", 
                    "calls to a base package.", "They are not part of the API,", 
                    "for use only by R itself", "and subject to change without notice.")
                else if (any(grepl("with DUP:", out))) 
                  wrapLog("DUP is no longer supported and will be ignored.")
                else wrapLog("See chapter 'System and foreign language interfaces' in the 'Writing R Extensions' manual.\n")
            }
            else resultLog(Log, "OK")
        }
    }
    check_R_files <- function(is_rec_pkg) {
        ...
        if (!is_base_pkg) {
            ...
            if (length(out)) {
                errorLog(Log)
                wrapLog("Incorrect (un)loading of package", "shared object.\n")
                printLog0(Log, paste(c(out, ""), collapse = "\n"))
        ...
        out1 <- if (length(out1) && length(out1a)) 
            c(out1, "", out1a)
        else c(out1, out1a)
        ...
        if (R_check_use_codetools && do_install) {
            ...
            if (config_val_to_logical(Sys.getenv("_R_CHECK_CODE_USAGE_WITH_ONLY_BASE_ATTACHED_", 
                ...
                if (length(pos <- which(startsWith(out3, "Undefined global functions or variables:")))) {
                  Rcmd <- sprintf("writeLines(strwrap(tools:::imports_for_undefined_globals(\"%s\"), exdent = 11))\n", 
                    paste(utils::tail(out3, -pos), collapse = " "))
                  miss <- R_runR2(Rcmd, "R_DEFAULT_PACKAGES=")
                  if (length(miss) && pkgname != "base") {
                    msg3 <- if (any(startsWith(miss, "importFrom(\"methods\""))) {
                    ...
                    out3 <- c(out3, c("Consider adding", paste0("  ", 
                      miss), msg3))
                  }
                }
            }
            else out3 <- R_runR2(Rcmd, "R_DEFAULT_PACKAGES=")
        }
        if (!is_base_pkg && R_check_use_codetools && R_check_dot_internal) {
            details <- pkgname != "relax"
            Rcmd <- paste(opWarn_string, "\n", if (do_install) 
                sprintf("tools:::.check_dotInternal(package = \"%s\",details=%s)\n", 
                  pkgname, details)
            else sprintf("tools:::.check_dotInternal(dir = \"%s\",details=%s)\n", 
                pkgdir, details))
            out4 <- R_runR2(Rcmd, "R_DEFAULT_PACKAGES=")
            if (!any(grepl("^Found.* .Internal call", out4))) 
                out4 <- NULL
        }
        ...
        if (!is_base_pkg && R_check_use_codetools && R_check_depr_def) {
            ...
            Rcmd <- paste(opWarn_string, "\n", if (do_install) 
                sprintf("tools:::.check_depdef(package = \"%s\", WINDOWS = %s)\n", 
                  pkgname, win)
            else sprintf("tools:::.check_depdef(dir = \"%s\", WINDOWS = %s)\n", 
                pkgdir, win))
        ...
        if (!is_base_pkg && R_check_use_codetools && R_check_bogus_return) {
            Rcmd <- paste(opWarn_string, "\n", if (do_install) 
                sprintf("tools:::.check_bogus_return(package = \"%s\")\n", 
                  pkgname)
            else sprintf("tools:::.check_bogus_return(dir = \"%s\")\n", 
                pkgdir))
        ...
        if (length(out1) || length(out2) || length(out3) || length(out4) || 
            length(out5) || length(out6) || length(out7) || length(out8) || 
            length(out9) || length(out10)) {
            ini <- character()
            if (length(out4) || (length(out8) && any(startsWith(out8, 
            ...
            if (length(ini)) 
                ini <- c("", "In addition to the above warning(s), found the following notes:", 
                  "")
            if (length(out1)) {
            ...
            if (length(out2)) {
                printLog0(Log, paste(c(ini, "Found the following possibly unsafe calls:", 
                  out2, ""), collapse = "\n"))
                ini <- ""
            }
            if (length(out3)) {
            ...
            if (length(out6)) {
                ...
                ini <- ""
                wrapLog(gettextf("See section %s in '%s'.", sQuote("Good practice"), 
                  "?attach"))
            }
            if (length(out7)) {
    ...
    check_Rd_files <- function(haveR, chkInternal = NA) {
        msg_writing_Rd <- c("See chapter 'Writing R documentation files' in the 'Writing R Extensions' manual.\n")
        if (dir.exists("man") && !extra_arch) {
            checkingLog(Log, "Rd files")
            t1 <- proc.time()
            minlevel <- if (is_base_pkg) 
                -Inf
            else as.numeric(Sys.getenv("_R_CHECK_RD_CHECKRD_MINLEVEL_", 
                "-1"))
            Rcmd <- paste(opWarn_string, "\n", sprintf("tools:::.check_package_parseRd('.', minlevel=%s)\n", 
                minlevel))
            out <- R_runR0(Rcmd, R_opts2, c(if (R_cdo) elibs_cdo else elibs, 
            ...
            Rcmd <- paste(opWarn_string, "\n", if (do_install) 
                sprintf("tools:::.check_Rd_metadata(package = \"%s\")\n", 
                  pkgname)
            else sprintf("tools:::.check_Rd_metadata(dir = \"%s\")\n", 
                pkgdir))
            out <- R_runR0(Rcmd, R_opts2, "R_DEFAULT_PACKAGES=NULL")
            if (length(out)) {
        ...
        if (dir.exists("man") && R_check_Rd_line_widths) {
            ...
            else sprintf("tools:::.check_Rd_line_widths(\"%s\")\n", 
                pkgdir))
            out <- R_runR0(Rcmd, R_opts2, "R_DEFAULT_PACKAGES=NULL")
            if (length(out)) {
                noteLog(Log)
        ...
        if (dir.exists("man") && R_check_Rd_xrefs) {
            ...
            Rcmd <- paste(opWarn_string, "\n", if (do_install) 
                sprintf("tools:::.check_Rd_xrefs(package = \"%s\")\n", 
                  pkgname)
            else sprintf("tools:::.check_Rd_xrefs(dir = \"%s\")\n", 
                pkgdir))
            any <- FALSE
            out <- R_runR0(Rcmd, R_opts2, "R_DEFAULT_PACKAGES=NULL")
            if (length(out)) {
                if (!all(grepl("(Package[s]? unavailable to check|Unknown package.*in Rd xrefs|Undeclared package.*in Rd xrefs)", 
                  out))) 
                  warningLog(Log)
                else noteLog(Log)
                any <- TRUE
            ...
            if (config_val_to_logical(Sys.getenv("_R_CHECK_XREFS_NOTE_MISSING_PACKAGE_ANCHORS_", 
                ...
                if (!inherits(bad, "error") && length(bad)) {
                  ...
                  msg <- c(if (any) "", strwrap("Found the following Rd file(s) with Rd \\link{} targets missing package anchors:"), 
                    strwrap(sprintf("  %s: %s", names(bad), vapply(bad, 
                      paste, "", collapse = ", ")), exdent = 4L, 
                      indent = 2L), strwrap("Please provide package anchors for all Rd \\link{} targets not in the package itself and the base packages."))
                  if (!any) {
                    noteLog(Log)
                    any <- TRUE
                  }
        ...
        if (!extra_arch && (haveR || dir.exists("data"))) {
            ...
            else sprintf("tools::undoc(dir = \"%s\")\n", pkgdir))
            use_methods <- if (pkgname == "methods") 
                TRUE
            else {
            ...
            out <- if (use_methods) {
                ...
                else "R_DEFAULT_PACKAGES='utils,grDevices,graphics,stats,methods'"
            ...
            if (length(out)) {
                warningLog(Log)
                printLog0(Log, paste(c(out, ""), collapse = "\n"))
                wrapLog("All user-level objects", "in a package", 
                  if (any(startsWith(out, "Undocumented S4"))) 
                    "(including S4 classes and methods)", "should have documentation entries.\n")
                ...
                if (haveR) {
                  Rcmd <- paste(opWarn_string, "\n", if (do_install) 
                    sprintf("tools::codoc(package = \"%s\")\n", 
                      pkgname)
                  else sprintf("tools::codoc(dir = \"%s\")\n", 
                    pkgdir))
                  out <- R_runR2(Rcmd)
                  if (length(out)) {
                    any <- TRUE
        ...
        if (dir.exists("man") && !extra_arch) {
            ...
            msg_doc_files <- c("Functions with \\usage entries", 
                "need to have the appropriate \\alias entries,", 
                "and all their arguments documented.\n", "The \\usage entries must correspond to syntactically", 
                "valid R code.\n")
            any <- FALSE
            Sys.setenv(`__R_CHECK_DOC_FILES_NOTE_IF_ALL_SPECIAL__` = "TRUE")
            ...
            if (length(out)) {
                any <- TRUE
                pos <- which(out == "All issues in internal Rd files checked specially.")
                if (length(pos)) {
                  noteLog(Log)
                  out <- out[-pos]
                }
            ...
            if (R_check_Rd_style && haveR) {
                msg_doc_style <- c("The \\usage entries for S3 methods should use", 
                  "the \\method markup and not their full name.\n")
                Rcmd <- paste(opWarn_string, "\n", if (do_install) 
                  sprintf("tools::checkDocStyle(package = \"%s\")\n", 
                    pkgname)
                else sprintf("tools::checkDocStyle(dir = \"%s\")\n", 
                  pkgdir))
                out <- R_runR2(Rcmd)
                if (length(out)) {
                  if (!any) 
        ...
        if (dir.exists("man") && R_check_Rd_contents && !extra_arch) {
            ...
            Rcmd <- paste(opWarn_string, "\n", sprintf("tools::checkRdContents(%s, chkInternal=%s)\n", 
                if (do_install) 
                  sprintf("package = \"%s\"", pkgname)
                else sprintf("dir = \"%s\"", pkgdir), R_check_Rd_internal_too))
            out <- R_runR0(Rcmd, R_opts2, "R_DEFAULT_PACKAGES=NULL")
            if (length(out)) {
                noteLog(Log)
                printLog0(Log, paste(c(out, ""), collapse = "\n"))
        ...
        if (dir.exists("man") && do_install && !extra_arch && 
            !is_base_pkg) {
            checkingLog(Log, "for unstated dependencies in examples")
            Rcmd <- paste(opW_shE_F_str, sprintf("suppressPackageStartupMessages(tools:::.check_packages_used_in_examples(package = \"%s\"))\n", 
                pkgname))
            out <- R_runR2(Rcmd, "R_DEFAULT_PACKAGES=NULL")
            exfile <- paste0(pkgname, "-Ex.R")
            if (length(out)) {
                failed <- any(grepl("parse error in file", out, 
                  fixed = TRUE))
                if (failed) 
                  errorLog(Log)
                else warningLog(Log)
    ...
    check_data <- function() {
        if (!is_base_pkg && dir.exists("data")) {
            ...
            dataFiles <- basename(list_files_with_type("data", 
                "data"))
            if (!any(grepl("\\.[Rr]$", fi))) {
                odd <- fi %w/o% c(dataFiles, "datalist")
                if (length(odd)) {
                  warn <- TRUE
                  msgs <- c(sprintf("Files not of a type allowed in a %s directory:\n", 
            ...
            if ("datalist" %in% fi) {
                if (file.info(sv <- file.path("data", "datalist"))$isdir) {
                  ...
                  if (any(bad <- !grepl("^[^ :]+($|: +[[:alpha:].])", 
                    dl))) {
                    warn <- TRUE
                    msgs <- c(msgs, sprintf("File %s contains malformed line(s):\n", 
            ...
            if (length(ans)) {
                bad <- names(ans)[vapply(ans, function(x) ".Random.seed" %in% 
                  x, NA)]
                if (length(bad)) {
                  warn <- TRUE
                  msg <- if (length(bad) > 1L) 
                    c(sprintf("Object named %s found in datasets:\n", 
                      ...
                      "\n"), "Please remove it.\n")
                  else c(sprintf("Object named %s found in dataset: ", 
                    sQuote(".Random.seed")), sQuote(bad), "\nPlease remove it.\n")
                  msgs <- c(msgs, msg)
                }
            }
            if (do_install) {
                ...
                if (!file.exists(file.path(instdir, "data", "Rdata.rdb"))) {
                  files <- basename(list_files_with_type("data", 
                    "data"))
                  ...
                  for (f in files) {
                    ...
                    if (length(out)) {
                      if (any(grepl("^(Warning|Error|No dataset created|Search path was changed)", 
                        out))) 
                        warn <- TRUE
                      msgs <- c(msgs, sprintf("Output for data(\"%s\", package = \"%s\"):\n", 
                        f, pkgname), paste(c(paste0("  ", out), 
                        ""), collapse = "\n"))
                    }
                  }
                  check_datalist <- Sys.getenv("_R_CHECK_DATALIST_", 
                    "FALSE")
                  check_datalist <- config_val_to_logical(check_datalist)
                  if (check_datalist && !warn) {
                    cmd <- sprintf("tools:::.check_package_datalist(\"%s\", \"%s\")", 
                      pkgname, libdir)
                    out <- R_runR(cmd, R_opts2)
                    if (length(out)) {
                      msgs <- c(msgs, c("File 'data/datalist' is out-of-date:\n", 
        ...
        if (!is_base_pkg && R_check_ascii_data && dir.exists("data")) {
            ...
            out <- R_runR0("tools:::.check_package_datasets('.')", 
                R_opts2, if (R_cdo_data) 
                  elibs_cdo
                else elibs)
            out <- filtergrep("Loading required package", out)
            out <- filtergrep("Warning: changing locked binding", 
                out, fixed = TRUE)
            out <- filtergrep("^OMP:", out)
            t2 <- proc.time()
            print_time(t1, t2, Log)
            if (length(out)) {
                ...
                bad2 <- any(grepl("(unable to find required package|there is no package called)", 
                  out))
                if (any(bad) || bad2) 
                  warningLog(Log)
                else noteLog(Log)
                ...
                if (bad2) 
                  if (R_cdo_data || R_check_suggests_only) 
                    printLog0(Log, "  The dataset(s) may use package(s) not declared in Depends/Imports.\n")
        ...
        if (!is_base_pkg) {
            ...
            if (thislazy || lazyz0) {
                ...
                if (thislazy && !dir.exists("data")) {
                  ...
                  if (lazyz0) 
                    printLog0(Log, "  'LazyDataCompression' is specified without a 'data' directory\n")
                }
                else if (!thislazy && lazyz0) {
                  noteLog(Log)
                  printLog0(Log, "  'LazyDataCompression' is specified without 'LazyData'\n")
                }
                else if (thislazy && lazyz0 && !(lazyz %in% c("gzip", 
                  "bzip2", "xz", "none"))) {
                  warningLog(Log)
                  printLog0(Log, sprintf("  undocumented value %s of field 'LazyDataCompression'\n", 
                    sQuote(lazyz)))
                }
                else if (thislazy && !lazyz0 && do_install) {
                  f <- file.path(libdir, pkgname, "data", "Rdata.rdb")
                  if (file.exists(f) && (fs <- file.size(f)) > 
                    5 * 1024^2) {
                    warningLog(Log)
                    printLog0(Log, sprintf("  LazyData DB of %.1f MB without LazyDataCompression set\n", 
                      fs/1024^2), "  See §1.1.6 of 'Writing R Extensions'\n")
                  }
                  else resultLog(Log, "OK")
                }
                else resultLog(Log, "OK")
        ...
        if (R_check_compact_data && file.exists(file.path("R", 
            "sysdata.rda"))) {
    ...
    check_doc_contents <- function() {
        ...
        if (length(bad)) {
            noteLog(Log)
            any <- TRUE
            printLog0(Log, "The following files are already in R: ", 
                paste(sQuote(bad), collapse = ", "), "\n", "Please remove them from your package.\n")
        }
        files2 <- dir(file.path(pkgdir, "inst", "doc"), recursive = TRUE, 
            pattern = "[.](cls|sty|drv)$", full.names = TRUE)
        files2 <- files2[basename(files2) %notin% c("jss.cls", 
            "jss.drv", "Rnews.sty", "RJournal.sty")]
        bad <- character()
        for (f in files2) {
            ...
            if (length(grep(pat, readLines(f, warn = FALSE), 
                useBytes = TRUE)) == 3L) 
        ...
        if (length(bad)) {
            if (!any) 
                noteLog(Log)
            any <- TRUE
            printLog0(Log, "The following files contain a license that requires\n", 
        ...
        if (any(bad)) {
            if (!any) 
                noteLog(Log)
            any <- TRUE
            printLog0(Log, "The following files look like leftovers/mistakes:\n", 
        ...
        vigns <- .package_vignettes_via_call_to_R(dir = pkgdir, 
        ...
        badd <- dirs[dirs %in% c("auto", "Bilder", "fig", "figs", 
            "figures", "Figures", "img", "images", "JSSstyle", 
        ...
        if (length(c(badf, badd))) {
            ...
            any <- TRUE
            if (length(badf)) 
                printLog0(Log, "The following files should probably not be installed:\n", 
                  paste(strwrap(paste(sQuote(badf), collapse = ", "), 
                    indent = 2, exdent = 2), collapse = "\n"), 
                  "\n")
            if (length(badd)) 
                printLog0(Log, "The following directories should probably not be installed:\n", 
    ...
    check_vign_contents <- function(ignore_vignettes = FALSE) {
        ...
        sources <- setdiff(list.files(file.path(pkgdir, "inst", 
        ...
        inst_doc_files <- list.files(file.path(pkgdir, "inst", 
            "doc"), recursive = TRUE)
        ...
        if (!is_base_pkg && length(vignette_files)) {
            if (!length(inst_doc_files)) {
                ...
                time_tol <- as.double(Sys.getenv("_R_CHECK_FILE_TIMES_TOL_", 
                  10))
                vignette_times <- file.mtime(file.path(vign_dir, 
                  vignette_files))
                inst_doc_times <- file.mtime(file.path(pkgdir, 
                  "inst", "doc", inst_doc_files))
                if (sum(!is.na(vignette_times)) && sum(!is.na(inst_doc_times)) && 
                  max(vignette_times, na.rm = TRUE) > max(inst_doc_times, 
                    na.rm = TRUE) + time_tol) {
                  if (!any) 
                    warningLog(Log)
                  any <- TRUE
                  msg <- c("Files in the 'vignettes' directory newer than all files in 'inst/doc':", 
                    .pretty_format(vignette_files[!is.na(vignette_times) & 
                      vignette_times > max(inst_doc_times, na.rm = TRUE)]))
                  keep <- is.na(vignette_times) | vignette_times <= 
                    max(inst_doc_times, na.rm = TRUE) + time_tol
        ...
        if (length(files) && !length(dir(file.path(pkgdir, "vignettes"), 
            pattern = pattern)) && is.na(desc["VignetteBuilder"])) {
            if (!any) 
                noteLog(Log)
            any <- TRUE
            printLog0(Log, "Package has no Sweave vignette sources and no VignetteBuilder field.\n")
        }
        vigns <- .package_vignettes_via_call_to_R(dir = pkgdir, 
            check = TRUE, libpaths = c(if (do_install) libdir, 
                .libPaths()))
        ...
        if (length(dirs)) {
            if (!any) 
                noteLog(Log)
            any <- TRUE
            printLog0(Log, if (length(dirs) > 1L) 
    ...
    check_doc_size <- function() {
        pdfs <- dir("inst/doc", pattern = "\\.pdf", recursive = TRUE, 
            full.names = TRUE)
        ...
        if (length(pdfs)) {
            ...
            dir.create(td)
            file.copy(pdfs, td)
            res <- compactPDF(td, gs_quality = "none")
            res <- format(res, diff = 1e+05)
            if (length(res)) {
                noteLog(Log)
                any <- TRUE
                printLog(Log, "  'qpdf' made some significant size reductions:\n", 
            ...
            if (R_check_doc_sizes2) {
                ...
                if (nzchar(gs_cmd)) {
                  ...
                  res <- format(res, diff = 256000)
                  if (length(res)) {
                    if (!any) 
                      warningLog(Log)
                    any <- TRUE
                    printLog(Log, "  'gs+qpdf' made some significant size reductions:\n", 
                      paste("  ", res, collapse = "\n"), "\n", 
    ...
    check_src_dir <- function(desc) {
        ...
        if (dir.exists("src") || dir.exists("inst/include")) {
            ...
            for (f in c(files, files2)) {
                ...
                if (grepl("\r", contents, fixed = TRUE, useBytes = TRUE)) 
                  bad_files <- c(bad_files, f)
                else if (nzchar(contents) && !grepl("\n$", contents, 
                  useBytes = TRUE)) 
                  no_eol <- c(no_eol, f)
            }
            if (length(bad_files) || length(no_eol)) 
                noteLog(Log, "")
            else resultLog(Log, "OK")
            if (length(bad_files)) {
        ...
        all_files <- dir("src", pattern = "^(Makevars|Makevars.in|Makefile|Makefile.in)$", 
            full.names = TRUE, recursive = TRUE)
        all_files <- c(all_files, dir(".", pattern = "^Makefile$", 
            full.names = TRUE, recursive = TRUE))
        ...
        if (length(makevars)) {
            ...
            out <- R_runR0(Rcmd, R_opts2, "R_DEFAULT_PACKAGES=NULL")
            if (length(out)) {
                if (any(grepl("^(Non-portable flags|Variables overriding)", 
                  out))) 
                  warningLog(Log)
                else noteLog(Log)
                printLog0(Log, paste(c(out, ""), collapse = "\n"))
        ...
        if (length(all_files)) {
            ...
            if (length(bad_files)) {
                if (!is.na(SysReq) && grepl("GNU [Mm]ake", gsub("[[:space:]]+", 
                  " ", SysReq))) {
                  ...
                  wrapLog("Portable Makefiles do not use GNU extensions", 
                    ...
                    "ifeq ... endif, .NOTPARALLEL", "See section 'Writing portable packages'", 
                    "in the 'Writing R Extensions' manual.\n")
                }
            }
            else resultLog(Log, "OK")
        }
        makefiles <- Sys.glob(file.path("src", c("Makevars", 
            "Makevars.in", "Makefile", "Makefile.win", "Makefile.ucrt")))
        if (length(makefiles)) {
            ...
            for (f in makefiles) {
                ...
                c2l <- grepl("\\$[{(]{0,1}LAPACK_LIBS", lines, 
                  useBytes = TRUE)
                c2b <- grepl("\\$[{(]{0,1}BLAS_LIBS", lines, 
                  useBytes = TRUE)
                ...
                if (any(c1 & c2l & !c2lb) && !have_F) {
                  if (!any) 
                    warningLog(Log)
                  any <- TRUE
                  printLog(Log, "  apparently using $(LAPACK_LIBS) without following $(BLAS_LIBS) in ", 
                    sQuote(f), "\n")
                }
                if (any(c1 & c2b & !c2bf) && !have_F) {
                  if (!any) 
                    warningLog(Log)
                  any <- TRUE
                  printLog(Log, "  apparently using $(BLAS_LIBS) without following $(FLIBS) in ", 
                    sQuote(f), "\n")
                }
            }
            if (!any) 
        ...
        makefiles <- Sys.glob(file.path("src", c("Makevars", 
            "Makevars.in", "Makevars.win", "Makevars.ucrt", "Makefile", 
            "Makefile.win", "Makefile.ucrt")))
        if (length(makefiles)) {
            ...
            for (m in makefiles) {
                ...
                have_c <- length(dir("src", pattern = "[.]c$", 
                  recursive = TRUE)) > 0L
                have_cxx <- length(dir("src", pattern = "[.](cc|cpp)$", 
                  recursive = TRUE)) > 0L
                ...
                for (f in c("C", "CXX", "F", "FC", "CPP")) {
                  ...
                  if (any(grepl(pat, lines, useBytes = TRUE))) {
                    if (!switch(f, C = have_c, CXX = have_cxx, 
                      F = have_f | have_f9x, FC = have_f9x, CPP = have_c | 
                        have_cxx)) {
                      msg <- c(msg, paste("  ", this2, "set in", 
                        sQuote(m), "without any corresponding files\n"))
                    }
                  }
                }
            }
        ...
        test_omp <- config_val_to_logical(Sys.getenv("_R_CHECK_SHLIB_OPENMP_FLAGS_", 
            "FALSE"))
        if (length(makefiles) && test_omp) {
            ...
            for (m in makefiles) {
                ...
                c1 <- grepl("^[[:space:]]*PKG_LIBS", lines, useBytes = TRUE)
                anyInLIBS <- any(grepl("SHLIB_OPENMP_", lines[c1], 
                  useBytes = TRUE))
                use_fc <- any(grepl("^USE_FC_TO_LINK", lines, 
                  useBytes = TRUE))
                have_c <- length(dir("src", pattern = "[.]c$", 
                  recursive = TRUE)) > 0L
                have_cxx <- length(dir("src", pattern = "[.](cc|cpp)$", 
                ...
                for (f in c("C", "CXX", "F", "FC")) {
                  ...
                  if (any(grepl(pat, lines, useBytes = TRUE))) {
                    ...
                    f_or_fc <- "F"
                    if (f == "FC") {
                      if (any(grepl("SHLIB_OPENMP_FCFLAGS", lines, 
                        useBytes = TRUE))) {
                        f_or_fc <- "FC"
                        if (!any) 
                          warningLog(Log)
                    ...
                    c_or_cxx <- if (have_cxx) 
                      "CXXFLAGS"
                    else "CFLAGS"
                    this2 <- if (f %in% c("F", "FC")) 
                      c_or_cxx
                    else this
                      ...
                      msg <- if (anyInLIBS) {
                        if (f == "F") 
                          ...
                          sprintf("SHLIB_OPENMP_%sFLAGS is included in PKG_FCFLAGS but not SHLIB_OPENMP_%s in PKG_LIBS\n", 
                            f_or_fc, c_or_cxx)
                        else sprintf("SHLIB_OPENMP_%s is included in PKG_%s but not in PKG_LIBS\n", 
                          this, this)
                      }
                      else {
                        msg3 <- TRUE
                    ...
                    if (any(c2 <- grepl(pat, lines, useBytes = TRUE))) {
                      ...
                      if (!anyInLIBS) 
                        msg3 <- TRUE
                      l <- lines[c2][1L]
                      found <- sub(".*SHLIB_OPENMP_([A-Z]*).*", 
                        "\\1", l, useBytes = TRUE)
                      printLog(Log, "  ", m, ": ", sprintf("incorrect macro SHLIB_OPENMP_%s included in PKG_%s\n", 
                        found, this))
                    }
                  }
                }
                ...
                for (f in c("C", "CXX", "F", "FC")) {
                  ...
                  if (res && f %in% c("F", "FC") && !use_fc) {
                    if (!any) 
                      noteLog(Log)
                    any <- TRUE
                    printLog(Log, "  ", m, ": ", sprintf("SHLIB_OPENMP_%s is included in PKG_LIBS but linking is by %s\n", 
                      this, if (have_cxx) 
                        "C++"
                      else "C"))
                    next
                  }
                  if (res && ((!have_cxx && f == "CXX") || (have_cxx && 
                    f == "C"))) {
                    if (!any) 
                      noteLog(Log)
                    any <- TRUE
                  ...
                  if (this %in% used) 
                    next
                  if (((!have_cxx && f == "C") || (have_cxx && 
                    f == "CXX")) && any(c("FFLAGS", "FCFLAGS") %in% 
                    used)) 
                    next
                  if (use_fc && f == "F" && used == "FCFLAGS") 
                    next
                  if (res) {
                    if (!any) 
                      noteLog(Log)
                    any <- TRUE
                    printLog(Log, "  ", m, ": ", sprintf("SHLIB_OPENMP_%s is included in PKG_LIBS but not in PKG_%s\n", 
                      this, this))
                  }
                }
                if (cnt > 1L) {
                  ...
                  if (!anyInLIBS) 
                    msg3 <- TRUE
                  printLog(Log, "  ", m, ": ", "it is not portable to include multiple", 
                ...
                if (any(grepl(pat, lines, useBytes = TRUE))) {
                  ...
                  any <- TRUE
                  msg2 <- TRUE
                  printLog(Log, "  ", m, ": ", "it is not portable to include", 
            ...
            if (!any) 
                ...
                wrapLog("Use of these macros is discussed in sect 1.2.1.1 of", 
                  ...
                  "so the matching macro must be used in", "PKG_CXXFLAGS (etc) and match that used in", 
                  "PKG_LIBS (except for Fortran: see the manual).\n")
                if (msg2) 
                  wrapLog("PKG_CPPFLAGS is used for both C and C++ code", 
                    "so it is not portable to use it", "for these macros.\n")
                if (msg3) 
                  wrapLog("Using a SHLIB_OPENMP_ macro for compilation", 
                    "but none in PKG_LIBS", "is not portable and may result in", 
                    "installation errors.\n")
            }
        }
        all_files <- dir(".", pattern = "^(Makefile|Makefile.in|Makefile.win|Makefile.ucrt|makefile|GNUmakefile)$", 
            recursive = TRUE)
        all_files <- unique(sort(all_files))
        if (length(all_files)) {
            checkingLog(Log, "include directives in Makefiles")
            bad_lines <- lapply(all_files, function(f) {
                s <- readLines(f, warn = FALSE)
                grep("^include .*R_HOME", s, value = TRUE)
            })
            bad_files <- all_files[lengths(bad_lines) > 0L]
            if (length(bad_files)) {
                noteLog(Log, "Found the following Makefile(s) with an include directive with a pathname using R_HOME:")
                printLog0(Log, .format_lines_with_indent(bad_files), 
                  "\n")
                msg <- c("Even though not recommended, variable R_HOME may contain spaces.", 
    ...
    check_src <- function() {
        ...
        if (config_val_to_logical(Check_pragmas) && any(dir.exists(c("src", 
            "inst/include")))) {
            ...
            if (length(ans)) {
                ...
                if (length(warn) || length(port)) {
                  ...
                  rest <- ans
                  if (length(warn)) {
                    msg <- c(msg, if (length(warn) == 1L) "File which contains pragma(s) suppressing important diagnostics" else "Files which contain pragma(s) suppressing important diagnostics", 
                      .pretty_format(warn))
                    rest <- setdiff(ans, warn)
                  }
                  if (length(port)) {
                    msg <- c(msg, if (length(port) == 1L) "File which contains non-portable pragma(s)" else "Files which contain non-portable pragma(s)", 
                      .pretty_format(port))
                  }
                  if (length(rest)) {
                    msg <- c(msg, if (length(rest) == 1L) "File which contains pragma(s) suppressing diagnostics:" else "Files which contain pragma(s) suppressing diagnostics:", 
                      .pretty_format(rest))
                  }
                }
                else {
                  ...
                  else "Files which contain pragma(s) suppressing diagnostics:"
        ...
        Check_flags <- Sys.getenv("_R_CHECK_COMPILATION_FLAGS_", 
            "FALSE")
        if (config_val_to_logical(Check_flags)) {
            instlog <- if (startsWith(install, "check")) 
                install_log_path
            else file.path(pkgoutdir, "00install.out")
            if (file.exists(instlog) && dir.exists("src")) {
                ...
                if (length(ll)) 
                  lines <- lines[-seq_len(ll[1L])]
                poss <- grep(" -[Wmf]", lines, useBytes = TRUE, 
                  value = TRUE)
                ...
                tokens <- gsub("[)]$", "", tokens, perl = TRUE, 
                  useBytes = TRUE)
                warns <- grep("^[-]W", tokens, value = TRUE, 
                  perl = TRUE, useBytes = TRUE)
                except <- Sys.getenv("_R_CHECK_COMPILATION_FLAGS_KNOWN_", 
                  "")
                except <- unlist(strsplit(except, "\\s", perl = TRUE))
                warns <- setdiff(warns, c(except, "-Wall", "-Wextra", 
                  "-Weverything", "-Wno-dev", "-Wstrict-prototypes", 
                  "-Wno-strict-prototypes"))
                warns <- warns[!startsWith(warns, "-Wl,")]
                diags <- grep(" -fno-diagnostics-show-option", 
                  tokens, useBytes = TRUE, value = TRUE)
                opts <- grep("-f(fast-math|unsafe-math-optimizations|associative-math|reciprocal-math)", 
                  tokens, useBytes = TRUE, value = TRUE)
                machs <- grep("^[-]m", tokens, value = TRUE, 
                  perl = TRUE, useBytes = TRUE)
                machs <- setdiff(machs, c(except, c("-m", "-msse2", 
                  "-mfpmath=sse", "-mstackrealign", "-m32", "-m64", 
                  "-multiply_defined")))
                machs <- machs[!startsWith(machs, "-mtune=")]
                if (grepl("darwin", R.version$platform)) 
                  machs <- machs[!startsWith(machs, "-mmacos")]
                ...
                if (any(startsWith(warns, "-Wno-")) || length(diags)) {
                  ...
                  msg <- c("Compilation used the following non-portable flag(s):", 
                    .pretty_format(sort(warns)), "including flag(s) suppressing warnings")
                  printLog0(Log, paste(c(msg, ""), collapse = "\n"))
                }
    ...
    check_sos <- function() {
        ...
        if (length(out) == 1L && startsWith(out, "Note:")) {
            ...
            if (haveObjs && any(grepl(pat, out)) && pkgname %notin% 
                "parallel") 
                warningLog(Log)
            else if (length(grep("Found non-API", out)) && any(grepl(rempat, 
                out))) {
                warningLog(Log)
                if (any(grepl("calls", out))) {
                  ...
                  out <- paste(c(out, "These entry points may be removed soon:", 
                    epq), collapse = "\n")
                }
                else out <- paste(c(out, "This entry point may be removed soon."), 
                  collapse = "\n")
            }
            else {
                ...
                if (haveObjs && (any(grepl(pat1, out)) && !any(grepl(pat2, 
                  out))) || (any(grepl(pat3, out)) && !any(grepl(pat4, 
                  out))) || (any(grepl(pat4, out)) && !any(grepl(pat3, 
                  out))) || any(grepl(pat5, out)) || any(grepl(pat6, 
                  out))) 
                  warningLog(Log)
                else noteLog(Log)
            }
            ...
            msg <- if (nBad) {
                ...
                else c("Compiled code should not call entry points which", 
                  "might terminate R nor write to stdout/stderr instead of", 
                  "to the console, nor use Fortran I/O nor system RNGs nor [v]sprintf.", 
                  "The detected symbols are linked", "into the code but might come from libraries", 
                  "and not actually be called.\n")
            }
            else character()
            if (nAPIs) 
                msg <- c(msg, "Compiled code should not call non-API entry points in R.\n")
            if (nRS) 
                msg <- c(msg, "It is good practice to register native routines and to disable symbol search.\n")
    ...
    check_loading <- function(arch = "") {
        ...
        if (length(st <- attr(out, "status"))) {
            ...
            wrapLog("Loading this package had a fatal error", 
                "status code ", st, "\n")
            if (length(out)) 
                printLog0(Log, paste(c("Loading log:", out, ""), 
                  collapse = "\n"))
            summaryLog(Log)
            do_exit(1L)
        }
        if (any(startsWith(out, "Error"))) {
            errorLog(Log)
            printLog0(Log, paste(c(out, ""), collapse = "\n"))
            wrapLog("\nIt looks like this package", "has a loading problem: see the messages", 
                "for details.\n")
            maybe_exit(1L)
        }
        else resultLog(Log, "OK")
        ...
        t2 <- proc.time()
        print_time(t1, t2, Log)
        if (any(startsWith(out, "Error")) || length(attr(out, 
            "status"))) {
            warningLog(Log)
            printLog0(Log, paste(c(out, ""), collapse = "\n"))
            wrapLog("\nIt looks like this package", "(or one of its dependent packages)", 
                "has an unstated dependence on a standard", "package.  All dependencies must be", 
        ...
        if (file.exists(file.path(pkgdir, "NAMESPACE"))) {
            ...
            env2 <- paste0("_R_LOAD_CHECK_S4_EXPORTS_=", if (env2 == 
                "all") 
                env2
            else pkgname)
            t1 <- proc.time()
            ...
            if (any(startsWith(out, "Error")) || length(attr(out, 
                ...
                if (pkgname == "tcltk") 
                  out <- filtergrep("Warning: no DISPLAY variable so Tk is not available", 
                    out, fixed = TRUE)
                check_imports_flag <- Sys.getenv("_R_CHECK_REPLACING_IMPORTS_", 
                  "TRUE")
                if (config_val_to_logical(check_imports_flag)) 
                  out <- filtergrep("Warning: replacing previous import", 
                    out, fixed = TRUE)
                if (any(startsWith(out, "Warning: S4 exports"))) {
                  warningLog(Log)
                  any <- if (length(out) == 1L) 
                    NA
                  else TRUE
                }
            ...
            if (is.na(any)) {
                ...
                wrapLog("\nA namespace must be able to be loaded", 
                  "with just the base namespace loaded:", "otherwise if the namespace gets loaded by a", 
                  "saved object, the session will be unable", 
                  "to start.\n\n", "Probably some imports need to be declared", 
                  "in the NAMESPACE file.\n")
            }
            else resultLog(Log, "OK")
            checkingLog(Log, "whether the namespace can be unloaded cleanly")
            Rcmd <- sprintf("invisible(suppressMessages(loadNamespace(\"%s\"))); cat('\n---- unloading\n'); unloadNamespace(\"%s\")", 
                pkgname, pkgname)
            t1 <- proc.time()
            out <- if (is_base_pkg && pkgname != "stats4") 
                R_runR0(Rcmd, opts, "R_DEFAULT_PACKAGES=NULL", 
                  arch = arch)
        ...
        if (pkgname %notin% dir(.Library)) {
            ...
            Rcmd <- sprintf("library(%s, lib.loc = '%s')", pkgname, 
                libdir)
            opts <- if (nzchar(arch)) 
                R_opts4
            else R_opts2
        ...
        if (!extra_arch && !is_base_pkg) {
            Rcmd <- sprintf("ls(asNamespace('%s'), all = TRUE)", 
                pkgname)
            out <- R_runR0(Rcmd, opts, arch = arch)
            if (any(grepl("[.]on(Load|Attach)", out))) {
                checkingLog(Log, "whether startup messages can be suppressed")
                Rcmd <- sprintf("suppressWarnings(suppressPackageStartupMessages(library(%s, lib.loc = '%s',  warn.conflicts=FALSE)))", 
                  pkgname, libdir)
                opts <- if (nzchar(arch)) 
                  R_opts4
                else R_opts2
                ...
                if (length(out)) {
                  noteLog(Log)
                  printLog0(Log, paste(c(out, ""), collapse = "\n"))
                  wrapLog("\nIt looks like this package", "(or a package it requires)", 
        ...
        if (!extra_arch && !is_base_pkg) {
            check_S3reg <- Sys.getenv("_R_CHECK_OVERWRITE_REGISTERED_S3_METHODS_", 
                "NA")
            check_S3reg <- if (check_S3reg == "NA") 
                check_incoming
            else {
            ...
            if (check_S3reg) {
                ...
                preloads <- c("MASS", "Matrix", "boot", "class", 
                  "cluster", "grDevices", "graphics", "grid", 
                  "lattice", "mgcv", "nlme", "nnet", "parallel", 
                  "rpart", "spatial", "splines", "stats", "survival", 
                  "tcltk", "tools", "utils")
                if (!is.na(match(pkgname, preloads))) {
                  rdepends <- .get_standard_package_dependencies(reverse = TRUE, 
                    recursive = TRUE)
                  preloads <- setdiff(preloads, c(pkgname, rdepends[[pkgname]]))
                }
                Rcmd <- c(sprintf("suppressPackageStartupMessages(loadNamespace('%s', lib.loc = '%s'))", 
                  preloads, .Library), Rcmd)
                env <- c(env, "R_DEFAULT_PACKAGES=NULL")
                out <- R_runR0(Rcmd, opts, env, arch = arch)
                if (any(grepl("^Registered S3 method.*standard package.*overwritten", 
                  out, useBytes = TRUE))) {
    ...
    run_examples <- function() {
        run_one_arch <- function(exfile, exout, arch = "", do_diff = TRUE) {
            ...
            cprof <- Sys.getenv("_R_CHECK_EXAMPLES_PROFILE_", 
                "")
            cprof <- if (!file.exists(cprof)) 
                ""
            else normalizePath(cprof)
            Ropts <- if (nzchar(cprof)) {
                if (nzchar(arch)) {
                  "--no-site-file --no-save --no-restore"
                }
                else "--no-site-file --no-save --no-restore --no-environ"
            }
            else if (nzchar(arch)) 
                R_opts3
            else R_opts
            if (use_valgrind) 
                Ropts <- paste(Ropts, "-d valgrind")
            t1 <- proc.time()
            tlim <- get_timeout(Sys.getenv("_R_CHECK_EXAMPLES_ELAPSED_TIMEOUT_", 
                Sys.getenv("_R_CHECK_ELAPSED_TIMEOUT_")))
            status <- R_runR0(NULL, c(Ropts, enc), c("LANGUAGE=en", 
                "_R_CHECK_INTERNALS2_=1", if (nzchar(arch)) env0, 
                jitstr, if (R_cdo_examples) elibs_cdo else elibs, 
                if (nzchar(cprof)) paste0("R_PROFILE_USER=", 
                  cprof)), stdout = exout, stderr = exout, stdin = exfile, 
                arch = arch, timeout = tlim)
            t2 <- proc.time()
            print_time(t1, t2, Log)
            if (status) {
                errorLog(Log, "Running examples in ", sQuote(basename(exfile)), 
                  " failed")
                txt <- tryCatch(suppressWarnings(readLines(exout, 
                  warn = FALSE)), error = function(e) {
                  Sys.sleep(2)
                  readLines(exout, warn = FALSE)
                })
                ...
                if (do_timings) {
                  theta <- as.numeric(Sys.getenv("_R_CHECK_EXAMPLE_TIMING_THRESHOLD_", 
                    "5"))
                  tfile <- paste0(pkgname, "-Ex.timings")
                  times <- utils::read.table(tfile, header = TRUE, 
                    row.names = 1L, colClasses = c("character", 
                      rep.int("numeric", 3)))
                  ...
                  keep <- ((times[[1L]] + times[[2L]] > theta) | 
                    (times[[3L]] > theta))
                  if (any(keep)) {
                    printLog(Log, sprintf("Examples with CPU (user + system) or elapsed time > %gs\n", 
                      theta))
            ...
            bad <- FALSE
            lines <- tryCatch(suppressWarnings(readLines(exout, 
                warn = FALSE)), error = function(e) {
                Sys.sleep(2)
                readLines(exout, warn = FALSE)
            })
            ...
            if (do_timings && do_diff) {
                ...
                theta <- as.numeric(Sys.getenv("_R_CHECK_EXAMPLE_TIMING_CPU_TO_ELAPSED_THRESHOLD_", 
                  NA_character_))
                if (!is.na(theta)) {
                  keep <- ((times[[1L]] + times[[2L]]) >= pmax(theta * 
                    times[[3L]], 1))
                  if (any(keep)) {
                    if (!any && check_incoming) {
                      noteLog(Log)
                    ...
                    printLog(Log, sprintf("Examples with CPU time > %g times elapsed time\n", 
                      theta))
                    bad <- times[keep, ]
            ...
            if (do_diff && file.exists(exsave)) {
                checkingLog(Log, "differences from ", sQuote(basename(exout)), 
                  " to ", sQuote(basename(exsave)))
                cmd <- paste0("invisible(tools::Rdiff('", exout, 
                  "', '", exsave, "',TRUE,TRUE))")
        ...
        if (!do_examples) 
            ...
            cmd <- sprintf("tools:::.createExdotR(\"%s\", \"%s\", silent = TRUE, use_gct = %s, addTiming = %s, commentDontrun = %s, commentDonttest = %s)", 
                pkgname, pkgtopdir, use_gct, do_timings, !run_dontrun, 
                !run_donttest)
            Rout <- tempfile("Rout")
            status <- R_runR0(cmd, R_opts2, stdout = Rout, stderr = Rout)
            exfile <- paste0(pkgname, "-Ex.R")
            if (status) {
                errorLog(Log, paste("Running massageExamples to create", 
                  sQuote(exfile), "failed"))
                printLog0(Log, paste(readLines(Rout, warn = FALSE), 
                  collapse = "\n"), "\n")
                maybe_exit(1L)
            }
            if (file.exists(exfile)) {
                enc <- if (length(suppressMessages(showNonASCIIfile(exfile)))) {
                  "--encoding=UTF-8"
                }
                else ""
                if (!this_multiarch) {
                  exout <- paste0(pkgname, "-Ex.Rout")
                  if (!run_one_arch(exfile, exout)) 
                    maybe_exit(1L)
                }
                else {
                  printLog(Log, "\n")
                  ...
                  for (arch in inst_archs) {
                    ...
                    if (arch %in% R_check_skip_examples_arch) {
                      ...
                      dir.create(tdir)
                      if (!dir.exists(tdir)) {
                        errorLog(Log, "unable to create examples directory")
                        summaryLog(Log)
                        do_exit(1L)
                      }
                      od <- setwd(tdir)
                      exout <- paste0(pkgname, "-Ex_", arch, 
                        ".Rout")
                      res <- res & run_one_arch(file.path("..", 
                        exfile), file.path("..", exout), arch)
                      setwd(od)
                    }
                  }
                  Log$stars <<- "*"
                ...
                if (file.exists(cntFile)) {
                  unlink(cntFile)
                  x <- Sys.getenv("_R_CHECK_DONTTEST_EXAMPLES_", 
                    "NA")
                  test_donttest <- !run_donttest && (if (x == 
                    "NA") 
                    as_cran
                  else config_val_to_logical(x))
                  if (test_donttest) {
                    ...
                    cmd <- sprintf("tools:::.createExdotR(\"%s\", \"%s\", silent = TRUE, use_gct = %s, addTiming = %s, commentDontrun = %s, commentDonttest = %s)", 
                      ...
                      !run_dontrun, FALSE)
                    Rout <- tempfile("Rout")
                    status <- R_runR0(cmd, R_opts2, stdout = Rout, 
                      stderr = Rout)
                    exfile <- paste0(pkgname, "-Ex.R")
                    if (status) {
                      errorLog(Log, paste("Running massageExamples to create", 
    ...
    run_tests <- function() {
        if (!extra_arch && !is_base_pkg) {
            checkingLog(Log, "for unstated dependencies in ", 
                sQuote(test_dir))
            Rcmd <- paste(opW_shE_F_str, sprintf("tools:::.check_packages_used_in_tests(\"%s\", \"%s\")\n", 
                pkgdir, test_dir))
            out <- R_runR2(Rcmd, "R_DEFAULT_PACKAGES=NULL")
            if (length(out)) {
                warningLog(Log)
        ...
        else checkingLog(Log, "tests in ", sQuote(test_dir))
        run_one_arch <- function(arch = "") {
            testsrcdir <- file.path(pkgdir, test_dir)
            testdir <- file.path(pkgoutdir, "tests")
            if (nzchar(arch)) 
                testdir <- paste(testdir, arch, sep = "_")
            if (!dir.exists(testdir)) 
                dir.create(testdir, mode = "0755")
            if (!dir.exists(testdir)) {
                errorLog(Log, sprintf("unable to create %s", 
                  sQuote(testdir)))
                summaryLog(Log)
                do_exit(1L)
            }
            file.copy(Sys.glob(paste0(testsrcdir, "/*")), testdir, 
                recursive = TRUE)
            setwd(testdir)
            logf <- gsub("\\", "/", tempfile(), fixed = TRUE)
            extra <- c(if (use_gct) "use_gct = TRUE", if (use_valgrind) "use_valgrind = TRUE", 
                if (!stop_on_test_error) "stop_on_error = FALSE", 
                paste0("Log=\"", logf, "\""))
            cmd <- paste0("tools:::.runPackageTestsR(", paste(extra, 
                collapse = ", "), ")")
            t1 <- proc.time()
            tlim <- get_timeout(Sys.getenv("_R_CHECK_TESTS_ELAPSED_TIMEOUT_", 
                Sys.getenv("_R_CHECK_ELAPSED_TIMEOUT_")))
            status <- R_runR0(cmd, if (nzchar(arch)) 
                R_opts4
            else R_opts2, env = c("LANGUAGE=en", "_R_CHECK_INTERNALS2_=1", 
                if (nzchar(arch)) env0, jitstr, elibs_tests), 
                stdout = "", stderr = "", arch = arch, timeout = tlim)
            t2 <- proc.time()
            print_time(t1, t2, Log)
            if (status) {
                errorLog(Log)
                if (Log$con > 0L && file.exists(logf)) {
                  cat(readLines(logf, warn = FALSE), sep = "\n", 
                    file = Log$con)
                }
                ...
                if (length(bad_files)) {
                  for (f in bad_files) {
                    lines <- readLines(f, warn = FALSE)
                    f <- file.path(test_dir, sub("out\\.fail$", 
                      "", f))
                    ...
                    if (R_check_suppress_RandR_message) 
                      lines <- filtergrep("^Xlib: *extension \"RANDR\" missing on display", 
                        lines, useBytes = TRUE)
                    printLog(Log, sprintf(paste(if (endsWith(f, 
                      ".Rin")) 
                      "Processing"
                    else "Running the tests in", "%s failed.\n"), 
                      sQuote(f)))
        ...
        if (do_install && do_tests) {
            if (!this_multiarch) {
                ...
                printLog(Log, "\n")
                res <- TRUE
                for (arch in inst_archs) if (arch %notin% R_check_skip_tests_arch) {
    ...
    run_vignettes <- function(desc) {
        theta <- as.numeric(Sys.getenv("_R_CHECK_VIGNETTE_TIMING_CPU_TO_ELAPSED_THRESHOLD_", 
            NA_character_))
        libpaths <- .libPaths()
        .libPaths(c(libdir, libpaths))
        ...
        if (do_install && !spec_install && !is_base_pkg && !extra_arch) {
            ...
            Rcmd <- paste(opW_shE_F_str, sprintf("tools:::.check_packages_used_in_vignettes(package = \"%s\")\n", 
                pkgname))
            out <- R_runR2(Rcmd, "R_DEFAULT_PACKAGES=NULL")
            if (length(out)) {
                noteLog(Log)
        ...
        if (!is_base_pkg) {
            ...
            if (dir.exists(dir)) {
                ...
                for (i in seq_along(vigns$docs)) {
                  ...
                  outputs[i] <- tryCatch({
                    find_vignette_product(name, what = "weave", 
                      final = TRUE, dir = dir, engine = engine)
                  }, error = function(e) {
            ...
            if (nb <- length(bad_vignettes)) {
                if (!any) 
                  warningLog(Log)
                any <- TRUE
                msg <- ngettext(nb, "Non-ASCII package vignette without specified encoding:\n", 
        ...
        if ("makefile" %in% dir(vigns$dir)) {
            if (!any) 
                warningLog(Log)
            any <- TRUE
            printLog(Log, "Found 'inst/doc/makefile': should be 'Makefile' and will be ignored\n")
        }
        if ("Makefile" %in% dir(vigns$dir)) {
            ...
            if (any(grepl("[^/]R +CMD", lines))) {
                if (!any) 
                  warningLog(Log)
                any <- TRUE
                printLog(Log, "Found 'R CMD' in Makefile: should be '\"$(R_HOME)/bin/R\" CMD'\n")
            }
            contents <- readChar(f, file.size(f), useBytes = TRUE)
            ...
            if (any(grepl("[^/]Rscript", lines))) {
                if (!any) 
                  warningLog(Log)
                any <- TRUE
                printLog(Log, "Found 'Rscript' in Makefile: should be '\"$(R_HOME)/bin/Rscript\"'\n")
            }
        }
        ...
        for (i in seq_along(vigns$docs)) {
            ...
            if (nb <- length(bad_vignettes)) {
                if (!any) 
                  warningLog(Log)
                any <- TRUE
                msg <- ngettext(nb, "Package vignette which is not in its specified encoding:\n", 
        ...
        bad_vignettes <- vigns$docs[!grepl("::", vapply(vigns$docs, 
            getVignetteEngine, ""), fixed = TRUE)]
        if (nb <- length(bad_vignettes)) {
            if (!any) 
                noteLog(Log)
            any <- TRUE
            msg <- ngettext(nb, "Package vignette with \\VignetteEngine{} not specifying an engine package:", 
        ...
        if (R_check_vignette_titles) {
            ...
            if (nb <- length(bad_vignettes)) {
                if (!any) 
                  noteLog(Log)
                any <- TRUE
                msg <- ngettext(nb, "Package vignette with placeholder title 'Vignette Title':\n", 
            ...
            if (any(dup <- duplicated(titles) & nzchar(titles))) {
                ...
                msg <- ngettext(length(dups), "Duplicated vignette title:", 
                  "Duplicated vignette titles:", domain = NA)
        ...
        if (!is_base_pkg && dir.exists(dir <- file.path(pkgdir, 
            ...
            for (i in seq_along(vigns$docs)) {
                ...
                dir.create(tdir)
                .eval_with_capture({
                  products <- tryCatch(buildVignette(file, dir = tdir, 
                    weave = FALSE, quiet = TRUE, engine = engine, 
                ...
                if (!inherits(products, "error") && length(products)) {
                  ...
                  if (!all(grepl("(^###|^[[:space:]]*$)", lines, 
                    useBytes = TRUE)) && !file.exists(file.path(dir, 
                    basename(products[1L])))) 
            ...
            if (nb <- length(bad_vignettes)) {
                if (!any) 
                  noteLog(Log)
                any <- TRUE
                msg <- ngettext(nb, "Package vignette without corresponding tangle output:", 
        ...
        if (do_install && do_vignettes) {
            build_vignettes <- parse_description_field(desc, 
                "BuildVignettes", TRUE)
            ...
            skip_run_maybe <- R_check_vignettes_skip_run_maybe && 
                do_build_vignettes
            savefiles <- file.path(dirname(vigns$docs), paste0(vigns$names, 
                ".Rout.save"))
            ran <- FALSE
            if (!skip_run_maybe || any(file.exists(savefiles))) {
                ...
                for (i in iseq) {
                  ...
                  enc <- vigns$encodings[i]
                  out1 <- c("  ", sQuote(file), if (nzchar(enc)) paste(" using", 
                    sQuote(enc)), "...")
                  Rcmd <- paste0(opWarn_string, "\ntools:::.run_one_vignette('", 
                    file, "', '", vigns$dir, "'", if (nzchar(enc)) 
                      paste0(", encoding = '", enc, "'"), ", pkgdir='", 
                    vigns$pkgdir, "')")
                  outfile <- paste0(file, ".log")
                  tlim <- get_timeout(Sys.getenv("_R_CHECK_ONE_VIGNETTE_ELAPSED_TIMEOUT_", 
                    Sys.getenv("_R_CHECK_ELAPSED_TIMEOUT_")))
                  t1b <- proc.time()
                  status <- R_runR0(Rcmd, if (use_valgrind) 
                    paste(R_opts2, "-d valgrind")
                  else R_opts2, env = c(jitstr, "R_BATCH=1234", 
                    if (R_cdo_vignettes) elibs_cdo else elibs, 
                    "_R_CHECK_INTERNALS2_=1"), stdout = outfile, 
                    stderr = outfile, timeout = tlim)
                  t2b <- proc.time()
                  out <- readLines(outfile, warn = FALSE)
                  pos <- which(out == " *** Run successfully completed ***")
                  if (!length(pos) || any(nzchar(out[seq_len(pos[1L] - 
                    1L)]))) 
                    ran <- TRUE
                  savefile <- savefiles[i]
                  if (length(grep("^  When (running|tangling|sourcing)", 
                    out, useBytes = TRUE))) {
                    out1 <- c(out1, " failed\n")
                    keep <- as.numeric(Sys.getenv("_R_CHECK_VIGNETTES_NLINES_", 
                      "10"))
                    res <- if (keep > 0) 
                      c(res, paste("when running code in", sQuote(file)), 
                        "  ...", utils::tail(out, keep))
                    else c(res, paste("when running code in", 
                      sQuote(file)), out)
                  }
                  else if (status || " *** Run successfully completed ***" %notin% 
                    out) {
                    keep <- as.numeric(Sys.getenv("_R_CHECK_VIGNETTES_NLINES_", 
                      "10"))
                    ...
                    if (length(out2)) {
                      ...
                      out1 <- c(out1, paste(c("", out2, ""), 
                        collapse = "\n"))
                    }
                    else {
                      out1 <- c(out1, print_time0(t1b, t2b))
                      out1 <- c(out1, " OK\n")
                      if (!config_val_to_logical(Sys.getenv("_R_CHECK_ALWAYS_LOG_VIGNETTE_OUTPUT_", 
                        use_valgrind))) 
                        unlink(outfile)
                    }
                  }
                ...
                if (!ran) {
                  ...
                  if (R_check_suppress_RandR_message) 
                    res <- filtergrep("^Xlib: *extension \"RANDR\" missing on display", 
                      res, useBytes = TRUE)
                  if (length(res)) {
                    if (length(grep("there is no package called", 
                      res, useBytes = TRUE))) {
                  ...
                  if (!WINDOWS && !is.na(theta)) {
                    ...
                    if (cpu >= pmax(theta * td[3L], 1)) {
                      ratio <- round(cpu/td[3L], 1L)
                      printLog(Log, sprintf("Running R code from vignettes had CPU time %g times elapsed time\n", 
                        ratio))
                    }
                  }
                }
            ...
            if (do_build_vignettes) {
                ...
                Rcmd <- if (!config_val_to_logical(Sys.getenv("_R_CHECK_BUILD_VIGNETTES_SEPARATELY_", 
                  "TRUE"))) 
                  sprintf("%s\ntools::buildVignettes(dir = '%s', skip = TRUE)", 
                    opWarn_string, file.path(pkgoutdir, "vign_test", 
                      pkgname0))
                else {
                  tf <- gsub("\\", "/", tempfile(fileext = ".rds"), 
                    fixed = TRUE)
                  saveRDS(c(jitstr, if (R_cdo_vignettes) elibs_cdo else elibs), 
                    tf)
                ...
                status <- R_runR0(Rcmd, if (use_valgrind) 
                  paste(R_opts2, "-d valgrind")
                else R_opts2, c(jitstr, if (R_cdo_vignettes) elibs_cdo else elibs), 
                  stdout = outfile, stderr = outfile, timeout = tlim)
                t2 <- proc.time()
                print_time(t1, t2, Log)
                out <- readLines(outfile, warn = FALSE)
                if (R_check_suppress_RandR_message) 
                ...
                ltx_err <- any(grepl("LaTeX error", out, ignore.case = TRUE, 
                  useBytes = TRUE))
                iskip <- grep("^Note: skipping .* dependencies:", 
                  out, useBytes = TRUE)
                any <- FALSE
                if (status) {
                  any <- TRUE
                  ...
                  if (keep > 0 && length(out) < keep) {
                    ...
                    printLog0(Log, paste(c("Error(s) in re-building vignettes:", 
                      "  ...", out, "", ""), collapse = "\n"))
                  }
                  else printLog0(Log, paste(c("Error(s) in re-building vignettes:", 
                    out, "", ""), collapse = "\n"))
                }
                  ...
                  msg <- ngettext(nw, "Warning in re-building vignettes:\n", 
                  ...
                  if (config_val_to_logical(Sys.getenv("_R_CHECK_CLEAN_VIGN_TEST_", 
                    "true"))) 
                  ...
                  if (length(iskip)) {
                    any <- TRUE
                    iempty <- which(out == "")
                    iskip <- unlist(lapply(iskip, function(i) i:(iempty[iempty > 
                      i][1L] - 1L)))
                    noteLog(Log)
                ...
                if (!WINDOWS && !is.na(theta)) {
                  ...
                  if (cpu >= pmax(theta * td[3L], 1)) {
                    ...
                    ratio <- round(cpu/td[3L], 1L)
                    printLog(Log, sprintf("Re-building vignettes had CPU time %g times elapsed time\n", 
                      ratio))
                  }
                }
                if (!any) 
    ...
    check_pkg_manual <- function(pkgdir, pkgname) {
        if (dir.exists(file.path(pkgdir, "help")) || dir.exists(file.path(pkgdir, 
            "man"))) {
            ...
            build_dir <- gsub("\\", "/", tempfile("Rd2pdf"), 
                fixed = TRUE)
            man_file <- paste0(pkgname, "-manual.pdf ")
            if (file.exists(man_file)) 
                unlink(man_file)
            args <- c("Rd2pdf ", Rd2pdf_opts, paste0("--build-dir=", 
                shQuote(build_dir)), "--no-clean", "-o ", man_file, 
                shQuote(topdir))
            t1 <- proc.time()
            ...
            if (res == 11) {
                ...
                warningLog(Log)
                printLog0(Log, paste0("LaTeX errors when creating PDF version.\n", 
                ...
                args <- c("Rd2pdf ", Rd2pdf_opts, paste0("--build-dir=", 
                  shQuote(build_dir)), "--no-clean", "--no-index", 
                  "--force", "-o ", man_file, topdir)
    ...
    check_Rd2HTML <- function(dir, installed = FALSE) {
        ...
        if (i1) {
            ...
            if (OK1) {
                ...
                results1 <- lapply(db, function(x) tryCatch({
                  Rd2HTML(x, out, concordance = TRUE)
                  tidy_validate(out, tidy = Tidy)
                }, error = identity))
                ignore <- Sys.getenv("_R_CHECK_RD_VALIDATE_RD2HTML_IGNORE_EMPTY_SPANS_", 
                  "true")
        ...
        if (i2) {
            ...
            if (OK2) 
                results2 <- check_math_rendering_in_Rd_db(eq = eq, 
                  katex = katex)
        }
        t2 <- proc.time()
        print_time(t1, t2, Log)
        if (i1) {
            if (!OK1) {
                noteLog(Log)
                any <- TRUE
                txt <- paste("Please obtain a recent version of HTML Tidy", 
                  "by downloading a binary release", "or compiling the source code from", 
                  "<https://www.html-tidy.org/>.")
                txt <- paste(strwrap(txt), collapse = "\n")
                printLog0(Log, c("Skipping checking HTML validation: ", 
                  attr(Tidy, "msg"), ".\n", txt, "\n"))
            }
            if (OK1 && length(errors <- attr(results1, "errors"))) {
                if (!any) 
                  noteLog(Log)
                any <- TRUE
                printLog0(Log, c("Encountered the following conversion/validation errors:\n", 
            ...
            if (OK1 && NROW(results1)) {
                if (!any) 
                  noteLog(Log)
                any <- TRUE
                printLog0(Log, c("Found the following HTML validation problems:\n", 
                  sprintf("%s:%s:%s (%s:%s): %s\n", sub("[Rr]d$", 
                    "html", results1[, "path"]), results1[, "line"], 
                    results1[, "col"], results1[, "srcFile"], 
                    results1[, "srcLine"], results1[, "msg"])))
            }
        }
        if (i2) {
            if (!OK2) {
                if (!any) 
                  noteLog(Log)
                any <- TRUE
                printLog0(Log, "Skipping checking math rendering: package 'V8' unavailable\n")
            }
            if (OK2 && NROW(results2)) {
                if (!any) 
                  noteLog(Log)
                any <- TRUE
                printLog0(Log, c("Found the following math rendering problems:\n", 
                  sprintf("%s%s: %s\n", results2[, 1L], results2[, 
                    2L], gsub("\n", "\n  ", results2[, 3L]))))
            }
        }
        if (!any) 
    ...
    check_executables <- function() {
        ...
        FILE <- "file"
        lines <- suppressWarnings(tryCatch(system2(FILE, "--version", 
            TRUE, TRUE), error = function(e) "error"))
        have_free_file <- any(grepl("^(file-[45]|magic file from)", 
            lines))
        if (!have_free_file) {
            FILE <- "gfile"
            lines <- suppressWarnings(tryCatch(system2(FILE, 
                "--version", TRUE, TRUE), error = function(e) "error"))
        ...
        if (have_free_file) {
            ...
            while (ll <- length(files)) {
                ...
                these <- files[chunk]
                files <- files[-chunk]
                lines <- suppressWarnings(system2(FILE, shQuote(these), 
                  TRUE, TRUE))
            ...
            if (length(execs)) {
                ...
                for (fp in c("foreign/tests/datefactor.dta")) known <- known | 
                  grepl(fp, pexecs)
                execs <- execs[!known]
            }
        }
        ...
        if (nb <- length(execs)) {
            msg <- ngettext(nb, "Found the following executable file:", 
                "Found the following executable files:", domain = NA)
            warningLog(Log, msg)
    ...
    .hidden_file_exclusions <- c(".Renviron", ".Rprofile", ".Rproj.user", 
        ".Rhistory", ".Rapp.history", ".tex", ".log", ".aux", 
        ".pdf", ".png", ".backups", ".cvsignore", ".cproject", 
        ".directory", ".dropbox", ".exrc", ".gdb.history", ".gitattributes", 
        ".gitignore", ".gitmodules", ".hgignore", ".hgtags", 
        ...
        if (length(dots)) {
            ...
            if (cran) {
                ...
                known <- known | grepl("^.Rbuildindex[.]", dots) | 
                  endsWith(dots, "inst/doc/.Rinstignore") | endsWith(dots, 
                  "inst/doc/.build.timestamp") | endsWith(dots, 
                  "vignettes/.Rinstignore") | grepl("^src.*/[.]deps$", 
                  dots)
                if (all(known)) 
    ...
    check_install <- function() {
        if (install == "skip") 
            ...
            tlim <- get_timeout(Sys.getenv("_R_CHECK_INSTALL_ELAPSED_TIMEOUT_", 
                Sys.getenv("_R_CHECK_ELAPSED_TIMEOUT_")))
            use_install_log <- (startsWith(install, "check") || 
                R_check_use_install_log || !isatty(stdout()))
            ...
            if (!use_install_log) {
                ...
                checkingLog(Log, "whether package ", sQuote(desc["Package"]), 
                  " can be installed")
                ...
                if (startsWith(install, "check")) {
                  ...
                  if (!nzchar(thislog)) {
                    errorLog(Log, sprintf("install log %s does not exist", 
                      sQuote(thislog)))
                    summaryLog(Log)
                    do_exit(2L)
                  }
                  ...
                  cat("* install options ", sQuote(INSTALL_opts), 
                    "\n\n", sep = "", file = outfile)
                  t1 <- proc.time()
                  install_error <- run_Rcmd(args, outfile, timeout = tlim)
                  t2 <- proc.time()
                  print_time(t1, t2, Log)
                  ...
                  if (!WINDOWS && !is.na(theta)) {
                    ...
                    if (td[3L] >= td0) {
                      ...
                      if (cpu >= pmax(theta * td[3L], 1)) {
                        ratio <- round(cpu/td[3L], 1L)
                        tOK_msg <- sprintf("Installation took CPU time %g times elapsed time\n", 
                          ratio)
                      }
                    }
                  }
                ...
                if (install_error) {
                  errorLog(Log, "Installation failed.")
                  printLog0(Log, "See ", sQuote(outfile), " for details.\n")
                  summaryLog(Log)
                  do_exit(1L)
                }
                if (install != "check") 
                ...
                lines00 <- unique(grep("^using (C compiler|C[+][+] compiler|Fortran compiler|SDK)", 
                  lines, value = TRUE))
                lines0 <- lines
                warn_re <- c("^WARNING:", "^Warning:", "^Warning.*: .*is deprecated[.]$", 
                  "^Rd (warning|error): ", ": warning: .*ISO C", 
                  ": warning: .* discards qualifiers from pointer target type", 
                  ": warning: .* is used uninitialized", ": warning: .* set but not used", 
                  ": warning: unused", ": warning: .* makes pointer from integer", 
                  ": warning: .* pointer.* conversion", ": warning: improper pointer", 
                  ": warning: unknown escape sequence", ": warning: use of non-standard escape character", 
                  "warning: void function", "warning: control reaches end of non-void function", 
                  "warning: no return statement in function returning non-void", 
                  "^ *# *warning", "Warning: # *warning", "missing links?:", 
                  "^Note: possible error in", "^Note: (break|next) used in wrong context: no loop is visible", 
                  "^  The prototype for class.*undefined slot", 
                  "'config' variable.*is deprecated")
                check_src_flag <- Sys.getenv("_R_CHECK_SRC_MINUS_W_IMPLICIT_", 
                  "TRUE")
                if (config_val_to_logical(check_src_flag)) {
                  warn_re <- c(warn_re, ": warning: implicit declaration of function", 
                    ": warning: incompatible implicit declaration of built-in function")
                }
                warn_re <- c(warn_re, ": warning: .* \\[-Wreturn-type-c-linkage\\]")
                warn_re <- c(warn_re, ": warning: pointer of type .* used in arithmetic", 
                  ": warning: .* \\[-Wformat-contains-nul\\]", 
                  ": warning: .* \\[-Wformat-zero-length\\]", 
                  ": warning: .* \\[-Wpointer-to-int-cast\\]", 
                  ": warning: .* \\[-Wsequence-point\\]", ": warning: .* \\[-Wformat-overflow=\\]", 
                  ": warning: .* \\[-Wformat-truncation=\\]", 
                  ": warning: .* \\[-Wnonull", ": warning: .* \\[-Walloc-size-larger-than=\\]", 
                  ": warning: .* \\[-Wterminate\\]", ": warning: .* \\[-Wint-conversion\\]", 
                  ": warning: .* \\[-Wconversion-null\\]", ": warning: .* GCC extension", 
                  ": warning: .* \\[-Wsizeof-pointer-memaccess\\]", 
                  ": warning: suggest parentheses around (comparison|assignment)", 
                  ": warning: .* \\[-Wstringop", ": warning: .* \\[-Wclass-memaccess\\]", 
                  ": warning: .* \\[-Wdeprecated\\]", ": warning: .* \\[-Waligned-new", 
                  ": warning: .* \\[-Wcatch-value=\\]", ": warning: .* \\[-Wnarrowing\\]", 
                  ": warning: .* \\[-Waddress", ": warning: .* \\[-Woverflow", 
                  ...
                  ": warning: .*\\[-Wlto", ": warning: .*\\[-Wodr\\]", 
                  ": warning: .*\\[-Wswitch\\]", ": warning: line number out of range", 
                  ...
                  ": warning: .*\\[-Wpointer-sign\\]", ": warning: .* \\[-Wformat=\\]", 
                  ": warning: .* \\[-Wstrict-prototypes\\]", 
                  ": warning: .* \\[-Wdeprecated-non-prototype\\]", 
                  "\\[-Wcpp\\] ", "\\[-W#warnings\\]", "\\[-Wrange-loop-construct\\]", 
                  "\\[-Warray-parameter=\\]", "\\[-Wtemplate-id-cdtor\\]", 
                  "\\[-Warray-parameter\\]", "\\[-Wuse-after-free\\]", 
                  "^warning: use of deprecated")
                warn_re <- c(warn_re, ": warning: .* \\[-Wunused-result\\]", 
                  ": warning: .* \\[-Warray-bounds\\]", ": warning: .* \\[-Wrestrict\\]")
                warn_re <- c(warn_re, ": warning: .* GNU extension", 
                  ": warning: .* \\[-Wdeprecated-register\\]", 
                  ": warning: .* \\[-Wdeprecated-declarations\\]", 
                  ": warning: .* \\[-Wformat-extra-args\\]", 
                  ": warning: .* \\[-Wformat-security\\]", ": warning: .* \\[-Wformat-insufficient-args\\]", 
                  ": warning: .* \\[-Wheader-guard\\]", ": warning: .* \\[-Wpointer-arith\\]", 
                  ": warning: .* \\[-Wunsequenced\\]", ": warning: .* \\[-Wvla-extension\\]", 
                  ": warning: .* \\[-Wmismatched-new-delete\\]", 
                  ": warning: .* \\[-Wabsolute-value\\]", ": warning: .* \\[-Wreorder\\]", 
                  ": warning: .* \\[-Wself-assign", ": warning: .* \\[-Wtautological", 
                  ...
                  ": warning: .* \\[-Wxor-used-as-pow\\]", ": warning: .* \\[-Winconsistent-missing-override\\]", 
                  ": warning: .* \\[-Wsizeof-array-div\\]", ": warning: .* \\[-Wvarargs\\]", 
                  ": warning: .* \\[-Wlogical-not-parentheses\\]", 
                  "warning: .* \\[-Wunknown-warning-option\\]", 
                  "warning: .* \\[-Wnested-anon-types\\]", "warning: .* is not needed and will not be emitted", 
                  "warning: .* \\[-Wnon-literal-null-conversion\\]", 
                  "warning: .* \\[-Wignored-optimization-argument\\]", 
                  "warning: .* \\[-Wunused-comparison\\]", "warning: .* \\[-Wliteral-conversion\\]", 
                  "warning: .* \\[-Wempty-body\\]", "warning: .* \\[-Wformat\\]", 
                  "warning: .* \\[-Wreturn-stack-address\\]", 
                  "warning: .* \\[-Wuninitialized\\]", "warning: .* \\[-Wuninitialized-const-reference\\]", 
                  "warning: .* \\[-Wsizeof-pointer-div\\]", "warning: .* \\[-Wnon-c-typedef-for-linkage\\]", 
                  ...
                  "warning: .* \\[-Winvalid-utf8\\]", "warning: .* \\[-Wunqualified-std-cast-call\\]", 
                ...
                lines <- grep("exceeds maximum object size.*-W(alloc-size-larger-than|stringop-overflow)", 
                  lines, value = TRUE, useBytes = TRUE, invert = TRUE)
                ...
                lines <- filtergrep("Warning: incomplete final line found by readLines", 
                  lines, useBytes = TRUE)
                check_Stangle <- Sys.getenv("_R_CHECK_STANGLE_WARNINGS_", 
                  "TRUE")
                ...
                if (!R_check_all_non_ISO_C) {
                  ...
                  lines <- filtergrep("warning: *ISO C forbids.*function pointer", 
                    lines, useBytes = TRUE)
                  if (WINDOWS) 
                    lines <- filtergrep("warning: *ISO C does not support.*ms_printf length modifier", 
                      lines, useBytes = TRUE)
                }
                ...
                if (!config_val_to_logical(check_src_flag)) {
                  ...
                  lines <- filtergrep("warning: .* set but not used", 
                    lines, ignore.case = TRUE, useBytes = TRUE)
                }
                check_src_flag <- Sys.getenv("_R_CHECK_SRC_MINUS_W_SOMETIMES_UNINITIALIZED_", 
                  "FALSE")
                if (!config_val_to_logical(check_src_flag)) {
                  lines <- filtergrep("warning: .* is used uninitialized whenever", 
                    lines, useBytes = TRUE)
                }
                check_src_flag <- Sys.getenv("_R_CHECK_WALL_FORTRAN_", 
                  "FALSE")
                if (!config_val_to_logical(check_src_flag)) {
                  warn_re <- c("Label .* at \\(1\\) defined but not used", 
                    "Line truncated at \\(1\\)", "ASSIGN statement at \\(1\\)", 
                    "Assigned GOTO statement at \\(1\\)", "arithmetic IF statement at \\(1\\)", 
                    "Nonconforming tab character (in|at)", "Obsolescent feature:")
                  warn_re <- c(warn_re, "Warning: .*\\[-Wconversion]", 
                    "Warning: .*\\[-Wmaybe-uninitialized]", "Warning: .*\\[-Wintrinsic-shadow]", 
                    "Warning: GNU Extension: DOUBLE COMPLEX", 
                    "Warning: GNU Extension: .*COMPLEX[*]16")
                  check_src_flag <- Sys.getenv("_R_CHECK_SRC_MINUS_W_UNUSED_", 
                    "FALSE")
                  if (!config_val_to_logical(check_src_flag)) 
                    warn_re <- c(warn_re, "Warning: .*\\[-Wunused-function]", 
                ...
                if (WINDOWS) {
                  ...
                  lines <- filtergrep("Warning: this package has a non-empty 'configure.win' file", 
                    lines)
                  lines <- filtergrep("Warning: .drectve .* unrecognized", 
                    lines)
                }
                check_imports_flag <- Sys.getenv("_R_CHECK_REPLACING_IMPORTS_", 
                  "TRUE")
                if (!config_val_to_logical(check_imports_flag)) 
                  lines <- filtergrep("Warning: replacing previous import", 
                    lines, fixed = TRUE)
                else {
                  this <- unique(grep("Warning: replacing previous import", 
                    lines, fixed = TRUE, value = TRUE))
                  this <- grep(paste0("when loading .*", pkgname, 
                    ".*$"), this, value = TRUE)
                  lines <- filtergrep("Warning: replacing previous import", 
                    lines, fixed = TRUE)
                  lines <- c(lines, this)
                }
                check_FirstLib_flag <- Sys.getenv("_R_CHECK_DOT_FIRSTLIB_", 
                  "FALSE")
                if (!config_val_to_logical(check_FirstLib_flag)) 
                  lines <- filtergrep("Warning: ignoring .First.lib()", 
                    lines, fixed = TRUE)
                lines <- unique(lines)
                ...
                lines <- lines[!ex]
                this <- grep("Error: compilation failed -", lines0, 
                  value = TRUE)
                lines <- c(lines, unique(this))
                note_re <- "warning: control may reach end of non-void function"
                notes <- grep(note_re, lines0, value = TRUE, 
                  useBytes = TRUE)
                notes <- unique(notes)
                if (any(grepl("using non-staged installation", 
                  lines0, useBytes = TRUE))) 
                  notes <- c("Non-staged installation was used", 
                    notes)
                if (length(lines)) {
                  warningLog(Log, "Found the following significant warnings:")
                  printLog0(Log, .format_lines_with_indent(lines), 
                    "\n")
                  if (length(notes)) {
                    printLog(Log, "Found the following additional notes/warnings:\n")
                  ...
                  if (any(grepl("^Note:", lines, useBytes = TRUE))) 
                    wrapLog("Information on the location(s)", 
                      "of code generating the", paste0(sQuote("Note"), 
                        "s"), "can be obtained by re-running with", 
                ...
                line <- unique(grep("^using C[+][+][12]", lines0, 
                  value = TRUE))
                if (length(line)) {
                  checkingLog(Log, "C++ specification")
                  std <- as.numeric(sub("using C[+][+]", "", 
                    line))
                  if (std < 17) {
                    noteLog(Log, sprintf("  Specified C++%d: please drop specification unless essential", 
                      std))
                  }
                  else if (std >= 20) {
                    resultLog(Log, "OK")
                    printLog(Log, sprintf("  Not all R platforms support C++%s\n", 
                      std))
                  }
                  else resultLog(Log, "OK")
                }
    ...
    check_install_sizes <- function() {
        ...
        if (!is.na(total) && total > 1024 * as.numeric(Sys.getenv("_R_CHECK_PKG_SIZES_THRESHOLD_", 
            unset = 5)) && pkgname != "Matrix") {
            noteLog(Log)
            ...
            rest <- rest[rest[, 1L] > 1024, ]
            if (nrow(rest)) {
                o <- sort.list(rest[, 2L])
                printLog(Log, "  sub-directories of 1Mb or more:\n")
                size <- sprintf("%4.1fMb", rest[, 1L]/1024)
                printLog0(Log, paste0("    ", format(rest[o, 
                  2L], justify = "left"), "  ", format(size[o], 
                  justify = "right"), "\n"))
            }
        }
        else resultLog(Log, "OK")
    ...
    check_description <- function() {
        checkingLog(Log, "for file ", sQuote(file.path(pkgname0, 
            "DESCRIPTION")))
        if ("DESCRIPTION" %in% dir(pkgdir)) {
            ...
            OK <- vapply(desc[mandatory], function(x) !is.na(x) && 
                nzchar(x), NA)
            if (!all(OK)) {
                fail <- mandatory[!OK]
                msg <- ngettext(length(fail), "Required field missing or empty:", 
            ...
            if (!grepl("^[[:alpha:]][[:alnum:].]*[[:alnum:]]$", 
                desc["Package"]) || endsWith(desc["Package"], 
                ".")) {
                warningLog(Log)
            ...
            errorLog(Log, "File DESCRIPTION does not exist but there is a case-insensitive match.")
            summaryLog(Log)
            do_exit(1L)
        }
        else {
            errorLog(Log, "File DESCRIPTION does not exist")
            summaryLog(Log)
            do_exit(1L)
        }
        if (!is.na(desc["Type"])) {
            ...
            if (desc["Type"] != "Package") {
                errorLog(Log, sprintf("Extensions with Type %s cannot be checked.", 
                  sQuote(desc["Type"])))
                summaryLog(Log)
                do_exit(0L)
            }
        ...
        if (!is.na(desc["Bundle"])) {
            ...
            errorLog(Log, sprintf("Looks like %s is a package bundle -- they are defunct", 
                sQuote(pkgname0)))
            summaryLog(Log)
            do_exit(1L)
        }
        messageLog(Log, sprintf("this is package %s version %s", 
            sQuote(desc["Package"]), sQuote(desc["Version"])))
        if (!is.na(encoding)) 
    ...
    check_CRAN_incoming <- function(localOnly, pkgSize) {
        ...
        if (length(res)) {
            ...
            if (length(out) == 1L && startsWith(out, "Maintainer: ")) {
                ...
                if (nzchar(maintainer) && identical(maintainer, 
            ...
            else if (length(res$bad_version) || length(res$strong_dependencies_not_in_mainstream_repositories) || 
                isTRUE(res$foss_with_BuildVignettes) || isTRUE(res$Maintainer_invalid_or_multi_person) || 
                isTRUE(res$empty_Maintainer_name) || isTRUE(res$Maintainer_needs_quotes)) 
                warningLog(Log)
    ...
    check_dependencies <- function() {
        if (!extra_arch && file.exists(file.path(pkgdir, "NAMESPACE"))) {
            checkingLog(Log, "package namespace information")
            ns <- tryCatch(parseNamespaceFile(basename(pkgdir), 
                dirname(pkgdir)), error = function(e) {
                errorLog(Log)
                printLog0(Log, "Invalid NAMESPACE file, parsing gives:", 
                  "\n", as.character(e), "\n")
                msg_NAMESPACE <- c("See section 'Package namespaces'", 
                  " in the 'Writing R Extensions' manual.\n")
            ...
            imp <- imp[lens == 2L]
            nm <- sapply(imp, `[[`, 1)
            lens <- vapply(imp, function(x) length(x[[2L]]), 
                0L)
            bad <- nm[lens == 0L]
            if (length(bad)) {
                OK <- FALSE
                msg <- if (length(bad) == 1L) 
                  sprintf("  Namespace with empty importFrom: %s", 
                    sQuote(bad))
                else paste0("  Namespaces with empty importFrom:\n", 
                  .pretty_format(sort(bad)))
                noteLog(Log, msg)
            }
            pi <- .split_description(.read_description(file.path(pkgdir, 
                "DESCRIPTION")))
            dependsMethods <- "methods" %in% c(names(pi$Depends), 
                names(pi$Imports))
            if (!dependsMethods && length(bad <- Filter(length, 
                ns[c("exportClasses", "exportMethods", "exportClassPatterns")]))) {
                OK <- FALSE
                noteLog(Log, ngettext(length(bad), "Found export directive that requires package 'methods':", 
                  "Found export directives that require package 'methods':", 
                  domain = NA))
                printLog0(Log, paste0(.pretty_format(names(bad)), 
                  collapse = "\n"), "\n")
        ...
        if (any(lengths(res) > 0L)) {
            out <- format(res)
            allowed <- c("suggests_but_not_installed", "enhances_but_not_installed", 
                "many_depends", "many_imports", "skipped", "hdOnly", 
                "orphaned2", "orphaned", "orphaned1", "required_for_checking_but_not_installed", 
                "no_vignettes", if (!check_incoming) "bad_engine")
            if (!all(names(res) %in% allowed)) {
                ...
                if (length(res$suggested_but_not_installed)) 
                  wrapLog("The suggested packages are required for", 
                    ...
                    "by setting the environment variable", "_R_CHECK_FORCE_SUGGESTS_", 
                    "to a false value.\n\n")
                wrapLog(msg_DESCRIPTION)
                summaryLog(Log)
                do_exit(1L)
    ...
    check_sources <- function() {
        ...
        if (!is.na(desc["Built"])) {
            ...
            ini <- character()
            pat <- "(a|o|[ls][ao]|sl|obj|dll)"
            any <- FALSE
            srcd <- file.path(pkgdir, "src")
            if (dir.exists(srcd) && length(of <- list_files_with_exts(srcd, 
                pat))) {
                if (!any) 
                  warningLog(Log)
                any <- TRUE
                of <- sub(paste0(".*/", file.path(pkgname, "src"), 
                  "/"), "", of)
                printLog0(Log, sprintf("Subdirectory %s contains apparent object files/libraries\n", 
                  sQuote(file.path(pkgname, "src"))), paste(strwrap(paste(of, 
                  collapse = " "), indent = 2L, exdent = 2L), 
                  collapse = "\n"), "\nObject files/libraries should not be included in a source package.\n")
                ini <- ""
            }
            ad <- list.dirs(pkgdir, recursive = FALSE)
            if (thispkg_src_subdirs != "no" && any(ind <- grepl("/src-(i386|x64|x86_64|ppc)$", 
                ad))) {
                if (!any) 
                  warningLog(Log)
                any <- TRUE
                msg <- ngettext(sum(ind), "Found the following directory with a name of a multi-arch build directory:\n", 
                  "Found the following directories with names of multi-arch build directories:\n", 
                  domain = NA)
                printLog0(Log, ini, msg, .format_lines_with_indent(basename(ad[ind])), 
                  "\n", "Most likely, these were included erroneously.\n")
                ini <- ""
            }
            if (thispkg_src_subdirs != "no" && dir.exists(srcd)) {
                setwd(srcd)
                if (!file.exists("Makefile") && !file.exists("Makefile.win") && 
                  !file.exists("Makefile.ucrt") && !(file.exists("Makefile.in") && 
                  spec_install)) {
                  srcfiles <- dir(".", all.files = TRUE)
                  srcfiles <- srcfiles[!dir.exists(srcfiles)]
                  srcfiles <- filtergrep("(\\.([cfmCM]|cc|cpp|f90|f95|mm|h|o|so)$|^Makevars|-win\\.def|^install\\.libs\\.R$)", 
                    srcfiles)
                  if (length(srcfiles)) {
                    if (!any) 
                      warningLog(Log)
                    any <- TRUE
                    msg <- c(ini, paste("Subdirectory", sQuote("src"), 
                      "contains:"), strwrap(paste(srcfiles, collapse = " "), 
                      indent = 2, exdent = 2), strwrap("These are unlikely file names for src files."), 
                      "")
                    printLog0(Log, paste(msg, collapse = "\n"))
                    ini <- ""
                  }
            ...
            bad <- bad[dirname(bad) != "src" | dirname(dirname(bad)) != 
                "."]
            if (length(bad)) {
                if (!any) 
                  noteLog(Log)
                any <- TRUE
                msg <- c(ini, "Found the following apparent object files/libraries:", 
            ...
            if (length(files) && all(!is.na(match(c("package.rds", 
                "hsearch.rds"), basename(files))))) {
                if (!any) 
                  noteLog(Log)
                any <- TRUE
                msg <- c(ini, sprintf("Subdirectory %s seems to contain an installed version of the package.\n", 
    ...
    do_exit <- if (no.q) 
        function(status) (if (status) 
            stop
        else message)(".check_packages() exit status ", status)
    ...
    Usage <- function() {
        cat("Usage: R CMD check [options] pkgs", "", "Check R packages from package sources, which can be directories or", 
            "package 'tar' archives with extension '.tar.gz', '.tar.bz2',", 
            "'.tar.xz' or '.tgz'.", "", "A variety of diagnostic checks on directory structure, index and", 
            ...
            "as is re-building the vignette PDFs.", "", "Options:", 
            ...
            "           checked package", "      --no-clean        do not clean 'outdir' before using it", 
            ...
            "", "Special-purpose options:", "      --check-subdirs=default|yes|no", 
            ...
            "                        sub-architecture.", "      --multiarch       do runtime tests on all installed sub-archs", 
    ...
    warnOption <- max(getOption("warn"), warnOption)
    ...
    if (no.q) {
        ..check.wd.. <- getwd()
        on.exit(setwd(..check.wd..), add = TRUE)
    }
    Renv <- Sys.getenv("R_CHECK_ENVIRON", unset = NA_character_)
    if (!is.na(Renv)) {
        if (nzchar(Renv) && file.exists(Renv)) 
            readRenviron(Renv)
    }
    else {
        rarch <- .Platform$r_arch
        if (nzchar(rarch) && file.exists(Renv <- paste0("~/.R/check.Renviron.", 
            rarch))) 
            readRenviron(Renv)
    ...
    while (length(args)) {
        ...
        if (a %in% c("-h", "--help")) {
            ...
            cat("R add-on package check: ", R.version[["major"]], 
    ...
    if (run_dontrun) 
        opts <- c(opts, "--run-dontrun")
    if (run_donttest) 
        opts <- c(opts, "--run-donttest")
    opts0 <- opts
    if (install == "fake") {
        do_examples <- do_tests <- do_vignettes <- do_build_vignettes <- 0
        spec_install <- TRUE
    ...
    install_log_path <- ""
    if (startsWith(install, "check")) {
        install_log_path <- tryCatch(file_path_as_absolute(substr(install, 
            7L, 1000L)), error = function(e) "")
    }
        ...
        if (WINDOWS) {
            ...
            if (length(archs) > 1L) 
                for (arch in archs) {
                  if (arch == rarch) 
                    next
                  cmd <- paste0(shQuote(file.path(R.home(), "bin", 
                    "R")), " --arch=", arch, " --version > /dev/null")
                  if (system(cmd)) 
    ...
    R_check_use_install_log <- config_val_to_logical(Sys.getenv("_R_CHECK_USE_INSTALL_LOG_", 
        "TRUE"))
    R_check_subdirs_nocase <- config_val_to_logical(Sys.getenv("_R_CHECK_SUBDIRS_NOCASE_", 
        "TRUE"))
    R_check_all_non_ISO_C <- config_val_to_logical(Sys.getenv("_R_CHECK_ALL_NON_ISO_C_", 
        "FALSE"))
    R_check_subdirs_strict <- Sys.getenv("_R_CHECK_SUBDIRS_STRICT_", 
        "default")
    R_check_Rd_contents <- config_val_to_logical(Sys.getenv("_R_CHECK_RD_CONTENTS_", 
        "TRUE"))
    R_check_Rd_line_widths <- config_val_to_logical(Sys.getenv("_R_CHECK_RD_LINE_WIDTHS_", 
        "FALSE"))
    R_check_Rd_style <- config_val_to_logical(Sys.getenv("_R_CHECK_RD_STYLE_", 
        "TRUE"))
    R_check_Rd_xrefs <- config_val_to_logical(Sys.getenv("_R_CHECK_RD_XREFS_", 
        "TRUE"))
    R_check_Rd_internal_too <- config_val_to_logical(Sys.getenv("_R_CHECK_RD_INTERNAL_TOO_", 
        "NA"))
    R_check_use_codetools <- config_val_to_logical(Sys.getenv("_R_CHECK_USE_CODETOOLS_", 
        "TRUE"))
    if (R_check_use_codetools) {
        tmp <- tryCatch(find.package("codetools"), error = identity)
        if (inherits(tmp, "error")) 
            R_check_use_codetools <- FALSE
    }
    R_check_executables <- config_val_to_logical(Sys.getenv("_R_CHECK_EXECUTABLES_", 
        "TRUE"))
    R_check_executables_exclusions <- config_val_to_logical(Sys.getenv("_R_CHECK_EXECUTABLES_EXCLUSIONS_", 
        "TRUE"))
    R_check_permissions <- config_val_to_logical(Sys.getenv("_R_CHECK_PERMISSIONS_", 
        as.character(.Platform$OS.type == "unix")))
    R_check_dot_internal <- config_val_to_logical(Sys.getenv("_R_CHECK_DOT_INTERNAL_", 
        "TRUE"))
    R_check_depr_def <- config_val_to_logical(Sys.getenv("_R_CHECK_DEPRECATED_DEFUNCT_", 
        "FALSE"))
    R_check_bogus_return <- config_val_to_logical(Sys.getenv("_R_CHECK_BOGUS_RETURN_", 
        "FALSE"))
    R_check_ascii_code <- config_val_to_logical(Sys.getenv("_R_CHECK_ASCII_CODE_", 
        "TRUE"))
    R_check_ascii_data <- config_val_to_logical(Sys.getenv("_R_CHECK_ASCII_DATA_", 
        "TRUE"))
    R_check_compact_data <- config_val_to_logical(Sys.getenv("_R_CHECK_COMPACT_DATA_", 
        "TRUE"))
    R_check_vc_dirs <- config_val_to_logical(Sys.getenv("_R_CHECK_VC_DIRS_", 
        "FALSE"))
    R_check_pkg_sizes <- config_val_to_logical(Sys.getenv("_R_CHECK_PKG_SIZES_", 
        "TRUE")) && nzchar(Sys.which("du"))
    R_check_doc_sizes <- config_val_to_logical(Sys.getenv("_R_CHECK_DOC_SIZES_", 
        "TRUE")) && nzchar(Sys.which(Sys.getenv("R_QPDF", "qpdf")))
    R_check_doc_sizes2 <- config_val_to_logical(Sys.getenv("_R_CHECK_DOC_SIZES2_", 
        "FALSE"))
    R_check_code_assign_to_globalenv <- config_val_to_logical(Sys.getenv("_R_CHECK_CODE_ASSIGN_TO_GLOBALENV_", 
        "FALSE"))
    R_check_code_attach <- config_val_to_logical(Sys.getenv("_R_CHECK_CODE_ATTACH_", 
        "FALSE"))
    R_check_code_data_into_globalenv <- config_val_to_logical(Sys.getenv("_R_CHECK_CODE_DATA_INTO_GLOBALENV_", 
        "FALSE"))
    R_check_suppress_RandR_message <- do_install_arg && config_val_to_logical(Sys.getenv("_R_CHECK_SUPPRESS_RANDR_MESSAGE_", 
        "TRUE"))
    R_check_force_suggests <- config_val_to_logical(Sys.getenv("_R_CHECK_FORCE_SUGGESTS_", 
        "TRUE"))
    R_check_skip_tests_arch <- unlist(strsplit(Sys.getenv("_R_CHECK_SKIP_TESTS_ARCH_"), 
        ",")[[1]])
    R_check_skip_examples_arch <- unlist(strsplit(Sys.getenv("_R_CHECK_SKIP_EXAMPLES_ARCH_"), 
        ",")[[1]])
    R_check_skip_arch <- unlist(strsplit(Sys.getenv("_R_CHECK_SKIP_ARCH_"), 
        ",")[[1]])
    R_check_unsafe_calls <- config_val_to_logical(Sys.getenv("_R_CHECK_UNSAFE_CALLS_", 
        "TRUE"))
    R_cdo <- config_val_to_logical(Sys.getenv("_R_CHECK_DEPENDS_ONLY_", 
        "FALSE"))
    R_cdo_data <- config_val_to_logical(Sys.getenv("_R_CHECK_DEPENDS_ONLY_DATA_", 
        R_cdo))
    R_cdo_examples <- config_val_to_logical(Sys.getenv("_R_CHECK_DEPENDS_ONLY_EXAMPLES_", 
        R_cdo))
    R_cdo_tests <- config_val_to_logical(Sys.getenv("_R_CHECK_DEPENDS_ONLY_TESTS_", 
        R_cdo))
    R_cdo_vignettes <- config_val_to_logical(Sys.getenv("_R_CHECK_DEPENDS_ONLY_VIGNETTES_", 
        R_cdo))
    R_check_suggests_only <- config_val_to_logical(Sys.getenv("_R_CHECK_SUGGESTS_ONLY_", 
        "FALSE"))
    R_check_FF <- Sys.getenv("_R_CHECK_FF_CALLS_", "true")
    R_check_FF_DUP <- config_val_to_logical(Sys.getenv("_R_CHECK_FF_DUP_", 
        "TRUE"))
    R_check_toplevel_files <- config_val_to_logical(Sys.getenv("_R_CHECK_TOPLEVEL_FILES_", 
        "FALSE"))
    R_check_exit_on_first_error <- config_val_to_logical(Sys.getenv("_R_CHECK_EXIT_ON_FIRST_ERROR_", 
        "FALSE"))
    R_check_vignettes_skip_run_maybe <- config_val_to_logical(Sys.getenv("_R_CHECK_VIGNETTES_SKIP_RUN_MAYBE_", 
        "TRUE"))
    R_check_serialization <- config_val_to_logical(Sys.getenv("_R_CHECK_SERIALIZATION_", 
        "FALSE"))
    R_check_things_in_check_dir <- config_val_to_logical(Sys.getenv("_R_CHECK_THINGS_IN_CHECK_DIR_", 
        "FALSE"))
    R_check_things_in_temp_dir <- config_val_to_logical(Sys.getenv("_R_CHECK_THINGS_IN_TEMP_DIR_", 
        "FALSE"))
    R_check_things_in_others <- config_val_to_logical(Sys.getenv("_R_CHECK_THINGS_IN_OTHER_DIRS_", 
        "FALSE"))
    R_check_vignette_titles <- config_val_to_logical(Sys.getenv("_R_CHECK_VIGNETTE_TITLES_", 
        "FALSE"))
    R_check_code_class_is_string <- config_val_to_logical(Sys.getenv("_R_CHECK_CODE_CLASS_IS_STRING_", 
        "FALSE"))
    tmp <- Sys.getenv("_R_CHECK_RD_VALIDATE_RD2HTML_", "unset")
    R_check_Rd_validate_Rd2HTML <- if (tmp == "unset") 
        NA
    else config_val_to_logical(tmp)
    R_check_Rd_math_rendering <- config_val_to_logical(Sys.getenv("_R_CHECK_RD_MATH_RENDERING_", 
        "FALSE"))
    if (!nzchar(check_subdirs)) 
        check_subdirs <- R_check_subdirs_strict
    if (as_cran) {
        if (extra_arch) {
        ...
        if (is.na(prev)) 
            Sys.setenv(`_R_CHECK_SCREEN_DEVICE_` = "stop")
        ...
        Sys.setenv(`_R_CHECK_R_ON_PATH_` = if (WINDOWS) 
            "FALSE"
        else "TRUE")
        ...
        if (is.na(prev)) 
            Sys.setenv(`_R_CHECK_MATRIX_DATA_` = "TRUE")
    ...
    snap <- if (R_check_things_in_others) 
        snapshot()
    else NULL
    ...
    msg_DESCRIPTION <- c("See section 'The DESCRIPTION file' in the 'Writing R Extensions' manual.\n")
    if (!length(pkgs)) {
    ...
    for (pkg in pkgs) {
        ...
        if (dir.exists(pkg)) {
            is_tar <- FALSE
            if (thispkg_subdirs == "default") 
                thispkg_subdirs <- "no"
        }
        else if (file.exists(pkg)) {
            is_tar <- TRUE
            if (thispkg_subdirs == "default") 
                thispkg_subdirs <- "yes-maybe"
            pkgname0 <- sub("\\.(tar\\.gz|tgz|tar\\.bz2|tar\\.xz)$", 
                "", pkgname0)
        ...
        if (!dir.exists(pkgoutdir)) {
            message(sprintf("ERROR: cannot create check dir %s", 
                sQuote(pkgoutdir)))
            do_exit(1L)
        }
        ...
        if (sp != 64) 
            messageLog(Log, "using platform: ", R.version$platform, 
                " (", sp, "-bit)")
        else messageLog(Log, "using platform: ", R.version$platform)
        vers <- R_compiled_by()
        if (any(nzchar(vers))) {
        ...
        if (is_tar) {
            ...
            if (utils::untar(pkg, exdir = dir, tar = Sys.getenv("R_INSTALL_TAR", 
                "internal"))) {
                checkingLog(Log, "whether tarball can be unpacked")
        ...
        if (!dir.exists(pkg)) {
            checkingLog(Log, "package directory")
            errorLog(Log, gettextf("package directory %s does not exist", 
                sQuote(pkg)))
            summaryLog(Log)
            do_exit(1L)
        }
        ...
        if (thispkg_src_subdirs == "yes-maybe") {
            if (WINDOWS) {
                if (file_test("-f", "configure")) 
                  thispkg_src_subdirs <- "no"
            }
            else {
        ...
        opts <- opts0
        if (!do_codoc) 
            opts <- c(opts, "--no-codoc")
        if (!do_examples && !spec_install) 
            opts <- c(opts, "--no-examples")
        if (!do_tests && !spec_install) 
            opts <- c(opts, "--no-tests")
        if (!do_manual && !spec_install) 
            opts <- c(opts, "--no-manual")
        if (ignore_vignettes) 
            opts <- c(opts, "--ignore-vignettes")
        else {
        ...
        if (use_gct) 
            opts <- c(opts, "--use-gct")
        if (use_valgrind) 
            opts <- c(opts, "--use-valgrind")
        if (!stop_on_test_error) 
            opts <- c(opts, "--no-stop-on-test-error")
        if (as_cran) 
            opts <- c(opts, "--as-cran")
        if (length(opts) > 1L) 
            messageLog(Log, "using options ", sQuote(paste(opts, 
                collapse = " ")))
        ...
        if (WINDOWS && grepl(" ", libdir)) 
            libdir <- gsub("\\", "/", utils::shortPathName(libdir), 
                fixed = TRUE)
        ...
        if (file.exists(f <- file.path(pkgdir, "DESCRIPTION.in")) && 
            ...
            desc <- desc[1L, ]
            if (identical(desc["Priority"], c(Priority = "base"))) {
                ...
                else messageLog(Log, "skipping installation test")
            }
        }
        ...
        if (!is_base_pkg) {
            ...
            check_incoming <- Sys.getenv("_R_CHECK_CRAN_INCOMING_", 
                "NA")
            check_incoming <- if (check_incoming == "NA") 
                as_cran
            else {
                config_val_to_logical(check_incoming)
            }
            check_incoming_remote <- Sys.getenv("_R_CHECK_CRAN_INCOMING_REMOTE_", 
                "NA")
            check_incoming_remote <- if (check_incoming_remote == 
                "NA") 
                as_cran
            else {
            ...
            if (check_incoming) 
                check_CRAN_incoming(!check_incoming_remote, pkg_size)
            if (do_install) {
            ...
            if ("NAMESPACE" %in% dir(pkgdir)) 
                ...
                errorLog(Log, "File NAMESPACE does not exist but there is a case-insenstiive match.")
                summaryLog(Log)
                do_exit(1L)
            }
            ...
            setwd(startdir)
            makevars <- Sys.glob(file.path(pkgdir, "src", c("Makevars.in", 
                "Makevars")))
            makevars <- basename(makevars)
            if (do_install) {
                check_install()
                if (R_check_pkg_sizes) 
                  check_install_sizes()
            }
            if (multiarch) {
                if (force_multiarch) 
                  ...
                  if (dir.exists(dd <- file.path(libdir, pkgname, 
                    "libs"))) {
                    inst_archs <- dir(dd)
                    inst_archs <- inst_archs[inst_archs %in% 
                      archs]
                    if (!identical(inst_archs, archs)) {
                      if (length(inst_archs) > 1) 
                        printLog0(Log, "NB: this package is only installed for sub-architectures ", 
                          paste(sQuote(inst_archs), collapse = ", "), 
                          "\n")
                      else {
                        printLog0(Log, "NB: this package is only installed for sub-architecture ", 
                          sQuote(inst_archs), "\n")
            ...
            if (R_check_suggests_only) 
                elibs <- setRlibs(pkgdir = pkgdir, libdir = libdir, 
                  suggests = TRUE)
            elibs_tests <- if (R_cdo_tests) {
                setRlibs(pkgdir = pkgdir, libdir = libdir, tests = TRUE)
            }
            else elibs
            if (R_cdo || R_cdo_examples || R_cdo_vignettes || 
                R_cdo_data) 
                elibs_cdo <- setRlibs(pkgdir = pkgdir, libdir = libdir)
        }
        else check_incoming <- FALSE
        setwd(startdir)
        check_pkg(pkgdir, pkgname, pkgoutdir, startdir, libdir, 
            desc, is_base_pkg, is_rec_pkg, thispkg_subdirs, extra_arch)
        if (!extra_arch && do_manual) {
            setwd(pkgoutdir)
            instdir <- file.path(libdir, pkgname)
            if (dir.exists(file.path(instdir, "help"))) 
                check_pkg_manual(instdir, desc["Package"])
            else check_pkg_manual(pkgdir, desc["Package"])
        }
        if (!extra_arch && do_manual && (isTRUE(R_check_Rd_validate_Rd2HTML) || 
            isTRUE(R_check_Rd_math_rendering))) {
            if (do_install) 
                check_Rd2HTML(file.path(if (is_base_pkg) 
                  .Library
                else libdir, pkgname), installed = TRUE)
            else check_Rd2HTML(pkgdir)
        }
        ...
        if (R_check_things_in_check_dir) {
            checkingLog(Log, "for non-standard things in the check directory")
            things <- setdiff(list.files(pkgoutdir, all.files = TRUE, 
                include.dirs = TRUE, no.. = TRUE), c("00check.log", 
                "00install.out", "00package.dcf", "00_pkg_src", 
                pkgname, sprintf("%s-Ex.%s", pkgname, c("R", 
                  "Rout", "pdf", "timings")), sprintf("%s-manual.%s", 
                  pkgname, c("log", "pdf")), "Rdlatex.log", "R_check_bin", 
                "build_vignettes.log", "tests", "vign_test", 
                if (this_multiarch) c(paste0("examples_", inst_archs), 
                  paste0(pkgname, "-Ex_", inst_archs, ".Rout"), 
                  paste0("tests_", inst_archs))))
            things <- things[!grepl("^Rplots.*[.]pdf$|[.]log$", 
                things)]
            if (length(things)) {
                noteLog(Log)
        ...
        if (R_check_things_in_others) {
            ...
            snap2 <- snapshot()
            ff <- character()
            for (i in seq_along(snap)) ff <- c(ff, setdiff(snap2[[i]], 
                snap[[i]]))
            if (length(ff)) {
                isdir <- isTRUE(file.info(ff)$isdir)
                ff[isdir] <- paste0(ff[isdir], "/")
                ff <- sub(paste0("^", normalizePath("~")), "~", 
                  ff)
