        ...
        "macros", "system.Rd")) 
{
    ...
    doPartialMarkup <- function(x, tags, i) {
        ...
        if (i < length(tags) && tags[i + 1L] == "TEXT" && length(x[[i]]) == 
            ...
            (tags[i] %notin% drop) && RdTags(x[[i]]) == "TEXT") {
            text1 <- x[[i]][[1L]]
            if (length(grep("[[:alpha:]]$", text1))) {
                text2 <- x[[i + 1L]]
                if (length(grep("^[[:alpha:]]", text2))) {
                  show(text1)
                  prevcol <<- prevcol + 1L
                  saveline <- prevline
    ...
    show <- function(x, extraSpace = FALSE) {
        ...
        switch(tag, KEEP = , TEXT = {
            ...
            if (keepSpacing) mycat(rep.int(" ", max(0, firstcol - 
                prevcol - 1L)), sep = "")
            x <- as.character(srcref)
            if (extraSpace) {
                if (grepl("[^\n]* ", x)) x <- sub(" ", "  ", 
                  x) else if (firstline == lastline) x <- paste0(x, 
                  " ")
            }
            mycat(x, sep = "")
            prevcol <<- lastcol
