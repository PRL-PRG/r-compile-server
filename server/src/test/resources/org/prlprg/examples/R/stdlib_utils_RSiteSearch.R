#? stdlib
`RSiteSearch` <- function (string, restrict = c("functions", "descriptions", "news", 
    "Rfunctions", "Rmanuals", "READMEs", "views", "vignettes"), 
    format, sortby = c("score", "date:late", "date:early", "subject", 
        "subject:descending", "size", "size:descending"), matchesPerPage = 20, 
    words = c("all", "any")) 
{
    matchingDB = list(functions = "cran-help", descriptions = "cran-info", 
        news = "cran-news", Rfunctions = "r-help", Rmanuals = "r-manuals", 
        READMEs = "cran-readme", views = "cran-views", vignettes = "cran-vignettes")
    string <- paste0("https://search.r-project.org/?FMT=query&P=", 
        gsub("%22", "\"", gsub("%20", "+", URLencode(gsub("[\\{\\}]", 
            "\"", trimws(string)), reserved = TRUE), fixed = TRUE), 
            fixed = TRUE))
    ...
    sortby <- paste0("SORT=", switch(sortby, score = "", `date:late` = "-0", 
        `date:early` = "%2B0", subject = "%2B3", `subject:descending` = "-3", 
        size = "%2B2", `size:descending` = "-2"))
    words <- match.arg(words)
    words <- paste0("DEFAULTOP=", switch(words, all = "and", 
        any = "or"))
