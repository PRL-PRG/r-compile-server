...
`demo` <- function (topic, package = NULL, lib.loc = NULL, character.only = FALSE, 
    verbose = getOption("verbose"), type = c("console", "html"), 
    echo = TRUE, ask = getOption("demo.ask"), encoding = getOption("encoding")) 
{
    type <- match.arg(type)
    html <- type == "html"
    if (html) {
    ...
    if (missing(topic)) {
        if (html) {
            browseURL(paste0("http://127.0.0.1:", port, "/doc/html/Search?package=", 
                paste(unique(basename(paths)), collapse = ";"), 
                "&agrep=0&types.demo=1&pattern="), browser)
            return(invisible())
        }
    ...
    if (html) {
        browseURL(paste0("http://127.0.0.1:", port, "/library/", 
            basename(pkgpath), "/Demo/", tools::file_path_sans_ext(basename(available))), 
            browser)
        return(invisible())
    }
