...
`make_sysdata_rda` <- function () 
{
    ...
    re <- "(.*) \\[(.*)\\]"
    ind <- grepl(re, dt)
    MARC_relator_db <- as.data.frame(cbind(term = sub(re, "\\1", 
        dt[ind]), code = sub(re, "\\2", dt[ind]), description = sub("\n\t UF.*", 
        "", dd[ind]), usage = ""), stringsAsFactors = FALSE)
    MARC_R_usage <- c(aut = "Use for full authors who have made substantial contributions to\nthe package and should show up in the package citation.", 
        ...
        cph = "Use for all copyright holders.", cre = "Use for the package maintainer.", 
    ...
    MARC_R_usage <- data.frame(code = names(MARC_R_usage), usage = c(MARC_R_usage), 
        stringsAsFactors = FALSE, row.names = NULL)
    pos <- match(MARC_relator_db$code, MARC_R_usage$code, nomatch = 0L)
    MARC_relator_db$usage[pos > 0L] <- gsub("\n", " ", MARC_R_usage$usage[pos], 
        fixed = TRUE)
    MARC_relator_db_codes_used_with_R <- sort(MARC_relator_db[nchar(MARC_relator_db$usage) > 
        0L, "code"])
    save(MARC_relator_db, MARC_relator_db_codes_used_with_R, 
