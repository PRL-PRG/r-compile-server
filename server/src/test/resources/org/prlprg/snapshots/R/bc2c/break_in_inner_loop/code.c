#include <runtime.h>
SEXP f_n1496356832_0(SEXP RHO, SEXP CCP);
SEXP f_n1496356832_0(SEXP RHO, SEXP CCP) {
  int STACK_SAVE = 8;
  CHECK_OVERFLOW(8);
  // CELLS
  DEFINE_BCELL2(C2); // symbol: 'x' (used: 5)
  DEFINE_BCELL2(C6); // symbol: 'i' (used: 2)
  DEFINE_BCELL2(C8); // symbol: 'j' (used: 3)
  // REGISTERS
  DEFINE_VAL(R1);
  DEFINE_VAL(R2);
  DEFINE_VAL(R3);
...
goto L7;
L7: /* stack: 6 */
  if (Rsh_StartLoopCntxt(&LOOP_CTX[0], RHO) /* stack: 6 */) {
	goto L23;
}
  goto L22;
L9: /* stack: 6 */
  ...
  Rsh_Pop(*R7) /* stack: 6 */;
  Rsh_GetVar(R7, Rsh_const(CCP, 6), &C8, RHO) /* symbol: 'j', stack: 7 */;
...
L23: /* stack: 6 */
  Rsh_EndLoopCntxt(&LOOP_CTX[0]) /* stack: 6 */;
