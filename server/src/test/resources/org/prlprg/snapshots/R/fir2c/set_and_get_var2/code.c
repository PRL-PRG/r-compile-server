...
D0:;
  ...
  Rsh_Fir_array_deopt_stack[0] = Rsh_Fir_reg_y;
  Rsh_Fir_deopt(3, 1, Rsh_Fir_array_deopt_stack, CCP, RHO);
  return R_NilValue;

L0:;
  // y1 = force? y
  SEXP Rsh_Fir_reg_y1 = Rsh_Fir_maybe_force(Rsh_Fir_reg_y);
  // checkMissing(y1)
  SEXP Rsh_Fir_array_args[1];
  Rsh_Fir_array_args[0] = Rsh_Fir_reg_y1;
  (void)(Rsh_Fir_intrinsic_checkMissing(CCP, RHO, 1, Rsh_Fir_array_args, Rsh_Fir_param_types_empty()));
  // st x = y1
  Rsh_Fir_store(Rsh_const(CCP, 2), Rsh_Fir_reg_y1, RHO);
  (void)(Rsh_Fir_reg_y1);
  // x = ld x
