...
SEXP Rsh_Fir_user_function_from_R_inner2650636851_(SEXP CCP, SEXP RHO, SEXP PARAMS_LIST);
...
SEXP Rsh_Fir_user_promise_inner2650636851_(SEXP CCP, SEXP RHO, int NCAPTURES, SEXP const **CAPTURES);
SEXP Rsh_Fir_user_promise_inner2650636851_1(SEXP CCP, SEXP RHO, int NCAPTURES, SEXP const **CAPTURES);
SEXP Rsh_Fir_user_function_from_R_main(SEXP CCP, SEXP RHO, SEXP PARAMS_LIST) {
...
SEXP Rsh_Fir_user_function_from_R_inner2650636851_(SEXP CCP, SEXP RHO, SEXP PARAMS_LIST) {
  // FIR inner2650636851 dynamic dispatch from R ([x, name])
  if (TYPEOF(PARAMS_LIST) != VECSXP) { Rf_PrintValue(PARAMS_LIST); Rsh_error("FIŘ expected a list for params"); }
  int NPARAMS = Rf_length(PARAMS_LIST);
  SEXP const *PARAMS = STDVEC_DATAPTR(PARAMS_LIST);
  return Rsh_Fir_user_function_inner2650636851_(CCP, RHO, NPARAMS, PARAMS, NULL);
}
SEXP Rsh_Fir_user_function_inner2650636851_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES) {
...
SEXP Rsh_Fir_user_version_inner2650636851_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  ...
  SEXP Rsh_Fir_reg_getNativeSymbolInfo;
  SEXP Rsh_Fir_reg_p;
  ...
  // getNativeSymbolInfo = ldf getNativeSymbolInfo
  Rsh_Fir_reg_getNativeSymbolInfo = Rsh_Fir_load_fun(Rsh_Fir_LoadFun_Local, Rsh_const(CCP, 3), RHO);
  // check L0() else D0()
  // L0()
  goto L0_;
...
L0_:;
  ...
  //   r2 = dyn as_character(name2);
  //   goto L0(r2);
  // L2():
  //   r = dyn base(<sym name>);
  //   goto L0(r);
  // }
  Rsh_Fir_reg_p = Rsh_Fir_make_promise(&Rsh_Fir_user_promise_inner2650636851_, 0, NULL, CCP, RHO);
  // p1 = prom<V +>{
  //   x1 = ld x;
  //   x2 = force? x1;
  //   checkMissing(x2);
  //   return x2;
  // }
  Rsh_Fir_reg_p1_ = Rsh_Fir_make_promise(&Rsh_Fir_user_promise_inner2650636851_1, 0, NULL, CCP, RHO);
  // r5 = dyn getNativeSymbolInfo[, PACKAGE](p, p1)
  ...
  Rsh_Fir_reg_r5_ = Rsh_Fir_call_dynamic(Rsh_Fir_reg_getNativeSymbolInfo, 2, Rsh_Fir_array_args5, Rsh_Fir_array_arg_names2, RHO);
  // check L1() else D1()
  // L1()
  goto L1_;
...
SEXP Rsh_Fir_user_promise_inner2650636851_(SEXP CCP, SEXP RHO, int NCAPTURES, SEXP const **CAPTURES) {
  ...
  if (NCAPTURES != 0) Rsh_error("FIŘ capture arity mismatch for inner2650636851/0: expected 0, got %d", NCAPTURES);
...
L1_:;
  ...
  Rsh_Fir_reg_r2_ = Rsh_Fir_call_dynamic(Rsh_Fir_reg_as_character, 1, Rsh_Fir_array_args2, Rsh_Fir_array_arg_names, RHO);
  // goto L0(r2)
  // L0(r2)
