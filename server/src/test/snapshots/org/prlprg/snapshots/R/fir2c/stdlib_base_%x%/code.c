...
SEXP Rsh_Fir_user_function_from_R_inner1261373293_(SEXP CCP, SEXP RHO, SEXP PARAMS_LIST);
...
SEXP Rsh_Fir_user_function_from_R_inner1261373293_(SEXP CCP, SEXP RHO, SEXP PARAMS_LIST) {
  // FIR inner1261373293 dynamic dispatch from R ([X, Y])
  if (TYPEOF(PARAMS_LIST) != VECSXP) { Rf_PrintValue(PARAMS_LIST); Rsh_error("FIŘ expected a list for params"); }
  int NPARAMS = Rf_length(PARAMS_LIST);
  SEXP const *PARAMS = STDVEC_DATAPTR(PARAMS_LIST);
  return Rsh_Fir_user_function_inner1261373293_(CCP, RHO, NPARAMS, PARAMS, NULL);
}
SEXP Rsh_Fir_user_function_inner1261373293_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES) {
...
L0_:;
  ...
  Rsh_Fir_reg_r2_ = Rsh_Fir_call_dynamic(Rsh_Fir_reg_kronecker, 2, Rsh_Fir_array_args2, Rsh_Fir_array_arg_names, RHO);
  // check L1() else D1()
  // L1()
  goto L1_;
