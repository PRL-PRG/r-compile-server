...
SEXP Rsh_Fir_user_function_from_R_inner2275580337_(SEXP CCP, SEXP RHO, SEXP PARAMS_LIST);
...
SEXP Rsh_Fir_user_promise_inner2275580337_(SEXP CCP, SEXP RHO, int NCAPTURES, SEXP const **CAPTURES);
SEXP Rsh_Fir_user_function_from_R_main(SEXP CCP, SEXP RHO, SEXP PARAMS_LIST) {
...
SEXP Rsh_Fir_user_function_from_R_inner2275580337_(SEXP CCP, SEXP RHO, SEXP PARAMS_LIST) {
  // FIR inner2275580337 dynamic dispatch from R ([xx, tz, cl])
  if (TYPEOF(PARAMS_LIST) != VECSXP) { Rf_PrintValue(PARAMS_LIST); Rsh_error("FIŘ expected a list for params"); }
  int NPARAMS = Rf_length(PARAMS_LIST);
  SEXP const *PARAMS = STDVEC_DATAPTR(PARAMS_LIST);
  return Rsh_Fir_user_function_inner2275580337_(CCP, RHO, NPARAMS, PARAMS, NULL);
}
SEXP Rsh_Fir_user_function_inner2275580337_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES) {
...
L1_:;
  ...
  //   goto L0(r);
  // }
  Rsh_Fir_reg_p = Rsh_Fir_make_promise(&Rsh_Fir_user_promise_inner2275580337_, 0, NULL, CCP, RHO);
  // goto L2(p)
  // L2(p)
...
SEXP Rsh_Fir_user_promise_inner2275580337_(SEXP CCP, SEXP RHO, int NCAPTURES, SEXP const **CAPTURES) {
  ...
  if (NCAPTURES != 0) Rsh_error("FIŘ capture arity mismatch for inner2275580337/0: expected 0, got %d", NCAPTURES);
