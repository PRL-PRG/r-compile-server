...
SEXP Rsh_Fir_user_function_from_R_inner3183788040_(SEXP CCP, SEXP RHO, SEXP PARAMS_LIST);
SEXP Rsh_Fir_user_function_inner3183788040_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES);
SEXP Rsh_Fir_user_version_inner3183788040_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS);
SEXP Rsh_Fir_user_promise_inner3183788040_(SEXP CCP, SEXP RHO, int NCAPTURES, SEXP const **CAPTURES);
SEXP Rsh_Fir_user_function_from_R_main(SEXP CCP, SEXP RHO, SEXP PARAMS_LIST) {
...
SEXP Rsh_Fir_user_version_main_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  ...
  (void)(R_NilValue);
  // r = clos inner3183788040
  Rsh_Fir_reg_r = Rsh_Fir_make_closure(&Rsh_Fir_user_function_inner3183788040_, RHO, CCP);
  // st `.POSIXlt` = r
...
SEXP Rsh_Fir_user_function_from_R_inner3183788040_(SEXP CCP, SEXP RHO, SEXP PARAMS_LIST) {
  // FIR inner3183788040 dynamic dispatch from R ([xx, tz, cl])
  if (TYPEOF(PARAMS_LIST) != VECSXP) { Rf_PrintValue(PARAMS_LIST); Rsh_error("FIŘ expected a list for params"); }
  int NPARAMS = Rf_length(PARAMS_LIST);
  SEXP const *PARAMS = STDVEC_DATAPTR(PARAMS_LIST);
  return Rsh_Fir_user_function_inner3183788040_(CCP, RHO, NPARAMS, PARAMS, NULL);
}
SEXP Rsh_Fir_user_function_inner3183788040_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES) {
  // FIR inner3183788040 dynamic dispatch ([xx, tz, cl])

  return Rsh_Fir_user_version_inner3183788040_v0_(CCP, RHO, NPARAMS, PARAMS);
}
SEXP Rsh_Fir_user_version_inner3183788040_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  // FIR inner3183788040 version 0 (*, *, * -+> V)

  if (NPARAMS != 3) Rsh_error("FIŘ arity mismatch for inner3183788040/0: expected 3, got %d", NPARAMS);
  ...
  SEXP Rsh_Fir_reg_cl_isMissing;
  SEXP Rsh_Fir_reg_cl_orDefault;
  SEXP Rsh_Fir_reg_p;
...
L1_:;
  ...
  // L1():
  //   c = ldf c in base;
  //   r2 = dyn c("POSIXlt", "POSIXt");
  //   goto L0(r2);
  // L2():
  //   r = dyn base("POSIXlt", "POSIXt");
  //   goto L0(r);
  // }
  Rsh_Fir_reg_p = Rsh_Fir_make_promise(&Rsh_Fir_user_promise_inner3183788040_, 0, NULL, CCP, RHO);
  // goto L2(p)
  // L2(p)
...
L4_:;
  ...
  Rsh_Fir_reg_r4_ = Rsh_Fir_call_dynamic(Rsh_Fir_reg_class__, 3, Rsh_Fir_array_args6, Rsh_Fir_array_arg_names2, RHO);
  // check L5() else D2()
  // L5()
  goto L5_;
...
L7_:;
  ...
  Rsh_Fir_reg_r5_ = Rsh_Fir_call_dynamic(Rsh_Fir_reg_attr__, 4, Rsh_Fir_array_args8, Rsh_Fir_array_arg_names3, RHO);
  // check L8() else D5()
  // L8()
  goto L8_;
...
SEXP Rsh_Fir_user_promise_inner3183788040_(SEXP CCP, SEXP RHO, int NCAPTURES, SEXP const **CAPTURES) {
  ...
  if (NCAPTURES != 0) Rsh_error("FIŘ capture arity mismatch for inner3183788040/0: expected 0, got %d", NCAPTURES);
...
L1_:;
  ...
  Rsh_Fir_reg_r2_ = Rsh_Fir_call_dynamic(Rsh_Fir_reg_c, 2, Rsh_Fir_array_args3, Rsh_Fir_array_arg_names, RHO);
  // goto L0(r2)
  // L0(r2)
...
L2_:;
  ...
  Rsh_Fir_reg_r1 = Rsh_Fir_call_dynamic(Rsh_Fir_reg_base, 2, Rsh_Fir_array_args4, Rsh_Fir_array_arg_names1, RHO);
  // goto L0(r)
  // L0(r)
