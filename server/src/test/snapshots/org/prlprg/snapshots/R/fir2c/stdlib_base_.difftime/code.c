...
SEXP Rsh_Fir_user_function_from_R_inner723830835_(SEXP CCP, SEXP RHO, SEXP PARAMS_LIST);
...
SEXP Rsh_Fir_user_function_from_R_inner723830835_(SEXP CCP, SEXP RHO, SEXP PARAMS_LIST) {
  // FIR inner723830835 dynamic dispatch from R ([xx, units, cl])
  if (TYPEOF(PARAMS_LIST) != VECSXP) { Rf_PrintValue(PARAMS_LIST); Rsh_error("FIŘ expected a list for params"); }
  int NPARAMS = Rf_length(PARAMS_LIST);
  SEXP const *PARAMS = STDVEC_DATAPTR(PARAMS_LIST);
  return Rsh_Fir_user_function_inner723830835_(CCP, RHO, NPARAMS, PARAMS, NULL);
}
SEXP Rsh_Fir_user_function_inner723830835_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES) {
...
L2_:;
  ...
  Rsh_Fir_reg_r1 = Rsh_Fir_call_dynamic(Rsh_Fir_reg_class__, 3, Rsh_Fir_array_args2, Rsh_Fir_array_arg_names, RHO);
  // check L3() else D2()
  // L3()
  goto L3_;
...
L5_:;
  ...
  Rsh_Fir_reg_r1_ = Rsh_Fir_call_dynamic(Rsh_Fir_reg_attr__, 4, Rsh_Fir_array_args4, Rsh_Fir_array_arg_names1, RHO);
  // check L6() else D5()
  // L6()
  goto L6_;
