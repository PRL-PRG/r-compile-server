...
SEXP Rsh_Fir_user_version_inner1179177493_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  ...
  SEXP Rsh_Fir_reg_besselI;  // besselI
...
L2_:;
  ...
  // besselI = ldf besselI in base
  Rsh_Fir_reg_besselI = Rsh_Fir_load_fun(Rsh_Fir_LoadFun_Base, Rsh_const(CCP, 0), RHO);
  // r9 = dyn besselI(x2, nu2, r8)
  ...
  Rsh_Fir_reg_r9_ = Rsh_Fir_call_dynamic(Rsh_Fir_reg_besselI, 3, Rsh_Fir_array_args3, Rsh_Fir_array_arg_names, CCP, RHO);
  // check L11() else D4()
  // L11()
  goto L11_;
...
L4_:;
  // r = dyn base(<lang besselI(x, nu, `+`(1.0, as.logical(expon.scaled)))>)
