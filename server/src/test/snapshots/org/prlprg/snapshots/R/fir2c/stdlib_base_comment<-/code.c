...
SEXP Rsh_Fir_user_version_inner736444261_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  ...
  SEXP Rsh_Fir_reg_comment__;  // comment__
...
L2_:;
  // r = dyn base(<lang `comment<-`(x, value)>)
...
L4_:;
  ...
  // comment__ = ldf `comment<-` in base
  Rsh_Fir_reg_comment__ = Rsh_Fir_load_fun(Rsh_Fir_LoadFun_Base, Rsh_const(CCP, 0), RHO);
  // r2 = dyn comment__(x2, value2)
  ...
  Rsh_Fir_reg_r2_ = Rsh_Fir_call_dynamic(Rsh_Fir_reg_comment__, 2, Rsh_Fir_array_args4, Rsh_Fir_array_arg_names1, CCP, RHO);
  // check L5() else D2()
  // L5()
  goto L5_;
