...
SEXP Rsh_Fir_user_version_inner2708337952_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  ...
  SEXP Rsh_Fir_reg_memory_profile;  // memory_profile
...
L1_:;
  // memory_profile = ldf `memory.profile` in base
  Rsh_Fir_reg_memory_profile = Rsh_Fir_load_fun(Rsh_Fir_LoadFun_Base, Rsh_const(CCP, 0), RHO);
  // r2 = dyn memory_profile()
  Rsh_Fir_reg_r2_ = Rsh_Fir_call_dynamic(Rsh_Fir_reg_memory_profile, 0, NULL, NULL, CCP, RHO);
  // check L3() else D0()
  // L3()
  goto L3_;

L2_:;
  // r = dyn base(<lang memory.profile()>)
