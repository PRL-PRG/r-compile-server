...
SEXP Rsh_Fir_user_version_inner2708337952_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  ...
  SEXP Rsh_Fir_reg_sys_on_exit;  // sys_on_exit
...
L1_:;
  // sys_on_exit = ldf `sys.on.exit` in base
  Rsh_Fir_reg_sys_on_exit = Rsh_Fir_load_fun(Rsh_Fir_LoadFun_Base, Rsh_const(CCP, 0), RHO);
  // r2 = dyn sys_on_exit()
  Rsh_Fir_reg_r2_ = Rsh_Fir_call_dynamic(Rsh_Fir_reg_sys_on_exit, 0, NULL, NULL, CCP, RHO);
  // check L3() else D0()
  // L3()
  goto L3_;

L2_:;
  // r = dyn base(<lang sys.on.exit()>)
