...
SEXP Rsh_Fir_user_function_inner2789031763_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES);
SEXP Rsh_Fir_user_version_inner2789031763_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS);
SEXP Rsh_Fir_user_function_main(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES) {
...
SEXP Rsh_Fir_user_version_main_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  ...
  (void)(R_NilValue);
  // r = clos inner2789031763
  Rsh_Fir_reg_r = Rsh_Fir_make_closure(&Rsh_Fir_user_function_inner2789031763_, RHO, CCP);
  // st qgeom = r
...
SEXP Rsh_Fir_user_function_inner2789031763_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES) {
  // FIR inner2789031763 dynamic dispatch ([p, prob, `lower.tail`, `log.p`])

  return Rsh_Fir_user_version_inner2789031763_v0_(CCP, RHO, NPARAMS, PARAMS);
}
SEXP Rsh_Fir_user_version_inner2789031763_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  // FIR inner2789031763 version 0 (*, *, *, * -+> V)

  if (NPARAMS != 4) Rsh_error("FIŘ arity mismatch for inner2789031763/0: expected 4, got %d", NPARAMS);
  ...
  SEXP Rsh_Fir_reg_C_qgeom;  // C_qgeom
  SEXP Rsh_Fir_reg_C_qgeom1_;  // C_qgeom1
...
L3_:;
  // C_qgeom = ld C_qgeom
  Rsh_Fir_reg_C_qgeom = Rsh_Fir_load(Rsh_const(CCP, 8), RHO);
  // check L5() else D0()
  // L5()
  goto L5_;

L4_:;
  // r = dyn base(<sym C_qgeom>, <sym p>, <sym prob>, <sym lower.tail>, <sym log.p>)
...
D0_:;
  ...
  Rsh_Fir_array_deopt_stack[0] = Rsh_Fir_reg_C_qgeom;
  Rsh_Fir_deopt(1, 1, Rsh_Fir_array_deopt_stack, CCP, RHO);
  return R_NilValue;

L5_:;
  // C_qgeom1 = force? C_qgeom
  Rsh_Fir_reg_C_qgeom1_ = Rsh_Fir_maybe_force(Rsh_Fir_reg_C_qgeom);
  // checkMissing(C_qgeom1)
  SEXP Rsh_Fir_array_args4[1];
  Rsh_Fir_array_args4[0] = Rsh_Fir_reg_C_qgeom1_;
...
D1_:;
  ...
  Rsh_Fir_array_deopt_stack1[0] = Rsh_Fir_reg_C_qgeom1_;
...
D2_:;
  // deopt 3 [C_qgeom1, p2, prob1]
  SEXP Rsh_Fir_array_deopt_stack2[3];
  Rsh_Fir_array_deopt_stack2[0] = Rsh_Fir_reg_C_qgeom1_;
...
D3_:;
  // deopt 4 [C_qgeom1, p2, prob2, lower_tail1]
  SEXP Rsh_Fir_array_deopt_stack3[4];
  Rsh_Fir_array_deopt_stack3[0] = Rsh_Fir_reg_C_qgeom1_;
...
D4_:;
  // deopt 5 [C_qgeom1, p2, prob2, lower_tail2, log_p1]
  SEXP Rsh_Fir_array_deopt_stack4[5];
  Rsh_Fir_array_deopt_stack4[0] = Rsh_Fir_reg_C_qgeom1_;
...
L9_:;
  ...
  // vargs = dots[p2, prob2, lower_tail2, log_p2]
  ...
  // r2 = `.Call`(C_qgeom1, vargs, <missing>)
  SEXP Rsh_Fir_array_args9[3];
  Rsh_Fir_array_args9[0] = Rsh_Fir_reg_C_qgeom1_;
