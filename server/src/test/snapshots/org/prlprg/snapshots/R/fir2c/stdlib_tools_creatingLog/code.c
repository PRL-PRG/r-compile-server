...
SEXP Rsh_Fir_user_function_inner3782021761_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES);
SEXP Rsh_Fir_user_version_inner3782021761_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS);
SEXP Rsh_Fir_user_promise_inner3782021761_(SEXP CCP, SEXP RHO);
SEXP Rsh_Fir_user_promise_inner3782021761_1(SEXP CCP, SEXP RHO);
SEXP Rsh_Fir_user_promise_inner3782021761_2(SEXP CCP, SEXP RHO);
SEXP Rsh_Fir_user_function_main(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES) {
...
SEXP Rsh_Fir_user_version_main_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  ...
  (void)(R_NilValue);
  // r = clos inner3782021761
  Rsh_Fir_reg_r = Rsh_Fir_make_closure(&Rsh_Fir_user_function_inner3782021761_, RHO, CCP);
  // st creatingLog = r
...
SEXP Rsh_Fir_user_function_inner3782021761_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES) {
  // FIR inner3782021761 dynamic dispatch ([Log, text])

  return Rsh_Fir_user_version_inner3782021761_v0_(CCP, RHO, NPARAMS, PARAMS);
}
SEXP Rsh_Fir_user_version_inner3782021761_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  // FIR inner3782021761 version 0 (*, * -+> V)

  if (NPARAMS != 2) Rsh_error("FIŘ arity mismatch for inner3782021761/0: expected 2, got %d", NPARAMS);
  ...
  // st text = text
  Rsh_Fir_store(Rsh_const(CCP, 2), Rsh_Fir_reg_text, RHO);
...
L0_:;
  ...
  //   return Log2;
  // }
  Rsh_Fir_reg_p = Rsh_Fir_make_promise(&Rsh_Fir_user_promise_inner3782021761_, CCP, RHO);
  // p1 = prom<V +>{
  //   Log3 = ld Log;
  //   Log4 = force? Log3;
  //   checkMissing(Log4);
  ...
  //   goto L0(dx);
  // }
  Rsh_Fir_reg_p1_ = Rsh_Fir_make_promise(&Rsh_Fir_user_promise_inner3782021761_1, CCP, RHO);
  // p2 = prom<V +>{
  //   text1 = ld text;
  //   text2 = force? text1;
  //   checkMissing(text2);
  //   return text2;
  // }
  Rsh_Fir_reg_p2_ = Rsh_Fir_make_promise(&Rsh_Fir_user_promise_inner3782021761_2, CCP, RHO);
  // r4 = dyn printLog(p, p1, " creating ", p2, " ...")
  ...
  Rsh_Fir_reg_r4_ = Rsh_Fir_call_dynamic(Rsh_Fir_reg_printLog, 5, Rsh_Fir_array_args8, Rsh_Fir_array_arg_names, CCP, RHO);
  // check L1() else D1()
  // L1()
  goto L1_;
...
SEXP Rsh_Fir_user_promise_inner3782021761_2(SEXP CCP, SEXP RHO) {
  // text1 = ld text
  Rsh_Fir_reg_text1_ = Rsh_Fir_load(Rsh_const(CCP, 2), RHO);
  // text2 = force? text1
