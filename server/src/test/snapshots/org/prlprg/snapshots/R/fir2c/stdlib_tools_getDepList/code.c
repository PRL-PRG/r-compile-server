...
SEXP Rsh_Fir_user_function_inner3828731956_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES);
SEXP Rsh_Fir_user_version_inner3828731956_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS);
SEXP Rsh_Fir_user_function_main(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES) {
...
SEXP Rsh_Fir_user_version_main_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  ...
  Rsh_Fir_reg_r = Rsh_Fir_make_closure(&Rsh_Fir_user_function_inner3828731956_, RHO, CCP);
  // st getDepList = r
...
SEXP Rsh_Fir_user_function_inner3828731956_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES) {
  // FIR inner3828731956 dynamic dispatch ([depMtrx, instPkgs, recursive, local, reduce, `lib.loc`])

  return Rsh_Fir_user_version_inner3828731956_v0_(CCP, RHO, NPARAMS, PARAMS);
}
SEXP Rsh_Fir_user_version_inner3828731956_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  // FIR inner3828731956 version 0 (*, *, *, *, *, * -+> V)

  if (NPARAMS != 6) Rsh_error("FIŘ arity mismatch for inner3828731956/0: expected 6, got %d", NPARAMS);

  // Local declarations
  SEXP Rsh_Fir_reg_depMtrx;  // depMtrx
  SEXP Rsh_Fir_reg_instPkgs;  // instPkgs
  ...
  // Bind parameters
  Rsh_Fir_reg_depMtrx = PARAMS[0];
  Rsh_Fir_reg_instPkgs = PARAMS[1];
  Rsh_Fir_reg_recursive = PARAMS[2];
  Rsh_Fir_reg_local = PARAMS[3];
  Rsh_Fir_reg_reduce = PARAMS[4];
  Rsh_Fir_reg_lib_loc = PARAMS[5];

  // mkenv
  Rsh_Fir_push_env(&RHO);
  (void)(R_NilValue);
  // st depMtrx = depMtrx
  Rsh_Fir_store(Rsh_const(CCP, 1), Rsh_Fir_reg_depMtrx, RHO);
  (void)(Rsh_Fir_reg_depMtrx);
  // st instPkgs = instPkgs
  Rsh_Fir_store(Rsh_const(CCP, 2), Rsh_Fir_reg_instPkgs, RHO);
  (void)(Rsh_Fir_reg_instPkgs);
...
L0_:;
  ...
  Rsh_Fir_store(Rsh_const(CCP, 4), Rsh_Fir_reg_recursive_orDefault, RHO);
  ...
  // L1(TRUE)
    Rsh_Fir_reg_local_orDefault = Rsh_const(CCP, 3);
    goto L1_;
  } else {
  // L1(local)
...
L1_:;
  ...
  // L2(TRUE)
    Rsh_Fir_reg_reduce_orDefault = Rsh_const(CCP, 3);
    goto L2_;
  } else {
  // L2(reduce)
...
L2_:;
  ...
  Rsh_Fir_store(Rsh_const(CCP, 6), Rsh_Fir_reg_reduce_orDefault, RHO);
  ...
  // L3(NULL)
    Rsh_Fir_reg_lib_loc_orDefault = Rsh_const(CCP, 7);
    goto L3_;
  } else {
  // L3(lib_loc)
...
L3_:;
  ...
  Rsh_Fir_store(Rsh_const(CCP, 8), Rsh_Fir_reg_lib_loc_orDefault, RHO);
