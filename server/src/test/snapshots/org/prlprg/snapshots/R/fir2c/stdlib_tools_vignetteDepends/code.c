...
SEXP Rsh_Fir_user_function_inner1283678011_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES);
SEXP Rsh_Fir_user_version_inner1283678011_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS);
SEXP Rsh_Fir_user_function_main(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES) {
...
SEXP Rsh_Fir_user_version_main_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  ...
  (void)(R_NilValue);
  // r = clos inner1283678011
  Rsh_Fir_reg_r = Rsh_Fir_make_closure(&Rsh_Fir_user_function_inner1283678011_, RHO, CCP);
  // st vignetteDepends = r
...
SEXP Rsh_Fir_user_function_inner1283678011_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES) {
  // FIR inner1283678011 dynamic dispatch ([vignette, recursive, reduce, local, `lib.loc`])

  return Rsh_Fir_user_version_inner1283678011_v0_(CCP, RHO, NPARAMS, PARAMS);
}
SEXP Rsh_Fir_user_version_inner1283678011_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  // FIR inner1283678011 version 0 (*, *, *, *, * -+> V)

  if (NPARAMS != 5) Rsh_error("FIŘ arity mismatch for inner1283678011/0: expected 5, got %d", NPARAMS);

  // Local declarations
  SEXP Rsh_Fir_reg_vignette;  // vignette
  ...
  // Bind parameters
  Rsh_Fir_reg_vignette = PARAMS[0];
  Rsh_Fir_reg_recursive = PARAMS[1];
  Rsh_Fir_reg_reduce = PARAMS[2];
  ...
  Rsh_Fir_push_env(&RHO);
  (void)(R_NilValue);
  // st vignette = vignette
  Rsh_Fir_store(Rsh_const(CCP, 1), Rsh_Fir_reg_vignette, RHO);
  (void)(Rsh_Fir_reg_vignette);
...
L0_:;
  ...
  Rsh_Fir_array_args1[0] = Rsh_Fir_reg_reduce;
  Rsh_Fir_reg_reduce_isMissing = Rsh_Fir_builtin_missing_v0(CCP, RHO, 1, Rsh_Fir_array_args1);
  // if reduce_isMissing then L1(TRUE) else L1(reduce)
...
L1_:;
  ...
  Rsh_Fir_store(Rsh_const(CCP, 4), Rsh_Fir_reg_reduce_orDefault, RHO);
  (void)(Rsh_Fir_reg_reduce_orDefault);
  // local_isMissing = missing.0(local)
  SEXP Rsh_Fir_array_args2[1];
  Rsh_Fir_array_args2[0] = Rsh_Fir_reg_local;
  Rsh_Fir_reg_local_isMissing = Rsh_Fir_builtin_missing_v0(CCP, RHO, 1, Rsh_Fir_array_args2);
  // if local_isMissing then L2(TRUE) else L2(local)
...
L4_:;
  // r = dyn _Defunct("vignetteInfo()$depends or package_dependencies()")
