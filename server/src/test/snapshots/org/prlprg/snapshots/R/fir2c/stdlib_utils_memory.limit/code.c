...
SEXP Rsh_Fir_user_function_inner3513620029_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES);
SEXP Rsh_Fir_user_version_inner3513620029_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS);
SEXP Rsh_Fir_user_function_main(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES) {
...
SEXP Rsh_Fir_user_version_main_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  ...
  (void)(R_NilValue);
  // r = clos inner3513620029
  Rsh_Fir_reg_r = Rsh_Fir_make_closure(&Rsh_Fir_user_function_inner3513620029_, RHO, CCP);
  // st `memory.limit` = r
...
SEXP Rsh_Fir_user_function_inner3513620029_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS, Rsh_Fir_Type const *PARAM_TYPES) {
  // FIR inner3513620029 dynamic dispatch ([size])

  return Rsh_Fir_user_version_inner3513620029_v0_(CCP, RHO, NPARAMS, PARAMS);
}
SEXP Rsh_Fir_user_version_inner3513620029_v0_(SEXP CCP, SEXP RHO, int NPARAMS, SEXP const *PARAMS) {
  // FIR inner3513620029 version 0 (* -+> V)

  if (NPARAMS != 1) Rsh_error("FIŘ arity mismatch for inner3513620029/0: expected 1, got %d", NPARAMS);
  ...
  // if size_isMissing then L0(NA_LGL) else L0(size)
  if (Rsh_Fir_is_true(Rsh_Fir_reg_size_isMissing)) {
  // L0(NA_LGL)
    Rsh_Fir_reg_size_orDefault = Rsh_const(CCP, 1);
    goto L0_;
  } else {
  // L0(size)
...
L1_:;
  // r = dyn warning[, `call.`]("'memory.limit()' is Windows-specific", FALSE)
