...
public final class BcLabel {
     ...
     *
     * @implNote This contains a map of positions in GNU-R bytecode to positions in our bytecode.
     *           We need this because every index in our bytecode maps to an instruction,
     *           while indexes in GNU-R's bytecode also map to the bytecode version and instruction metadata. */
    static class Factory {
        ...
        BcLabel make(int gnurLabel) {
            ...
            if (target == -1) {
                ...
                throw new IllegalArgumentException("GNU-R position maps to the middle of one of our instructions: " + gnurLabel + " between " + earlier + " and " + later);
            }
            return new BcLabel(target);
        }

        /** Create an object which creates labels from GNU-R labels,
         * by building the map of positions in GNU-R bytecode to positions in our bytecode
         * (see {@link Factory} implNote). */
        static class Builder {
            ...
            @CanIgnoreReturnValue Builder step(int sourceOffset, @SuppressWarnings("SameParameterValue") int targetOffset) {
