...
public class CFGCompiler {
   ...
   * control-flow-graph}.
   */
  static void compileCFG(
      Bc bc, CFG cfg, boolean isPromise, Node<? extends EnvSXP> enclos, Module module) {
   ...
   */
  private record ComplexAssign(boolean isSuper, RegSymSXP name, Node<? extends SEXP> originalRhs) {}
  ...
  private static class Call {
    ...
    sealed interface Fun {
      record Value(Node<? extends SEXP> fun) implements Fun {}

      record Builtin(BuiltinId id) implements Fun {}
    }

    /**
     * Stores information about a call argument: a {@link Node<? extends SEXP>} node and optional
     * name.
     */
    private record Arg(Node<? extends SEXP> value, @Nullable String name) {
      static Arg missing() {
        return new Arg(Constant.of(SEXPs.MISSING_ARG), null);
      }

      Arg(Node<? extends SEXP> value) {
        this(value, null);
      }
    }
  ...
  private final Node<? extends SEXP> env;
   ...
   */
  private CFGCompiler(
      Bc bc, CFG cfg, boolean isPromise, Node<? extends SEXP> enclos, Module module) {
    // TODO
    ...
    //
    //    if (isPromise) {
    //      // If this is a promise, it re-uses ENCLOS (assignments in a promise assign the variable
    // after
    //      // the promise is evaluated)
    //      env = enclos;
    //    } else {
    //      // If this is a closure, it has its own environment (assignments in a closure don't
    // rebind
    //      // outside the closure, unless they are super-assignments).
    //      env = cursor.insert("inclos", new MkEnv(enclos));
    //    }
    //
    //    // Insert the "real" entry block, where the instructions from the closure's bytecode
    // start.
    //    // The previous block had implicit instructions added by default to every closure (though
    // they
    //    // may be elided later).
  ...
  void doCompile() {
    ...
    //      // We will add more BBs when compiling some instructions, but IR in those BBs will be
    // specific
    //      // to the instruction, whereas labels in instructions may jump to within the IR of
    // previously-
    //      // compiled instructions. To insert a BB later that jumps to previously-compiled IR:
    // it's
    //      // possible, but adds complexity, because we have to remember which bytecode positions
    //      // correspond to which IR positions at these positions, and move already-compiled IR
    // into the
    //      // new BB. It's easier to insert these BBs first, then we don't have to move IR and we
    // only
    //      // need
    ...
    //            // We need to insert a jump because in the bytecode it just falls through, but
    // there's
    ...
    //            // This happens after a compiled `repeat`, so the following bytecode is
    // unreachable and
    //            // we can even remove `nextBb` (since it's entirely unreachable). Maybe it's not
    // correct
    //            // in other cases.
  ...
  // instructions.
  //   * The only scenario we need to insert a jump into IR from previously-compiled instructions,
  // is
  //   * when we insert a jump to a label; we exploit this and insert basic blocks for all of the
  // labels
  ...
  //   *
  //   * <p>Specifically, this returns the block inserted by {@link #ensureBbAt(BcLabel, String)},
  // which
  ...
  //   *
  //   * <p>Specifically, returns the BB in {@code bbByLabel} mapped to the position immediately
  // after,
  ...
  //    switch (instr) {
  //      case Return() -> {
  //        var retVal = pop(Node<? extends SEXP>.class);
  ...
  //        var bb = bbAt(label);
  //        cursor.addJump(new JumpData.Branch(get(ast), pop(Node<? extends SEXP>.class),
  // bbAfterCurrent(), bb));
  //        addPhiInputsForStack(bb);
  //      }
  ...
  //                  null, BuiltinId.PRINT_VALUE, ImmutableList.of(pop(Node<? extends
  // SEXP>.class)), env));
  ...
  //        // maintain a stack of loops (markers, `cond` and `end` blocks) within each
  // `CFGCompiler`.
  //        // In the simple non-promise cases, we encounter `break` or `next` while the loop stack
  // has
  //        // an element, which is the inner-most loop that we want to break or continue. In a
  // promise,
  //        // if there is no inner loop, the loop stack is empty, so we know to fail and can report
  // the
  //        // failure accurately.
  //
  //        // We already handle `for` loops in `StartFor` (due to the very specific bytecode
  // pattern of
  //        // for loops), but need to handle `while` and `repeat` since they don't have specific
  //        // bytecode instructions.
  //        if (isForLoop) {
  //          // Specifically, we step over this instruction if we encounter `StartFor`, so it's
  // never
  ...
  //        // This takes advantage of invariants expected between `StartFor/StepFor/EndFor` and
  // just
  //        // compiles the entire for loop logic (everything other than the loop forBody). We then
  // do a
  //        // weak sanity check that the bytecode upholds these invariants by including `StepFor`
  // in
  ...
  //          if (stepGoto == null) {
  //            throw fail(
  //                "StartFor followed by StartLoopCntxt, expected the StartLoopCntxt to be followed
  // by Goto");
  //          }
  //          if (!bbByLabel.containsKey(bcPos + 3)) {
  //            throw fail(
  //                "StartFor followed by StartLoopCntxt followed by Goto, expected a label to be
  // immediately after the Goto");
  //          }
  ...
  //
  //        // For loop init
  //        var seq = cursor.insert(new StmtData.ToForSeq(pop(Node<? extends SEXP>.class)));
  ...
  //        // For loop step
  //        moveTo(stepBb);
  //        var index = stepBb.addPhi(Node<? extends SEXP>.class);
  //        index.rename("index");
  ...
  //        // When we encounter `StepFor`, we know that the body is over. Note that we may no
  // longer be
  ...
  //          // The `StartLoopCntxt` could be made into a no-op, but the `Goto` would break this
  // setup
  ...
  //      }
  //      case DoLoopNext() -> {
  //        // The bytecode compilers don't actually generate these instructions, are they
  // deprecated?
  ...
  //        // This is the instruction immediately after the end of the for loop body (excluding
  // `next`
  //        // and `break`).
  //
  ...
  //        }
  //
  //        // We must explicitly move to `endBb` because there may not be a label corresponding to
  // it,
  ...
  //        // However it has a simple (unhelpful) implementation, so even if unused, we'll support
  // it.
  //        pop(Node<? extends SEXP>.class);
  //        pop(Node<? extends SEXP>.class);
  ...
  //        pushInsert(new StmtData.Force(pop(Node<? extends SEXP>.class), compileFrameState(),
  // env));
  //      }
  //      case DdVal(var name) -> {
  ...
  // env));
  //      }
  //      case SetVar(var name) -> insert(new StmtData.StVar(get(name), top(Node<? extends
  // SEXP>.class), env));
  ...
  //        // ???: GNU-R calls `SYMVALUE` and `INTERNAL` to implement these, but we don't store
  // that in
  ...
  //      case CheckFun() -> pushCallFunInsert(new StmtData.ChkFun(pop(Node<? extends
  // SEXP>.class)));
  //      case MakeProm(var code) -> {
  //        // REACH: Could cache compiled promise CFGs (env will always be different) in the
  // module...
  //        // TODO: infer name
  //        Promise promise;
  //        try {
  ...
  //      case PushArg() -> pushCallArg(pop(Node<? extends SEXP>.class));
  ...
  //        // Note that some of these are constant symbols and language objects, they should be
  // treated
  //        // like `eval`. We also assume builtins never use names.
  //        // GNU-R just passes `CDR(call)` to the builtin.
  //        var args =
  //            ast.args().stream()
  //                .map(arg -> (Node<? extends SEXP>) new Constant(arg.value()))
  ...
  //
  //        // Note that we don't use `module.getOrCompile` because the inner closure will probably
  // have
  //        // a unique environment, so we don't even bother trying to cache it outside of the outer
  //        // closure.
  //        // Also, we intentionally only compile the baseline version because the inner closure
  // will
  ...
  //        var rhs = top(Node<? extends SEXP>.class);
  ...
  //                    case 3 ->
  //                        new StmtData.Extract1_2D(ast, args.get(0), args.get(1), args.get(2),
  // env);
  //                    case 4 ->
  ...
  //                    case 3 ->
  //                        new StmtData.Extract2_2D(ast, args.get(0), args.get(1), args.get(2),
  // env);
  //                    default ->
  //                        throw new UnreachableError("index should've been range-checked above");
  //                  });
  ...
  //
  //        var target = top(Node<? extends SEXP>.class);
  ...
  //
  //        var rhs = pop(Node<? extends SEXP>.class);
  //        var target = top(Node<? extends SEXP>.class);
  //        compileGeneralDispatchCommon(BuiltinId.DOLLAR_GETS, false, get(ast), target, rhs,
  // after);
  //
  //        var call = popCall();
  ...
  //      }
  //      case IsNull() ->
  //          pushInsert(new StmtData.Eq(null, pop(Node<? extends SEXP>.class), new
  // Constant(SEXPs.NULL)));
  //      case IsLogical() -> pushInsert(new StmtData.GnuRIs(IsTypeCheck.LGL, pop(Node<? extends
  // SEXP>.class)));
  //      case IsInteger() -> pushInsert(new StmtData.GnuRIs(IsTypeCheck.INT, pop(Node<? extends
  // SEXP>.class)));
  //      case IsDouble() -> pushInsert(new StmtData.GnuRIs(IsTypeCheck.REAL, pop(Node<? extends
  // SEXP>.class)));
  //      case IsComplex() -> pushInsert(new StmtData.GnuRIs(IsTypeCheck.CPLX, pop(Node<? extends
  // SEXP>.class)));
  //      case IsCharacter() -> pushInsert(new StmtData.GnuRIs(IsTypeCheck.STR, pop(Node<? extends
  // SEXP>.class)));
  //      case IsSymbol() -> pushInsert(new StmtData.GnuRIs(IsTypeCheck.SYM, pop(Node<? extends
  // SEXP>.class)));
  //      case IsObject() -> pushInsert(new StmtData.GnuRIs(IsTypeCheck.NON_OBJECT, pop(Node<?
  // extends SEXP>.class)));
  //      case IsNumeric() -> pushInsert(new StmtData.GnuRIs(IsTypeCheck.NUM, pop(Node<? extends
  // SEXP>.class)));
  ...
  //        pushInsert(new StmtData.AsLogical(pop(Node<? extends SEXP>.class)));
  //        cursor.addJump(
  //            new JumpData.Branch(get(ast), top(Node<? extends SEXP>.class), bbAfterCurrent(),
  // shortCircuitBb));
  ...
  //        pushInsert(new StmtData.AsLogical(pop(Node<? extends SEXP>.class)));
  //        cursor.addJump(
  //            new JumpData.Branch(get(ast), top(Node<? extends SEXP>.class), shortCircuitBb,
  // bbAfterCurrent()));
  //        addPhiInputsForStack(shortCircuitBb);
  //      }
  //      case Or2nd(var ast) -> {
  ...
  //      case SetVar2(var name) -> insert(new StmtData.StVarSuper(get(name), top(Node<? extends
  // SEXP>.class), env));
  //      case StartAssign2(var name) -> {
  //        // GNU-R has "cells" and stores the assign on the main stack.
  //        // But we don't have cells, and since we're compiling, we can store the assignment on
  // its
  //        // own stack.
  ...
  //        var rhs = pop(Node<? extends SEXP>.class);
  //        var lhs = pop(Node<? extends SEXP>.class);
  //        prependCallArg(lhs);
  //        pushCallArg(rhs);
  //        compileCall(ast);
  //      }
  ...
  //        prependCallArg(top(Node<? extends SEXP>.class));
  //        compileCall(ast);
  //      }
  //      case SpecialSwap() -> {
  //        var value = pop(Node.class);
  ...
  //        var numLabels = numLabelsIdx == null ? null : get(numLabelsIdx);
  //
  //        var value = pop(Node<? extends SEXP>.class);
  //
  ...
  //
  //        // Has one predecessor (no phis) so we can call `cursor.moveToStart` instead of
  // `moveTo`.
  ...
  //
  //        // Has two predecessors, but the second defines no values (means no phis), so we can
  // call
  ...
  //        }
  //
  //        // The `switch` executes a GOTO on all branches, so the following code is unreachable
  // unless
  ...
  //      case Subassign2N(var _, var n) -> compileDefaultDispatchN(Dispatch.Type.SUBASSIGN2N, n +
  // 2);
  ...
  //          pushInsert(new StmtData.Math1(get(ast), funId, pop(Node<? extends SEXP>.class)));
  ...
  //        var args = funAndArgs.stream().map(a -> (Node<? extends SEXP>)
  // a).collect(ImmutableList.toImmutableList());
  //        funAndArgs.clear();
  //
  ...
  //          pushInsert(
  //              new StmtData.CallBuiltin(
  //                  get(ast), BuiltinId.SEQ_ALONG, ImmutableList.of(pop(Node<? extends
  // SEXP>.class)), env));
  //      case SeqLen(var ast) ->
  //          pushInsert(
  //              new StmtData.CallBuiltin(
  //                  get(ast), BuiltinId.SEQ_LEN, ImmutableList.of(pop(Node<? extends
  // SEXP>.class)), env));
  ...
  //                .map(v -> (Node<? extends SEXP>) new Constant(v))
  //                .collect(ImmutableList.toImmutableList());
  ...
  //        pop(Node<? extends SEXP>.class); // the `CallBuiltin` pushed above.
  //
  //        // Has one predecessor (no phis) so we can call `cursor.moveToStart` instead of
  // `moveTo`.
  ...
  //    StmtData.Node<? extends SEXP>_ make(@Nullable LangSXP ast, Node<? extends SEXP> value);
  //  }
  //
  //  @FunctionalInterface
  //  private interface MkBinopFn {
  //    StmtData.Node<? extends SEXP>_ make(@Nullable LangSXP ast, Node<? extends SEXP> lhs, Node<?
  // extends SEXP> rhs);
  //  }
  //
  //  private StmtData.Node<? extends SEXP>_ mkUnop(ConstPool.Idx<LangSXP> ast, MkUnopFn unop) {
  //    return unop.make(get(ast), pop(Node<? extends SEXP>.class));
  //  }
  //
  //  private StmtData.Node<? extends SEXP>_ mkBinop(ConstPool.Idx<LangSXP> ast, MkBinopFn binop) {
  //    var rhs = pop(Node<? extends SEXP>.class);
  //    var lhs = pop(Node<? extends SEXP>.class);
  ...
  //
  //  /**
  //   * End the previously-compiled for loop instruction (the latest {@link
  // #pushWhileOrRepeatLoop(BB,
  //   * BB)} or {@link #pushForLoop(BB, BB, BB)}): pop its data, assert that the loop is of the
  // correct
  ...
  //            // If there is a `break` instruction in a for loop, there will be a label at
  // `EndFor`,
  //            // so `loop.end` will only consist of a `Goto` to the "real" end BB. This is
  // allowed,
  ...
  //  }
  //
  //  private void compileStartDispatch(Dispatch.Type type, ConstPool.Idx<LangSXP> ast, BcLabel
  // after) {
  //    compileStartDispatch(type, get(ast), bbAt(after));
  //  }
  //
  //  private void compileStartDispatch(Dispatch.Type type, LangSXP ast, BB after) {
  //    var rhs = type.hasRhs ? pop(Node<? extends SEXP>.class) : null;
  //    var target = top(Node<? extends SEXP>.class);
  ...
  //      BuiltinId fun, boolean isNForm, LangSXP ast, Node<? extends SEXP> target, @Nullable Node<?
  // extends SEXP> rhs, BB after) {
  ...
  //    var target1 = pop(Node<? extends SEXP>.class);
  ...
  //      int minNumArgs,
  //      int maxNumArgs,
  //      BiFunction<LangSXP, List<Node<? extends SEXP>>, StmtData.Node<? extends SEXP>_>
  // dispatchInstrData) {
  //    assert !type.isNForm : "use compileDefaultDispatchN for `...N` bytecodes";
  //
  //    if (type.hasRhs) {
  //      pushCallArg(pop(Node<? extends SEXP>.class));
  //    }
  //
  ...
  //    } else {
  //      // Instead of compiling a call, insert our specialized instruction with the call
  // arguments.
  ...
  //      Dispatch.Type type,
  //      int numArgs,
  //      @Nullable BiFunction<LangSXP, List<Node<? extends SEXP>>, StmtData.Node<? extends SEXP>_>
  // dispatchInstrData) {
  ...
  //      if (!(arg instanceof Node<? extends SEXP>)) {
  //        throw fail(
  //            "expected Node<? extends SEXP> on stack for `compileDefaultDispatchN("
  //                + type
  //                + ", "
  //                + numArgs
  //                + ", ...)`");
  ...
  //    var args = (List<Node<? extends SEXP>>) (List<?>) uncastedArgs;
  //
  //    // Unlike `compileDefaultDispatch`, there's no call, instead we take the args from the
  // stack.
  //    // Like `compileDefaultDispatch`, there's either a specialized instruction or a fallback
  // call,
  //    // although in this the number of arguments is known.
  //    var result =
  //        dispatchInstrData == null
  //            ? new StmtData.CallSafeBuiltin(dispatch.ast, type.builtin,
  ...
  //            .collect(ImmutableList.toImmutableList());
  //    var args = call.args.stream().map(arg ->
  // arg.value).collect(ImmutableList.toImmutableList());
  ...
  //    // when the bytecode compiler compiles a `switch` with a missing label, it inserts a branch
  // that
  ...
  //    // stack to be balanced, and this call return is unaccounted for. Therefore, we have to
  // special-
  ...
  //    // special-case (unless there's another compiler bug causing stack imbalances, and even
  // then,
  ...
  //    // call, which is different than the AST interpreter which errors regardless. This and the
  // fact
  //    // that the stack now has an extra value until the function returns, are why it may be a
  // bug.
  ...
  //   *
  //   * <p>Replacing the stack with phis is part of converting a stack-based bytecode to SSA-based
  // IR.
  ...
  //   * later.
  //   *
  //   * <p>If the current block is a predecessor to the given block, the given block's stack will
  // have
  //   * a phi for each stack node in the current block, and one of the inputs will be said node
  // (see
  //   * {@link #addPhiInputsForStack(BB)}). We call {@link #addPhiInputsForStack(BB)} here for
  //   * convenience, although it makes this method have 2 functions (add phis if the next block is
  // a
  ...
  //          : "tried to move to BB "
  //              + bb.id()
  //              + " which isn't a successor and didn't have phis added yet, so we don't know what
  ...
  //   *
  //   * <p>The currently block is required to be one of the given block's predecessors. This needs
  // to
  //   * be called for every jump to a block returned by {@link #bbAt(BcLabel)} in order to uphold
  // the
  ...
  //   *
  //   * <p>This is the same as {@link #addPhiInputsForStack(BB)} except uses {@code incoming}
  // instead
  ...
  //   */
  //  private void pushInsert(StmtData.Node<? extends SEXP>_ data) {
  //    push(cursor.insert(data));
  //  }
  //
  //  /** {@link #pushInsert(Node<? extends SEXP>_)} but gives the instruction the name of the
  // symbol. */
  //  private void pushInsert(ConstPool.Idx<RegSymSXP> name, StmtData.Node<? extends SEXP>_ data) {
  //    push(cursor.insert(get(name).name(), data));
  //  }
  //
  //  /**
  //   * Insert a statement that produces a value, and begin a call with it as the function via
  // {@link
  //   * #pushCall(Node<? extends SEXP>)}.
  //   */
  //  private void pushCallFunInsert(StmtData.Node<? extends SEXP>_ data) {
  //    pushCall(cursor.insert(data));
  //  }
  //
  //  /** {@link #pushCallFunInsert(Node<? extends SEXP>_)} but gives the instruction the name of
  // the symbol. */
  //  private void pushCallFunInsert(ConstPool.Idx<RegSymSXP> name, StmtData.Node<? extends SEXP>_
  // data) {
  //    pushCall(cursor.insert(get(name).name(), data));
  //  }
  //
  //  /**
  //   * Insert a statement that produces a value, and add it to the current call arguments via
  // {@link
  //   * #pushCallArg(Node<? extends SEXP>)}.
  //   */
  //  private void pushCallArgInsert(StmtData.Node<? extends SEXP>_ data) {
  //    pushCallArg(cursor.insert(data));
  //  }
  //
  //  /** {@link #pushCallArgInsert(Node<? extends SEXP>_)} but gives the instruction a name. */
  //  private void pushCallArgInsert(String name, StmtData.Node<? extends SEXP>_ data) {
  ...
  //  }
  //
  //  // - Since we're converting from stack-based, temporal bytecode to SSA form, we use the
  // virtual
  //  //   stacks to keep track of data from previous instructions that affects future instructions
  //  //   (specifically, that data is what's "pushed", when the future instructions use the data
  // it's
  ...
  //   *
  //   * <p>If we return in a loop, the loop stack won't be empty. Additionally, if we return in a
  // for
  //   * loop, the node stack will contain the loop index.
  //   *
  //   * <p>Otherwise, the bytecode compiler pops everything before returning, so the stack should
  // be
  ...
  //   * interpreter stack, it pushes the node that stores their abstract value ({@link Node}) onto
  // the
  //   * "virtual stack"; and when the compiler encounters {@link BcInstr.Pop} and other
  // instructions
  ...
  //
  //  /**
  //   * Pop a node from the "virtual stack" that was pushed by the last call to {@link
  // #push(Node)}.
  //   *
  ...
  //   *
  //   * <p>"Local" because promises and `eval` are weird and can reference the loop of the
  // environment
  //   * they are provided, which is a long-jump in GNU-R. We don't currently support compiling
  // this.
  //   */
  ...
  //   *
  //   * <p>GNU-R stores the complex assign values on the main stack, but we also store them on
  // their
  ...
  //   * the future.
  //   */
  //  private void pushComplexAssign(boolean isSuper, RegSymSXP name, Node<? extends SEXP> lhs,
  // Node<? extends SEXP> rhs) {
  ...
  //   * #pushComplexAssign(boolean, RegSymSXP, Node<? extends SEXP>, Node<? extends SEXP>)}),
  // assert that the given {@code isSuper}
  //   * and {@code name} match its own, and remove it from the stack.
  //   */
  //  private Node<? extends SEXP> popComplexAssign(boolean isSuper, RegSymSXP name) {
  ...
  //    }
  //
  //    return pop(Node<? extends SEXP>.class);
  //  }
  //
  //  /** Begin a new call of the given function (abstract value). */
  //  private void pushCall(Node<? extends SEXP> fun) {
  ...
  //
  //  /**
  //   * Get the current call (the one that was pushed by the last call to {@link #pushCall(Node<?
  // extends SEXP>)}).
  //   */
  ...
  //
  //  /**
  //   * Pop the current call (the one that was pushed by the last call to {@link #pushCall(Node<?
  // extends SEXP>)}).
  //   */
  ...
  //   *
  //   * <p>This is required for {@link GetterCall} and {@link SetterCall}, since the {@code lhs} of
  // the
  //   * complex assignment is prepended in the GNU-R. See also {@link #pushCallArg(Node<? extends
  // SEXP>)}, which
  //   * appends the argument.
  //   */
  //  private void prependCallArg(Node<? extends SEXP> value) {
  ...
  // later.
  //   */
  //  private void pushCallArg(Node<? extends SEXP> value) {
  ...
  //   *
  //   * @see #pushCallArg(Node<? extends SEXP>)
  //   */
