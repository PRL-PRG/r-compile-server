...
public sealed interface InstrOrPhi extends NodeWithCfg permits Instr, Phi {
  /** (A view of) the instruction or phi's arguments, which are the other nodes it depends on.
   * If a phi, these are its {@link Phi#inputs()} without the extra type information or associated
   * basic block. */
  @UnmodifiableView List<Node> args();
   ...
   */
  @UnmodifiableView List<Node> returns();
  ...
  /** Itself. */
  @Override
  default InstrOrPhi origin() { return this; }
}
