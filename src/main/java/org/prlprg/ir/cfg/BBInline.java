...
interface BBInline extends BBCompoundMutate {
  ...
  default BB inlineAt(int index, CFG cfgToInline) {
    ...
    var oldToNewArgs = new HashMap<Node, Node>(cfgToInline.numNodes());
    BiConsumer<InstrOrPhi, InstrOrPhi> prePatchInstrOrPhi =
        (oldInstrOrPhi, newInstrOrPhi) -> {
          ...
          for (var i = 0; i < oldReturns.size(); i++) {
            oldToNewArgs.put(oldReturns.get(i), newReturns.get(i));
          }
        };
    for (var oldBB : cfg().iter()) {
      ...
      for (var oldStmt : oldBB.stmts()) {
        var newStmt = newBB.insertAt(newBB.stmts().size(), oldStmt.id().name(), oldStmt.data());
        prePatchInstrOrPhi.accept(oldStmt, newStmt);
      }
      if (oldBB.jump() != null) {
        var newJump =
            newBB.addJump(oldBB.jump().id().name(), oldBB.jump().data().replaceReturnWith(lastBB));
        for (var oldTarget : newJump.targets()) {
          var newTarget = Objects.requireNonNull(oldToNewBBs.get(oldTarget.id()));
          newJump.replace(oldTarget, newTarget);
        }
        prePatchInstrOrPhi.accept(oldBB.jump(), newJump);
      }
    }
    // Replace node arguments
    for (var newInstrOrPhi : newInstrOrPhis) {
      for (var oldArg : newInstrOrPhi.args()) {
        // If `newArg` is a global it won't be in the list.
        var newArg = oldToNewArgs.getOrDefault(oldArg, oldArg);
        assert newArg.cfg() == null || newArg.cfg() == cfg();

        newInstrOrPhi.replace(oldArg, newArg);
      }
    }
