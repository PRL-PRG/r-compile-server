...
public record RArgumentType(String name, RType type) {
  /** Create an argument type list from the argument list (uses {@link RType#exact}). */
  ...
  /** Create an argument type from the argument (uses {@link RType#exact}). */
  public static RArgumentType exact(TaggedElem argument) {
    return new RArgumentType(argument.tagOrEmpty(), RType.exact(argument.value()));
  }
