 ...
 * promise is evaluated ("forced") when its parameter gets read (e.g. "f <- function(a, b) if (b <
 * 10) a else 0"" will evaluate "b", then evaluate "a" if the result is less than 10). The promise
 * expression's AST can also be accessed without forcing the promise by passing it to the substitute
 * function (e.g. "f <- function(a) substitute(a)", "f(b + c)" will return a quoted AST "b + c".
 * Besides returning quoted ASTs, "substitute" can be combined with "eval" to evaluate the AST in a
 * different environment, reinterpreted like "plot(y = 2 * x + 5)" will plot a graph of the function
 * "2 * x + 5" with "y" on the y-axis and "x" on the x-axis, or more).
 *
 * <p>For more information, see <a href="https://adv-r.hadley.nz/functions.html#promises">Advanced R
 * on promises</a>.
 *
 * @param expr The expression that will be evaluated when the promise is forced. This is either the
 *     AST of the expression or bytecode, which contains the ASt of the expression as its source
