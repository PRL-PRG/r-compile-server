 ...
 *
 * <p>We don't track its contents because 1) it may be shared across compilations and we compile
 * SEXPs in a pure context (we don't want compiling one SEXP to produce side-effects which affect
 * compiling others), 2) we want to compile SEXPs in a more general context than their exact
 * top-level environment, so they can be reused.
 */
